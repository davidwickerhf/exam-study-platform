...
export function useCourseCanvas() {
  ...
  const refreshStatus = useCallback(async () => {
    ...
    try {
      const body = await json('/api/account/integrations/canvas/corpus')
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [connectionResult, statusResult] = await Promise.allSettled([json('/api/account/integrations/canvas'), refreshStatus()])
      ...
      const results = await Promise.allSettled((body.connections || []).map(async (connection: Connection) => {
        const data = await json(`/api/integrations/canvas/courses?canvasUrl=${encodeURIComponent(connection.origin)}`)
  ...
  useEffect(() => {
    if (!active) return
    const poll = () => { if (!document.hidden) void refreshStatus().catch(() => setActionError('Collection status could not be refreshed. Check again below.')) }
  ...
  const collect = async (shells: CanvasShell[]) => {
    ...
    if (mounted.current) {
      ...
      try { await refreshStatus() } catch { setActionError(current => [current, 'Collection status could not be refreshed. Check again below.'].filter(Boolean).join(' ')) }
      setBusy(false)
    }
