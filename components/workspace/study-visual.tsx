...
export function StudyVisual({ visual }: { visual: StudyVisualSpec }) {
  ...
  if (d.kind === 'process') {
    ...
    description = <div className="mt-4 border-l-2 border-primary pl-4" aria-live="polite"><p className="text-sm font-medium"><StudyInline>{d.nodes[index].label}</StudyInline></p><p className="mt-1 text-sm leading-relaxed text-muted-foreground"><StudyInline>{d.nodes[index].description}</StudyInline></p><ul className="mt-2 space-y-1 text-xs text-muted-foreground">{d.edges.filter(e => e.from === d.nodes[index].id).map((e,i) => <li key={i}><StudyInline>{`${e.label || 'Leads to'} → ${d.nodes.find(n => n.id === e.to)?.label || ''}`}</StudyInline></li>)}</ul></div>
    fallback = <ol className="space-y-2">{d.nodes.map(n => <li key={n.id}><strong><StudyInline>{n.label}</StudyInline>:</strong> <StudyInline>{n.description}</StudyInline></li>)}{d.edges.map((e,i) => <li key={`edge-${i}`}><StudyInline>{`${d.nodes.find(n => n.id === e.from)?.label || ''} → ${d.nodes.find(n => n.id === e.to)?.label || ''}: ${e.label}`}</StudyInline></li>)}</ol>
