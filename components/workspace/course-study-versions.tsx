...
import { ArrowRightIcon, PlusIcon, BookOpenIcon, TargetIcon, FolderOpenIcon, LockKeyholeIcon, LibraryIcon } from 'lucide-react'
...
export function CourseStudyVersions({
  ...
  return <section aria-label="Your study guides">
    ...
    {error ? <Alert variant="destructive"><AlertDescription>{error}<Button variant="outline" size="sm" className="mt-3" onClick={()=>setLoadRevision(n=>n+1)}>Try again</Button></AlertDescription></Alert>
      ...
      : active ? <>
        {selected.length > 1 && <label className="mb-5 flex flex-wrap items-center gap-3 text-sm text-muted-foreground">Study guide<select aria-label="Study guide" className="h-10 max-w-full rounded-md border bg-card px-3 text-foreground" value={active.id} onChange={e=>choose(e.target.value)}>{selected.map(v=><option key={v.id} value={v.id}>{v.title}{v.bundle ? ' · Whole course' : v.managed ? ' · Part of course run' : ''}</option>)}</select></label>}
        <div className="course-guide-heading course-band">
          <div className="min-w-0"><h3 className="text-lg font-semibold leading-6">{active.title}</h3><div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground"><span className="inline-flex items-center gap-1.5"><LockKeyholeIcon className="size-3"/>Private guide</span><span>{active.course.academicYear}</span><span>{active.bundle ? `Whole course · ${active.bundle.guides.length} ${active.bundle.guides.length === 1 ? 'guide' : 'guides'} planned` : active.activeRevisionId ? `${active.chapterPreviews?.length || active.history[0]?.chapters || 0} ${(active.chapterPreviews?.length || active.history[0]?.chapters) === 1 ? 'chapter' : 'chapters'}` : generationLabel(active.draft)}</span>{active.managed && <Link href={`/app/study/${active.managed.parentVersionId}`} className="inline-flex items-center gap-1.5 text-primary hover:underline"><LibraryIcon className="size-3"/>Part of course run</Link>}</div></div>
          <Link href={`/app/study/${active.id}`} className={buttonVariants({size:'sm'})}>{active.bundle ? 'View course run' : active.activeRevisionId ? 'Open guide' : 'View generation'}<ArrowRightIcon/></Link>
        </div>
