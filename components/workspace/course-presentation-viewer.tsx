...
export default function CoursePresentationViewer({ assetId, title, initialPage = 1, onPageChange }: { assetId: string; title: string; initialPage?: number; onPageChange?: (page:number)=>void }) {
  ...
  return <div className="flex min-h-0 flex-1 flex-col gap-3">
    ...
    {text ? <Text assetId={assetId} /> : <Pdf url={`/api/corpus/assets/${encodeURIComponent(assetId)}/slides.pdf`} title={title} initialPage={initialPage} onPageChange={onPageChange} slides />}
  </div>
}
