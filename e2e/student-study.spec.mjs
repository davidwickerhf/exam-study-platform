...
test('material viewers render PDF and slide graphics, preserve extracted text, and show code and notebook outputs', async ({ page }) => {
  ...
  const entries = [
    { assetId:'viewer-html', filename:'Canvas announcement.html', mediaType:'text/html', bytes:Buffer.from('<h1>Choose your team</h1><p>Find <strong>two partners</strong>.</p><script>window.previewExecuted=true</script><img src="https://evil.test/pixel" onerror="window.previewExecuted=true"><a href="javascript:alert(1)">Unsafe link</a><a href="https://canvas.example.test/course">Canvas source</a>') },
    { assetId:'viewer-markdown', filename:'Course links.md', mediaType:'text/markdown', bytes:Buffer.from('# Course links\n\n- [Canvas](https://canvas.example.test/course)\n\n| Team | Size |\n| --- | --- |\n| A | 3 |\n\n```python\nprint(3)\n```\n\n[Bad](javascript:alert(1))\n<script>window.previewExecuted=true</script>') },
  ...
  await dialog.getByRole('button',{name:'Ask tutor',exact:true}).click()
  const documentTutor=page.getByRole('complementary',{name:'Document tutor',exact:true})
  await expect(documentTutor).toBeVisible()
  await expect(dialog.getByRole('spinbutton',{name:'Page number'})).toHaveValue('2')
  await expect(documentTutor.getByText('Source: Course manual.pdf · Page 2', {exact:true})).toBeVisible()
  expect(page.url()).toBe(documentUrl)
  await page.screenshot({path:'/tmp/wicker-document-tutor.png'})
  await documentTutor.getByRole('button',{name:'Close document tutor'}).click()
  await expect(dialog.getByRole('spinbutton',{name:'Page number'})).toHaveValue('2')
  ...
  await page.route('https://evil.test/**', route=>{unsafeRequests++;return route.abort()})
  await page.getByRole('button',{name:/^Canvas announcement /}).click()
  const html=page.frameLocator('iframe[title="HTML document"]')
  await expect(html.getByRole('heading',{name:'Choose your team'})).toBeVisible()
  await expect(html.getByRole('link',{name:'Canvas source'})).toHaveAttribute('rel','noopener noreferrer')
  await expect(html.locator('script,img,form,svg')).toHaveCount(0)
  await expect(html.getByRole('link',{name:'Unsafe link'})).toHaveCount(0)
  expect(await page.locator('iframe[title="HTML document"]').getAttribute('sandbox')).not.toMatch(/allow-scripts|allow-same-origin/)
  expect(await page.evaluate(()=>window.previewExecuted)).toBeUndefined()
  expect(unsafeRequests).toBe(0)
  await page.screenshot({path:'/tmp/wicker-html-document.png'})
  await page.keyboard.press('Escape')
  await page.getByRole('button',{name:/^Course links /}).click()
  await expect(dialog.getByRole('heading',{name:'Course links',exact:true})).toBeVisible()
  await expect(dialog.getByRole('table')).toBeVisible()
  await expect(dialog.getByRole('link',{name:'Canvas',exact:true})).toHaveAttribute('target','_blank')
  await expect(dialog.getByRole('link',{name:'Bad',exact:true})).toHaveCount(0)
  await page.setViewportSize({width:390,height:844})
  await expect.poll(()=>page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true)
  await page.screenshot({path:'/tmp/wicker-markdown-mobile.png'})
  await page.getByRole('button',{name:'Close reference',exact:true}).first().click()
  await page.setViewportSize({width:1280,height:900})
...
test('source-grounded study, persisted notes, exercises, mock exam and private sharing', async ({
  ...
  await page.locator('summary').filter({hasText:/^Chapter options$/}).click()
  await expect(
  ...
  await page.locator('summary').filter({hasText:/^Guide options$/}).click()
  await page.getByRole('button', { name: 'Build practice exam', exact: true }).click()
  await page
...
test('guided lesson exposes interactive diagrams, optional depth and progressive practice on desktop and mobile', async ({ page }) => {
  ...
  await page.locator('.study-reader-goals summary').click()
  await expect(page.locator('.study-reader-goals li .katex').first()).toBeVisible()
...
test('chapter workspace restores course papers, free answer-key grading, source inspection and tutor panel', async ({page}) => {
  ...
  await expect(focusedPaper.getByRole('button', { name: 'Focus document', exact: true })).toBeVisible()
  await expect(page.getByRole('region', { name: 'Past paper library' })).toBeVisible()
  await focusedPaper.getByRole('button', { name: 'Focus document', exact: true }).click()
...
test('course exercises reuse Practice, include personal guides and keep course navigation compact', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: /Start studying|Continue studying|Review guide/ })).toBeVisible()
  await page.getByText('Explore chapters', {exact:true}).click()
  await expect(page.locator('.study-library-guide ol')).toBeVisible()
  ...
  await page.getByRole('button', {name:/^Guide practice/}).click()
  await page.locator('.question-overview .question-session-map button').first().click()
  ...
  await page.setViewportSize({ width: 1280, height: 900 })
  await page.getByRole('button', { name: 'Back to question bank', exact: true }).click()
...
test('course materials send the opened document and current PDF page to the tutor',async({page})=>{
  ...
  const tutor=page.getByRole('complementary',{name:'Document tutor'})
...
test('group directory exposes long names, filters by year and course, and keeps browsing state after roster review', async({page})=>{
  ...
  await expect(page.getByRole('combobox',{name:'Academic year'})).toHaveValue('2026-2027')
  await expect(page.getByRole('button',{name:'View members of Project team 13',exact:true})).toHaveCount(0)
  await expect(page.locator('option[value=all]')).toHaveCount(0)
  await page.getByRole('combobox',{name:'Academic year'}).selectOption('2025-2026')
  await expect(page.getByRole('button',{name:/View members of/})).toHaveCount(12)
  await expect(page.getByRole('button',{name:'View members of Project team 13',exact:true})).toBeVisible()
  await page.reload()
  await expect(page.getByRole('combobox',{name:'Academic year'})).toHaveValue('2025-2026')
  await page.getByRole('button',{name:/^Global groups/}).click()
  await page.getByRole('button',{name:/^Course teams/}).click()
  await expect(page.getByRole('combobox',{name:'Academic year'})).toHaveValue('2025-2026')
  await page.getByRole('combobox',{name:'Academic year'}).selectOption('2026-2027')
...
test('project steps preserve relative deadlines and open original evidence in place',async({page})=>{
  await page.clock.setFixedTime(new Date('2026-09-07T08:00:00Z'))
  const action={title:'Upload presentation slides',parent:'Group project',kind:'submission',deadline:null,deadlineText:'One hour before your presentation',prerequisite:'After your group receives its slot',notes:'Upload a PDF of your slides.',evidence:[{chunkId:1,assetId:'project-manual',title:'Project manual.pdf',page:2}]}
  await page.route('**/api/workspace-shell',async route=>{const response=await route.fetch(),body=await response.json();await route.fulfill({json:{...body,priorityCourses:[{id:'iot',code:'BCS3120',name:'Internet of Things',courseProfile:{assessment:{status:'confirmed',actions:[action]}}}]}})})
  await page.route('**/api/calendar/events',route=>route.fulfill({json:{events:[],attendance:{summary:{},courses:[]}}}))
  await page.route('**/api/integrations/canvas/hub?*',route=>route.fulfill({json:{connected:true,assignments:[]}}))
  await page.route('**/api/corpus/assets/project-manual',route=>route.fulfill({contentType:'application/pdf',body:previewPdf('Upload slides one hour before your presentation',2)}))
  await page.goto('/app/priorities')
  const region=page.getByRole('region',{name:'All priorities',exact:true})
  await expect(region).toContainText('One hour before your presentation')
  await expect(region).toContainText('After your group receives its slot')
  await region.getByText('View source',{exact:true}).click()
  await region.getByRole('button',{name:'Open document',exact:true}).click()
  await expect(page.getByRole('dialog')).toContainText('Project manual.pdf')
  await expect(page.locator('[data-pdf-page="2"] .pdf-reader-text')).toContainText('Upload slides one hour')
  await expect(page).toHaveURL(/\/app\/priorities$/)
  await page.getByRole('button',{name:'Close',exact:true}).click()
  await expect(page.getByRole('dialog')).not.toBeVisible()
  await page.screenshot({path:'/tmp/wicker-priority-steps-desktop.png',fullPage:true})
  await page.setViewportSize({width:390,height:844})
  await expect.poll(()=>page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth)).toBe(true)
...
test('study reader keeps chapters discoverable, searchable and navigable at desktop and mobile widths', async ({ page }) => {
  await page.route('**/api/tutor**', route=>route.fulfill({json:{available:true,conversation:null,conversations:[],receipts:[],memory:{facts:[],plans:[]},attachments:[]}}))
  await page.route(`**/api/study-versions/${versionId}`, async route => {
    if (route.request().method() !== 'GET') return route.continue()
    const response = await route.fetch()
    const payload = await response.json()
    const first = payload.revision.chapters[0]
    // Exercise a realistic multi-chapter layout without generating extra AI content.
    payload.revision.chapters = [first, { ...first, id: 'subtraction', title: 'Subtraction and checking your work' }, { ...first, id: 'units', title: 'Working with quantities and units' }]
    payload.revision.topics = payload.revision.chapters.map(chapter => ({ ...payload.revision.topics[0], id: chapter.id, title: chapter.title }))
    payload.revision.topics.push({ ...payload.revision.topics[0], id: 'pending', title: 'Multiplication' })
    payload.progress = [{ topicId: first.id, read: true }]
    await route.fulfill({ response, json: payload })
  })
  await page.setViewportSize({ width: 1536, height: 1000 })
  await page.goto(`/app/study/${versionId}`)
  const contents = page.getByRole('navigation', { name: 'Study chapters', exact: true })
  await expect(contents).toBeVisible()
  await expect(page.getByRole('progressbar', { name: 'Chapters marked read' })).toHaveAttribute('value', '1')
  await expect(contents.getByRole('button', { name: /Multiplication/ })).toBeDisabled()
  await page.getByLabel('Find a chapter').fill('no match')
  await expect(page.getByRole('status').filter({ hasText: 'No chapters match' })).toBeVisible()
  await page.getByLabel('Find a chapter').fill('checking')
  await expect(contents.getByRole('button')).toHaveCount(1)
  await contents.getByRole('button').click()
  await expect(page.getByRole('heading', { name: 'Subtraction and checking your work', exact: true })).toBeFocused()
  await expect(page).toHaveURL(/chapter=subtraction/)
  await page.getByLabel('Find a chapter').fill('')
  await page.evaluate(()=>window.scrollTo(0,0))
  await page.screenshot({ path: '/tmp/wicker-study-redesign-desktop.png' })
  await page.getByRole('button', { name: 'Study with tutor', exact: true }).click()
  await expect(page.getByRole('complementary', { name: 'Course tutor', exact: true })).toBeVisible()
  await expect(page.getByRole('textbox', { name: 'Ask the tutor' })).toBeVisible()
  await page.screenshot({ path: '/tmp/wicker-reader-tutor-desktop.png' })
  await page.setViewportSize({ width: 390, height: 844 })
  await expect(page.getByRole('complementary', { name: 'Course tutor', exact: true })).toBeVisible()
  await page.screenshot({ path: '/tmp/wicker-reader-tutor-mobile.png' })
  await page.getByRole('button', { name: 'Back to study', exact: true }).click()
  await page.setViewportSize({ width: 1536, height: 1000 })
  await page.getByRole('complementary', { name: 'Course tutor', exact: true }).getByRole('button', { name: 'Close reference', exact: true }).click()
  ...
  await page.locator('.study-reader-picker').click()
  await expect(contents).toBeVisible()
  await contents.getByRole('button', { name: /Working with quantities/ }).click()
  await expect(contents).toBeHidden()
  await expect(page.getByRole('heading', { name: 'Working with quantities and units', exact: true })).toBeFocused()
  await expect(page.getByRole('button', { name: 'Next chapter', exact: true })).toBeDisabled()
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true)
  await page.evaluate(()=>window.scrollTo(0,0))
  await page.screenshot({ path: '/tmp/wicker-study-redesign-mobile.png' })
  await page.reload()
  await expect(page.getByRole('heading', { name: 'Working with quantities and units', exact: true })).toBeVisible()
})

test('question overview exposes the whole bank by paper and guide while answering stays focused', async ({ page }) => {
  const questions = Array.from({ length: 30 }, (_, index) => ({
    id: `overview-${index}`, question: `Question ${index + 1}: Explain how to check the result of adding two disjoint groups.`, expected: 'Subtract one group from the total.', type: 'written', courseId: course.courseCode, courseCode: course.courseCode, chapterId: index < 10 ? 'paper' : index < 20 ? 'addition' : 'checking', chapterName: index < 10 ? 'Original exam questions' : index < 20 ? 'Addition' : 'Checking your work', guideTitle: index < 10 ? 'Practice exam 2025' : 'Foundations study guide', practiceOrigin: index < 10 ? 'paper' : 'generated', source: index < 10 ? 'Original practice exam · 2025' : 'Generated chapter practice', study: { versionId, revisionId:'overview',topicId:index < 10 ? 'paper' : 'addition',questionId:`q${index}`,setId:index < 10 ? 'paper-set' : null }
  }))
  await page.route('**/api/practice', route=>route.fulfill({json:{questions,courses:[],source:'personal',generated:true}}))
  await page.setViewportSize({width:1440,height:1000})
  await page.goto('/app/practice')
  const overview = page.getByRole('region',{name:'Question overview'})
  await expect(overview.locator('.question-session-map button')).toHaveCount(30)
  await expect(page.getByRole('button',{name:'Check answer',exact:true})).toHaveCount(0)
  await page.screenshot({path:'/tmp/wicker-practice-overview-desktop.png'})
  await page.getByRole('button',{name:/^From course papers/}).click()
  await expect(overview.locator('.question-session-map button')).toHaveCount(10)
  await overview.locator('.question-session-map button').nth(8).click()
  await expect(overview).toHaveCount(0)
  await expect(page.getByText(questions[8].question,{exact:true})).toBeVisible()
  await page.getByRole('textbox',{name:'Your answer',exact:true}).fill('I would subtract one group to check the total.')
  await page.getByRole('button',{name:'Next',exact:true}).click()
  await page.getByRole('button',{name:'Previous',exact:true}).click()
  await expect(page.getByRole('textbox',{name:'Your answer',exact:true})).toHaveValue('I would subtract one group to check the total.')
  await page.screenshot({path:'/tmp/wicker-practice-question-desktop.png'})
  await page.getByRole('button',{name:'Back to question bank'}).click()
  await page.getByRole('button',{name:/^Guide practice/}).click()
  await expect(overview.locator('.question-session-map button')).toHaveCount(20)
  await expect(overview.getByRole('heading',{name:'Foundations study guide'})).toHaveCount(2)
  await page.setViewportSize({width:390,height:844})
  await page.screenshot({path:'/tmp/wicker-practice-overview-mobile.png'})
  await overview.locator('.question-session-map button').nth(15).click()
  await expect(page.getByText(questions[25].question,{exact:true})).toBeVisible()
  await expect(page.getByRole('button',{name:'Work through this with tutor'})).toBeVisible()
  expect(await page.evaluate(()=>document.documentElement.scrollWidth <= innerWidth)).toBe(true)
  await page.evaluate(()=>window.scrollTo(0,0))
  await page.screenshot({path:'/tmp/wicker-practice-question-mobile.png'})
})

test('guide library lists every guide with real progress and materials exposes year choices', async ({ page }) => {
  await page.route('**/api/account/integrations/canvas/corpus?view=summary',route=>route.fulfill({json:{status:{courses:[{courseCode:course.courseCode,courseName:course.courseName,editions:[{academicYear:'2026-2027',sources:2},{academicYear:'2025-2026',sources:4},{academicYear:'2024-2025',sources:6}]}],latestJobs:[],jobs:[]}}}))
  await page.route('**/api/corpus/materials?*',route=>route.fulfill({json:{materials:['Lecture notes.pdf','Practice questions.pdf','Course reading.pdf'].map((filename,i)=>({assetId:`material-${i}`,filename,sourcePath:filename,sourceType:'file',mediaType:'application/pdf',byteSize:1200,academicYear:'2026-2027',period:'1',current:true,url:`/api/corpus/assets/material-${i}`}))}}))

  await run(async()=>{const programmeId=await activeProgrammeId();await mutateStudyVersion(versionId,v=>{v.programmeId=programmeId})})
  await page.route('**/api/state',route=>route.fulfill({json:{courses:[{id:course.courseCode,code:course.courseCode,name:course.courseName,chapters:[],items:[]}]}}))
  await page.route('**/api/study-versions?*',async route=>{
    const response=await route.fetch(), payload=await response.json(), guide=payload.versions.find(v=>v.id===versionId)
    expect(guide.chapterPreviews[0]).toHaveProperty('read')
    payload.versions=Array.from({length:6},(_,i)=>({...guide,id:`${versionId}-${i}`,title:['Foundations of reasoning','Search strategies','Working with quantities','Checking your solutions','Applications and examples','Exam revision'][i],chapterPreviews:[{id:'first',title:'Understand the ideas',read:i<3},{id:'second',title:'Put it into practice',read:i===0}]}))
    await route.fulfill({response,json:payload})
  })
  await page.setViewportSize({width:1440,height:1000})
  await page.goto(`/app/courses/${course.courseCode}?tab=study&year=${course.academicYear}`)
  await expect(page.locator('.study-library-guide')).toHaveCount(6)
  await expect(page.getByRole('combobox',{name:'Study guide',exact:true})).toHaveCount(0)
  await expect(page.getByRole('link',{name:'Continue studying',exact:true})).toHaveCount(2)
  await page.screenshot({path:'/tmp/wicker-guide-library-desktop.png'})
  await page.getByLabel('Find a study guide').fill('Search strategies')
  await expect(page.locator('.study-library-guide')).toHaveCount(1)
  await page.getByLabel('Find a study guide').fill('')
  await page.setViewportSize({width:390,height:844})
  await page.screenshot({path:'/tmp/wicker-guide-library-mobile.png'})
  await page.getByRole('tab',{name:'Materials',exact:true}).click()
  await expect(page.getByRole('group',{name:'Material academic year'})).toBeVisible()
  await expect(page.getByRole('group',{name:'Material academic year'}).getByRole('button',{name:'2025-2026',exact:true})).toBeVisible()
  await page.getByRole('group',{name:'Material academic year'}).getByRole('button',{name:'2025-2026',exact:true}).click()
  await expect(page).toHaveURL(/year=2025-2026/)
  await page.getByRole('group',{name:'Material academic year'}).getByRole('button',{name:'All years',exact:true}).click()
  await expect(page).toHaveURL(/year=all/)
  await expect(page.getByRole('button',{name:'All years',exact:true}).first()).toHaveAttribute('aria-pressed','true')
  await page.screenshot({path:'/tmp/wicker-materials-mobile.png',animations:'disabled'})
  await page.setViewportSize({width:1440,height:1000})
  await page.screenshot({path:'/tmp/wicker-materials-desktop.png'})
  expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true)
})
