...
test('chapter workspace restores course papers, free answer-key grading, source inspection and tutor panel', async ({page}) => {
  ...
  await expect(page.getByRole('complementary',{name:'Chapter tutor',exact:true})).toBeVisible()
  await page.getByRole('complementary',{name:'Chapter tutor',exact:true}).getByRole('button',{name:'Close reference',exact:true}).click()
  await page.getByRole('tab',{name:/^Practice \(\d+\)$/}).click()
