...
import { formatBytes } from "@/lib/v2/account.mjs";
import { localIsoDate } from "@/lib/v2/home.mjs";
import { currentRequest, REQUEST_CHUNK_BYTES, REQUEST_STATUS_LABEL, requestPayload, stageState, validateRequestFiles } from "@/lib/v2/course-request.mjs";
...
export default function CourseRequestPage() {
  ...
  if (course === undefined) return <div className="mx-auto flex w-full max-w-[1180px] flex-col gap-4 p-8"><Skeleton className="h-12 w-96"/><Skeleton className="h-5 w-72"/><Skeleton className="h-80 w-full"/></div>;
  if (!course) return <div className="mx-auto w-full max-w-[1180px] p-8"><Empty><EmptyHeader><EmptyTitle>Course not found</EmptyTitle><EmptyDescription>{error ?? "This course is no longer in your academic record or timetable."}</EmptyDescription></EmptyHeader><Link className={buttonVariants({ variant: "secondary" })} href="/v2/planning?tab=courses">Review courses</Link></Empty></div>;
  if (editorial) return <div className="mx-auto w-full max-w-[1180px] p-8"><Empty><EmptyHeader><CheckIcon/><EmptyTitle>Material is available for {course.code || course.name}</EmptyTitle><EmptyDescription>This course now has maintained study content.</EmptyDescription></EmptyHeader><Link className={buttonVariants()} href={`/v2/courses/${encodeURIComponent(editorial.id)}`}>Open course</Link></Empty></div>;

  return <div className="mx-auto flex w-full max-w-[1180px] flex-col gap-8 p-8">
    <header className="flex flex-col gap-2 border-b pb-6"><Link className="text-muted-foreground w-fit text-sm" href="/v2/planning?tab=courses">← Courses</Link><h1 className="font-heading text-5xl leading-none tracking-tighter">{course.code ? `${course.code} · ` : ""}{course.name}</h1><p className="text-muted-foreground max-w-3xl">{course.inferredFromTimetable ? "Your live timetable identifies this as a current teaching course. " : "This course is in your academic record. "}Maintained study content is not available yet.</p></header>
