...
export default function CourseRequestPage() {
  ...
  return <div className="mx-auto flex w-full max-w-[1180px] flex-col gap-8 p-5 sm:p-8">
    <header className="flex flex-col gap-2 border-b pb-6"><Link className="text-muted-foreground w-fit text-sm" href="/app/planning?tab=courses">← Courses</Link><h1 className="font-heading text-5xl leading-none tracking-tighter">{course.code ? `${course.code} · ` : ""}{course.name}</h1><p className="text-muted-foreground max-w-3xl">{course.inferredFromTimetable ? "Your live timetable identifies this as a current teaching course. " : "This course is in your academic record. "}Maintained study content is not available yet.</p></header>
