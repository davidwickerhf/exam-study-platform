...
const defaults: RefreshSettings = { enabled: true, updatesMinutes: 30, materialsMinutes: 1440, studyStatus: "studying" };
...
export function CanvasRefreshSettings({ origin, saved, collectionEnabled, onSaved }: { origin: string; saved?: RefreshSettings; collectionEnabled: boolean; onSaved: () => void }) {
  ...
  return <form onSubmit={save} className="mt-5 border-t pt-5">
    <div className="flex items-start justify-between gap-4"><div><h3 className="text-sm font-semibold">Automatic refresh</h3><p className="text-muted-foreground mt-1 max-w-2xl text-sm">Follows your academic calendar as periods change. New active editions are collected once. After that, scheduled material refreshes collect new and changed files; lightweight checks run between them.</p></div><label className="flex shrink-0 items-center gap-2 text-sm font-medium"><input type="checkbox" className="size-4 accent-primary" checked={value.enabled} disabled={busy} onChange={event => setValue({ ...value, enabled: event.target.checked })} />Enabled</label></div>
    ...
    <fieldset disabled={busy} className="mt-4 grid gap-4 sm:grid-cols-2">
      <label className="text-sm font-medium">Canvas update checks<select className={control} value={value.updatesMinutes} disabled={!active} onChange={event => setValue({ ...value, updatesMinutes: Number(event.target.value) })}>{[15,30,60,180,360,1440].map(minutes => <option key={minutes} value={minutes}>{frequency(minutes)}</option>)}</select></label>
      <label className="text-sm font-medium">Material collection<select className={control} value={value.materialsMinutes} disabled={!active} onChange={event => setValue({ ...value, materialsMinutes: Number(event.target.value) })}>{[60,360,720,1440,10080].map(minutes => <option key={minutes} value={minutes}>{frequency(minutes)}</option>)}</select></label>
