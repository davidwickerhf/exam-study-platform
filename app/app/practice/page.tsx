          ...
          <section className="mx-auto flex w-full max-w-[980px] flex-col overflow-hidden rounded-sm border bg-card">
            <div className="flex flex-col gap-3 border-b bg-muted/35 px-5 py-4 sm:px-7">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <Button variant="ghost" size="sm" nativeButton={false} render={<Link href={`/app/courses/${encodeURIComponent(current.courseId)}/${encodeURIComponent(String(current.chapterId))}`} />}>
                  Open chapter
                </Button>
...
function MocksTab({
  ...
  const openReview = async (session: MockSession) => {
    ...
    try {
      ...
      history.replaceState(null, "", `/app/practice?tab=mocks&session=${encodeURIComponent(session.id)}`);
    } catch (cause) {
      setFailure((cause as Error).message);
    }
  };
    ...
    return (
      <div className="flex flex-col gap-5">
        <Button
          variant="ghost"
          className="self-start"
          onClick={() => { setSelected(null); history.replaceState(null, "", "/app/practice?tab=mocks"); }}
        >
          Back to mock log
        </Button>
        <SectionHead
