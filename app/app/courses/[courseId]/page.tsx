...
import { StudySourceInspector } from '@/components/workspace/study-source-inspector'
    ...
    <main ref={setCourseElement} className="flex w-full min-w-0 flex-col" data-course-detail>
      ...
      <div className="course-workspace">
        ...
        <Tabs value={tab} orientation={wide ? 'vertical' : 'horizontal'} onValueChange={value => selectTab(value as CourseTab)} className="course-layout">
          <aside className="course-navigation">
            ...
            {['study','history','groups'].includes(tab) && <div className="course-edition-control">
      ...
      {!!items.length && (
        <section className="course-section flex flex-col gap-3">
          ...
          <TabsContent value="papers" className="min-w-0"><StudyPaperBank active={tab === 'papers'} course={{courseCode: course.code || course.id, courseName: course.name, academicYear: year === 'all' ? 'undated' : year, period: String(academicCourse?.period || '')}} /></TabsContent>
      ...
      <section id="attendance" className="scroll-mt-8 course-section overflow-hidden">
        ...
        {calendarLoading ? <p role="status" className="text-muted-foreground px-5 py-6 text-sm">Reading attendance…</p> : calendarError ? <div role="alert" className="px-5 py-6"><p className="text-sm">{calendarError}</p><Button variant="outline" size="sm" className="mt-3" onClick={retry}>Try again</Button></div> : attendance ? <>
          ...
          {attendance.ruleExtractionStatus==='needs-review' && <div className="border-b px-5 py-4 text-sm"><strong>Some course requirements could not be checked</strong><p className="mt-1 text-muted-foreground">Automatic syllabus analysis did not complete. Exact rules recovered from your sources are shown below; other requirements remain unknown.</p><Link href="/app/settings/canvas-sync" className="mt-2 inline-block text-primary underline">Open Canvas sync</Link></div>}
          {!!attendance.unknownRequirementSessions && <p className="border-b px-5 py-4 text-sm text-muted-foreground">Attendance requirements are not confirmed for {attendance.unknownRequirementSessions} scheduled sessions. Check the current syllabus; an unknown requirement does not mean attendance is optional.</p>}
          {!!attendance.unmatchedRules?.length && <section className="border-b px-5 py-5"><h3 className="text-base font-semibold">Attendance and participation in the syllabus</h3><p className="mt-1 text-sm text-muted-foreground">These source rules have not been matched to a specific timetable session. A tutorial label alone does not establish whether it is a lab or debate.</p><ul className="mt-4 divide-y">{attendance.unmatchedRules.map((rule, i) => <li key={i} className="py-3 text-sm"><strong className="capitalize">{rule.activity}{rule.assessed ? ' · Assessed participation' : ''}</strong><p className="mt-1 leading-6">{rule.text}</p>{rule.evidence?.filter((ref,index,refs)=>ref.assetId&&refs.findIndex(r=>r.assetId===ref.assetId)===index).map(ref=><StudySourceInspector key={ref.assetId} source={{key:ref.assetId!,assetId:ref.assetId,title:ref.title || 'Course source.pdf',kind:'canvas',sha256:'',academicYear:attendance.ruleAcademicYear || '',url:`/api/corpus/assets/${ref.assetId}`}} chunks={[]} initialPage={ref.page || 1} label="View source"/>)}</li>)}</ul></section>}

          {(attendance.requiredScheduled > 0 || attendance.rule) && <div className="bg-accent/35 border-b px-5 py-4">
            <div className="flex items-start gap-3"><span className="bg-primary/10 text-primary grid size-8 shrink-0 place-items-center rounded-md"><CalendarCheckIcon className="size-4" /></span><div className="min-w-0 flex-1"><div className="flex flex-wrap items-baseline justify-between gap-2"><strong className="text-sm">Required attendance</strong><span className={`text-sm font-semibold ${NUMERALS}`}>{attendance.requiredRate == null ? 'No marked sessions' : `${attendance.requiredRate}%`}</span></div><p className="text-muted-foreground mt-1 text-xs">{attendance.allowedMisses == null ? `${attendance.requiredMissed} required ${attendance.requiredMissed === 1 ? 'session' : 'sessions'} missed` : `${attendance.requiredMissed} of ${attendance.allowedMisses} allowed misses used · ${attendance.allowedMissesRemaining} remaining`}</p>{attendance.rule && <details className="mt-3 border-t pt-3"><summary className="cursor-pointer text-xs font-semibold">View source rule</summary><p className="text-muted-foreground mt-2 text-xs leading-relaxed">{attendance.rule}</p><p className="text-primary mt-1 text-[11px] font-semibold">{attendance.ruleSource}</p></details>}</div></div>
          </div>}
          <div>
