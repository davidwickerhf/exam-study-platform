...
import { tutorFailure, type TutorFailure } from '@/lib/tutor-errors.mjs'
...
export type TutorContext = {
  chapterView?: string | null
  courseTab?: string | null; courseTabLabel?: string | null; academicYear?: string | null
  selection?: {kind: 'document' | 'question' | 'group'; groupId?: string; canvasUrl?: string; title: string; text?: string; sourceAssetId?: string; page?: number} | null
...
type Hub = {
  ...
  conversation: { id: string; title: string | null; messages: Message[]; reply?: {status: string; failure?: TutorFailure} | null } | null
}
...
function ActionDocket({ compact = false, proposals, drafts, receipts, conversationId, reviewIds, onComplete }: { compact?: boolean; proposals: Proposal[]; drafts: TutorPresentation['drafts']; reviewIds: string[]; receipts: Receipt[]; conversationId: string | null; onComplete: (result: { receipts: Receipt[]; memory: Hub['memory'] }) => void }) {
  ...
  const expandedContent=expanded && Boolean(pending.length || drafts.length || completed.length)
  return <aside data-compact={compact || undefined} data-expanded={expandedContent || undefined} id="tutor-actions" tabIndex={-1} aria-label="Proposed actions" className={`flex min-h-0 flex-col outline-none border-l max-lg:border-t max-lg:border-l-0 lg:w-[350px] lg:shrink-0 ${expanded && (pending.length || drafts.length || completed.length) ? 'max-lg:max-h-[65%] max-lg:shrink-0' : 'max-lg:h-14 max-lg:shrink-0'}`}>
...
export function TutorWorkspace({ initialContext = {}, embedded = false }: { initialContext?: TutorContext; embedded?: boolean }) {
  ...
  const [error, setError] = useState<string | null>(null)
  const [replyFailure, setReplyFailure] = useState<TutorFailure | null>(null)
  ...
  const load = async (id?: string) => {
    ...
    requestRef.current?.abort(); requestRef.current = null; setSending(false); setError(null); setReplyFailure(null)
    ...
    try {
      ...
      const reply = data.conversation?.reply
      setReplyFailure(reply && ['failed', 'stopped'].includes(reply.status) ? tutorFailure(reply, reply.status === 'stopped') : null)
  ...
  const ask = async (value: string, retrying = false) => {
    ...
    setDraft(''); setSending(true); setProgress('Checking your question…'); setPartialAnswer(''); setError(null); setReplyFailure(null)
    ...
    try {
      ...
      if (isCurrent()) {
        setReplyFailure(tutorFailure(cause, controller.signal.aborted))
        workspaceCache.invalidate(key => key.startsWith('/api/tutor'))
      }
  ...
  const retryReply = () => {
    ...
    void ask(messages.at(-1)!.content, true)
  }
  ...
  const composer = <div className="border-t bg-card px-4 py-4 sm:px-6"><div className="mx-auto max-w-[1240px] rounded-[10px] border bg-card"><div className="text-muted-foreground flex items-center gap-2 border-b px-4 py-2.5 text-xs"><Globe2Icon className="size-3.5 shrink-0" /><strong className="text-foreground">{initialContext.courseCode || 'Workspace-wide'}</strong><span>·</span><span className={embedded ? "min-w-0 truncate" : "max-sm:hidden"}>{initialContext.courseTabLabel || (initialContext.chapterName ? `starting from ${initialContext.chapterName}, with access to the full workspace` : 'checking every connected course and source')}</span></div>{embedded && initialContext.selection && <p className="border-b px-4 py-2 text-xs text-muted-foreground">{initialContext.selection.kind==='document' ? 'Source' : initialContext.selection.kind==='group' ? 'Group' : 'Question'}: {initialContext.selection.title}{initialContext.selection.page ? ` · Page ${initialContext.selection.page}` : ''}</p>}
    ...
    <form data-tour="tutor-composer" data-tour-ready={!loadingConversation} className="flex items-end gap-3 p-3" onSubmit={(event) => { event.preventDefault(); void ask(draft) }}><Button type="button" variant="ghost" size="icon" disabled={uploading} aria-label="Add a private file or picture" onClick={() => fileRef.current?.click()}>{uploading ? <Spinner /> : <PaperclipIcon />}</Button><Textarea value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void ask(draft) } }} rows={1} disabled={sending || loadingConversation} aria-label="Ask the tutor" placeholder={sending ? 'Checking your sources…' : embedded ? 'Ask about this course…' : 'Ask anything about your studies…'} className="max-h-36 min-h-9 resize-none border-0 bg-transparent px-1 py-2 shadow-none focus-visible:ring-0" /><Button type="submit" size="icon" aria-label="Send question" disabled={!draft.trim() || sending || loadingConversation}>{sending ? <Spinner /> : <SendIcon />}</Button></form></>}</div>{!tutorUnavailable && <p className="text-muted-foreground mx-auto mt-2 max-w-[1240px] text-center text-[11px] max-sm:hidden">Files are stored privately and indexed for future retrieval. Tutor proposes changes before it acts.</p>}</div>

  return <div className={`flex min-h-0 w-full flex-col overflow-hidden ${embedded ? 'tutor-embedded h-full bg-background' : 'h-[calc(100dvh-7.5rem)] md:h-dvh'}`}>
    {!embedded && <header data-tour="tutor" data-tour-ready={!loadingConversation} className="flex shrink-0 items-center gap-2 border-b px-5 py-4 sm:gap-3 sm:px-8"><div className="min-w-0 flex-1"><h1 className="font-heading text-[32px] leading-none font-semibold tracking-[-0.03em]">Tutor</h1><p className="text-muted-foreground mt-1.5 text-sm max-sm:hidden">Course explanations, study decisions and approved actions, grounded in your private workspace.</p></div><Link href="/app/tutor/work" className="text-primary shrink-0 text-xs font-semibold sm:text-sm">Study work</Link><Button variant="ghost" size="sm" onClick={() => { setView('chat'); void load(); setDraft(''); setActiveAttachments([]) }}><PlusIcon data-icon="inline-start" />New</Button></header>}
    <nav aria-label="Tutor views" className="flex shrink-0 gap-6 border-b px-5 sm:px-8">{([['chat', 'Conversation'], ['history', 'History'], ['sources', 'Sources']] as const).map(([id, label]) => <button key={id} aria-current={view === id ? 'page' : undefined} onClick={() => { setView(id); setLibraryQuery(''); if (!embedded) { const url = new URL(window.location.href); url.searchParams.set('view', id); window.history.replaceState(null, '', url) } }} className={`relative min-h-11 border-b-2 px-0.5 text-sm ${view === id ? 'border-foreground font-semibold text-foreground' : 'border-transparent text-muted-foreground hover:text-foreground'}`}>{label}{id === 'chat' && sending && <span className="ml-2 inline-block size-1.5 rounded-full bg-primary" />}</button>)}{embedded && <Button className="ml-auto" size="icon-sm" variant="ghost" aria-label="New tutor conversation" onClick={()=>{setView('chat');void load();setDraft('');setActiveAttachments([])}}><PlusIcon/></Button>}</nav>
