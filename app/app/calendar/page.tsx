...
import {
  ...
  CircleAlertIcon, Clock3Icon, ExternalLinkIcon, EyeIcon, EyeOffIcon,
  GripVerticalIcon, Link2Icon, MoreHorizontalIcon, PanelLeftCloseIcon,
...
type SourceRow = { id: string; label: string; count: number; detail?: string; feedId?: string; problem?: string }
...
function RailDivider({ side, open, onOpen, onResize, onNudge }: { side: 'left' | 'right'; open: boolean; onOpen: () => void; onResize: (event: ReactPointerEvent<HTMLDivElement>) => void; onNudge: (delta: number) => void }) {
  ...
  return <div role={open ? 'separator' : undefined} aria-orientation="vertical" aria-label={open ? `Resize ${side === 'left' ? 'calendar list' : 'day desk'}` : undefined} tabIndex={open ? 0 : -1} onPointerDown={open ? onResize : undefined} onKeyDown={open ? (event) => { if (event.key === 'ArrowLeft' || event.key === 'ArrowRight') { event.preventDefault(); onNudge(event.key === 'ArrowLeft' ? -16 : 16) } } : undefined} className={cn('group relative z-20 hidden h-full touch-none select-none bg-border before:absolute before:inset-y-0 before:-left-2 before:-right-2 focus-visible:outline-2 focus-visible:outline-primary xl:flex', side === 'right' && 'lg:flex', open ? 'cursor-col-resize' : 'cursor-default')}>{open ? <GripVerticalIcon className="text-muted-foreground bg-background pointer-events-none absolute top-1/2 left-1/2 size-4 -translate-1/2 opacity-0 transition-opacity group-hover:opacity-100 group-focus:opacity-100" /> : <button type="button" onClick={onOpen} className="bg-background text-muted-foreground hover:text-foreground absolute top-4 left-1/2 grid size-7 -translate-x-1/2 place-items-center rounded-md border" aria-label={`Open ${side === 'left' ? 'calendars' : 'day desk'}`}><OpenIcon className="size-4" /></button>}</div>
}
...
export default function CalendarPage() {
  ...
  useEffect(() => { const number = (key: string, fallback: number) => Math.round(Number(localStorage.getItem(key))) || fallback; setLeftWidth(Math.min(LEFT_MAX, Math.max(LEFT_MIN, number('wicker-calendar-left-width', 216)))); setRightWidth(Math.min(RIGHT_MAX, Math.max(RIGHT_MIN, number('wicker-calendar-right-width', 310)))); setLeftOpen(localStorage.getItem('wicker-calendar-left-open') !== 'false'); setRightOpen(localStorage.getItem('wicker-calendar-right-open') !== 'false') }, [])
  ...
  const sourceCounts = useMemo(() => { const tally: Record<string, number> = {}; for (const event of payload?.events ?? []) tally[sourceId(event)] = (tally[sourceId(event)] ?? 0) + 1; return tally }, [payload])
  ...
  const sourceRows = useMemo<SourceRow[]>(() => { if (!payload) return []; const problems = new Map((payload.problems ?? []).map((problem) => [problem.id, problem.error])); const rows: SourceRow[] = [{ id: 'personal:wicker', label: 'Wicker calendar', detail: 'Creates and edits events', count: sourceCounts['personal:wicker'] || 0 }, { id: 'plan', label: 'Academic plan', detail: 'Exams and planned dates', count: sourceCounts.plan || 0 }, { id: 'institution', label: 'University calendar', detail: 'Periods and academic dates', count: sourceCounts.institution || 0 }]; for (const feed of payload.feeds ?? []) rows.push({ id: `feed:${feed.id}`, feedId: feed.id, label: feed.label, detail: feed.lastSyncedAt ? `Updated ${new Intl.RelativeTimeFormat('en', { numeric: 'auto' }).format(Math.round((new Date(feed.lastSyncedAt).getTime() - Date.now()) / 86_400_000), 'day')}` : 'Subscribed calendar', count: sourceCounts[`feed:${feed.id}`] || feed.eventCount || 0, problem: problems.get(feed.id) }); if (payload.canvas?.connected) rows.push({ id: 'canvas', label: 'Canvas', detail: 'Assignments and course events', count: sourceCounts.canvas || 0, problem: [...problems.entries()].find(([id]) => id.startsWith('canvas:'))?.[1] }); return rows }, [payload, sourceCounts])
  ...
  async function deleteEvent(event: CalendarEvent) { if (!event.managedEventId || !window.confirm(`Delete “${event.title}”?`)) return; setComposerSaving(true); setComposerError(null); try { const response = await fetch(`/api/calendar/events/${encodeURIComponent(event.managedEventId)}`, { method: 'DELETE', headers: { accept: 'application/json', 'content-type': 'application/json' }, body: JSON.stringify({ expectedRevision: academicRevision }) }); const body = await response.json().catch(() => null); if (!response.ok) throw new Error(body?.error || `Calendar event returned ${response.status}`); setAcademicRevision(body.workspace.revision); setComposer(null); setSelected(null); await loadCalendar() } catch (cause) { setComposerError((cause as Error).message) } finally { setComposerSaving(false) } }
  ...
  const sourceList = (prefix: string) => payload ? <ul className="px-2 pb-2">{sourceRows.map((source) => <li key={source.id} className="group grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2 rounded-md px-2 py-2 hover:bg-muted/55"><button type="button" onClick={() => toggleSource(source.id)} className="text-primary grid size-6 place-items-center" aria-label={`${hiddenSources.has(source.id) ? 'Show' : 'Hide'} ${source.label}`}>{hiddenSources.has(source.id) ? <EyeOffIcon className="size-4 text-muted-foreground" /> : <EyeIcon className="size-4" />}</button><button type="button" onClick={() => toggleSource(source.id)} className="min-w-0 text-left"><strong className="block truncate text-xs">{source.label}</strong><small className={cn('text-muted-foreground mt-0.5 block truncate text-[10px]', source.problem && 'text-destructive')}>{source.problem || source.detail}</small></button><div className="flex items-center gap-1"><span className={`text-muted-foreground text-[11px] ${NUMERALS}`}>{source.count}</span>{source.feedId && <DropdownMenu><DropdownMenuTrigger render={<Button variant="ghost" size="icon-sm" aria-label={`Manage ${source.label}`} disabled={sourceSaving === source.feedId} />}><MoreHorizontalIcon /></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onClick={() => void manageFeed(source.feedId!, 'sync')}><RefreshCwIcon />Refresh now</DropdownMenuItem><DropdownMenuItem variant="destructive" onClick={() => void manageFeed(source.feedId!, 'remove')}><Trash2Icon />Remove calendar</DropdownMenuItem></DropdownMenuContent></DropdownMenu>}</div><Checkbox id={`${prefix}-${source.id}`} checked={!hiddenSources.has(source.id)} onCheckedChange={() => toggleSource(source.id)} className="sr-only" /></li>)}</ul> : <Skeleton className="mx-4 h-40" />
  ...
  return <div className="flex h-[calc(100dvh-4rem)] min-h-0 flex-col md:h-dvh">
    ...
    <details className="bg-card shrink-0 border-b lg:hidden"><summary className="cursor-pointer px-4 py-3 text-sm font-semibold">Calendars and sources</summary>{sourceList('mobile')}<div className="border-t p-3"><Button variant="outline" className="w-full" onClick={() => setConnectOpen(true)}><Link2Icon data-icon="inline-start" />Connect calendar</Button></div></details>
    ...
    <EventComposer seed={composer} courses={courses} saving={composerSaving} error={composerError} onOpenChange={(open) => { if (!open) { setComposer(null); setComposerError(null) } }} onSave={(input) => void saveEvent(input)} onDelete={(event) => void deleteEvent(event)} />
  </div>
}
