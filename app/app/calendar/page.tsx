  ...
  return (
    <div className="flex h-[calc(100dvh-8rem)] flex-col gap-4 p-3 sm:p-6 md:h-dvh">
      ...
      <details className="border-y py-2 lg:hidden">
        ...
        <ul className="mt-3 grid grid-cols-2 gap-2 pb-1">
          {payload && Object.entries(payload.categories).filter(([id]) => counts[id]).map(([id, label]) => (
            <li key={id} className="flex min-w-0 items-center gap-2">
              <Checkbox id={`mobile-cat-${id}`} checked={!hidden.has(id)} onCheckedChange={(checked) => setHidden((previous) => {
                const next = new Set(previous)
                if (checked) next.delete(id)
                else next.add(id)
                return next
              })} />
              <label htmlFor={`mobile-cat-${id}`} className="min-w-0 truncate text-xs">{label}</label>
            </li>
          ))}
        </ul>
      </details>
