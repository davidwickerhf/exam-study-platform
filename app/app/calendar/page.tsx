...
import { calendarLocalDay, calendarFilterMatches } from '@/lib/calendar-display.mjs'
  ...
  return (
    <section className="border-b px-5 py-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <span className={cn('rounded-md px-2 py-1 text-[10px] font-semibold tracking-[0.07em] uppercase', event.attendanceRequired ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground')}>{calendarEventEmphasis(event).label}</span>
      </div>
      <div className="mt-3 grid grid-cols-3 overflow-hidden rounded-md border">
        {(['attended', 'missed', 'excused'] as const).map((status) => <button key={status} type="button" disabled={saving || future} aria-pressed={event.attendanceStatus === status} onClick={() => onMark(status)} className={cn('flex h-9 items-center justify-center gap-1 border-r text-xs font-semibold last:border-r-0 disabled:cursor-not-allowed disabled:opacity-45', event.attendanceStatus === status ? 'bg-foreground text-card' : 'bg-card text-muted-foreground hover:bg-muted')}>{status === 'attended' && <CheckIcon className="size-3.5" />}{attendanceLabel(status)}</button>)}
      </div>
  ...
  return (
    <aside className="bg-card h-full min-h-0 overflow-y-auto" aria-label="Day desk">
      <header className="flex min-h-[88px] items-start justify-between gap-4 border-b px-5 py-5">
        <div className="min-w-0"><p className={LABEL}>{selected ? 'Selected event' : 'Day desk'}</p><h2 className="font-heading mt-1.5 text-2xl leading-[1.05] font-semibold tracking-[-0.025em]">{selected ? selected.category === 'timetable' ? selected.courseName || selected.title : selected.title : fullDay(selectedDate)}</h2></div>
      ...
      {selected ? <>
        <section className="border-b px-5 py-5">
          <div className="grid gap-4">
            <div className="grid grid-cols-[1.1rem_minmax(0,1fr)] gap-2.5"><Clock3Icon className="text-muted-foreground size-4" /><span><strong className={`block text-sm ${NUMERALS}`}>{eventTime(selected)}</strong><small className="text-muted-foreground mt-0.5 block text-xs">{fullDay(calendarLocalDay(selected))}</small></span></div>
            ...
            <div className="grid grid-cols-[1.1rem_minmax(0,1fr)] gap-2.5"><CalendarDaysIcon className="text-muted-foreground size-4" /><span><strong className="block text-sm">{selected.feedLabel || (selected.source === 'canvas' ? 'Canvas' : selected.editable ? 'Wicker calendar' : 'Academic plan')}</strong><small className="text-muted-foreground mt-0.5 block text-xs">{selected.editable ? 'You can edit this event' : 'Read-only source'}</small></span></div>
          </div>
...
export default function CalendarPage() {
  ...
  const toggleEventFilter = (filter: string) => setEventFilters(current=>current.includes(filter) ? current.filter(value=>value!==filter) : [...current,filter])
  ...
  const attendanceRequest = useRef(0); const attendanceInFlight = useRef(false)
  ...
  const events = useMemo(() => (payload?.events ?? []).filter((event) => !hiddenSources.has(sourceId(event)) && calendarFilterMatches(event, eventFilters)), [payload, hiddenSources, eventFilters])
  const dayEvents = useMemo(() => events.filter((event) => calendarLocalDay(event) === selectedDate).sort((left, right) => left.start.localeCompare(right.start)), [events, selectedDate])
  ...
  async function markAttendance(status: AttendanceStatus) {
    if (!selected || academicRevision === null || attendanceInFlight.current) return
    const target = selected, previous = target.attendanceStatus || 'unknown', request = ++attendanceRequest.current
    attendanceInFlight.current = true
    setAttendanceSaving(true); setAttendanceError(null)
    setPayload(held => patchCalendarAttendance(held, target.id, status))
    setSelected(held => held?.id === target.id ? { ...held, attendanceStatus: status } : held)
    try {
      const response = await fetch('/api/attendance', { method: 'PUT', headers: { accept: 'application/json', 'content-type': 'application/json' }, signal: AbortSignal.timeout(20_000), body: JSON.stringify({ event: target, status, expectedRevision: academicRevision }) })
      ...
      workspaceCache.invalidate('/api/academics')
      // Reconcile in the background; this is not part of the save interaction.
      // A late response cannot overwrite a newer mark or restore old selection.
      void cachedWorkspaceJson<CalendarData>('/api/calendar/events', true).then(data => {
        if (attendanceRequest.current !== request) return
        setPayload(data)
        setSelected(held => held ? data.events.find(event => event.id === held.id) || held : held)
      }).catch(() => {})
    } catch (cause) {
      setPayload(held => patchCalendarAttendance(held, target.id, previous))
      setSelected(held => held?.id === target.id ? { ...held, attendanceStatus: previous } : held)
      setAttendanceError(`Could not save attendance for ${target.courseName || target.title}. Your previous mark was restored. ${(cause as Error).message}`)
      // A conflict in another tab must not leave retries using the stale revision.
      void cachedWorkspaceJson<AcademicsData>('/api/academics', true).then(data => setAcademicRevision(data.workspace?.revision ?? null)).catch(() => {})
    } finally { attendanceInFlight.current = false; setAttendanceSaving(false) }
  }
  ...
  if (error && !payload) return <div className="mx-auto w-full max-w-[1400px] p-5 sm:p-8"><Empty><EmptyHeader><EmptyTitle>Your calendar could not be read</EmptyTitle><EmptyDescription>{error}</EmptyDescription></EmptyHeader></Empty></div>
  ...
  return <div className="flex h-[calc(100dvh-4rem)] min-h-0 flex-col md:h-dvh">
    {error && <p role="alert" className="text-destructive shrink-0 border-b px-4 py-3 text-sm sm:px-6">{error}</p>}
    {payload?.problems?.map((problem) => <p key={problem.id} role="status" className="shrink-0 border-b px-4 py-3 text-sm sm:px-6"><strong>{problem.label}: </strong>{problem.error}{problem.usingSaved && <> Showing the saved timetable{problem.savedAt ? ` from ${new Date(problem.savedAt).toLocaleString()}` : ''}; recent changes may be missing.</>}</p>)}
    <header data-tour="calendar-controls" data-tour-ready={Boolean((payload && ready) || error)} className="bg-background flex min-h-[72px] shrink-0 flex-wrap items-center justify-between gap-3 border-b px-4 py-3 sm:px-6"><div className="flex min-w-0 flex-wrap items-center gap-2 sm:gap-3"><Button onClick={() => { setComposerError(null); setComposer({ mode: 'create', date: selectedDate }) }}><PlusIcon data-icon="inline-start" />Create</Button><Button variant="outline" onClick={() => moveCalendar('today')} disabled={!ready}>Today</Button><div className="flex"><Button variant="ghost" size="icon" onClick={() => moveCalendar('prev')} disabled={!ready} aria-label="Previous"><ChevronLeftIcon /></Button><Button variant="ghost" size="icon" onClick={() => moveCalendar('next')} disabled={!ready} aria-label="Next"><ChevronRightIcon /></Button></div><h1 className={`font-heading ml-1 text-xl font-semibold tracking-[-0.025em] ${NUMERALS}`}>{title || 'Calendar'}</h1></div><div className="bg-card flex overflow-x-auto rounded-lg border p-1" role="tablist" aria-label="Calendar view">{VIEWS.map((entry) => <button key={entry.id} type="button" role="tab" aria-selected={view === entry.id} disabled={!ready} onClick={() => changeView(entry.id)} className={cn('h-8 min-w-16 rounded-md px-3 text-xs font-semibold disabled:opacity-50', view === entry.id ? 'bg-foreground text-card' : 'text-muted-foreground hover:text-foreground')}>{entry.label}</button>)}</div></header>
    <div className="flex shrink-0 flex-wrap items-center gap-2 border-b px-4 py-2 text-xs sm:px-6" role="group" aria-label="Filter calendar events">
      <Button size="sm" variant={!eventFilters.length ? 'secondary' : 'ghost'} aria-pressed={!eventFilters.length} onClick={()=>setEventFilters([])}>All events</Button>
      {[
        {id:'required',label:'Mandatory attendance',color:'border-primary text-primary'},
        {id:'attended',label:'Attended',color:'border-[#24553c] text-[#24553c] bg-[#e0f2e8]'},
        {id:'missed',label:'Missed',color:'border-[#822d42] text-[#822d42] bg-[#fbe5e8]'},
        {id:'excused',label:'Excused',color:'border-[#75521b] text-[#75521b] bg-[#fff0d9]'},
        {id:'unmarked',label:'Unmarked',color:'border-muted-foreground text-muted-foreground'},
        {id:'deadlines',label:'Exams & deadlines',color:'border-destructive text-destructive'},
      ].map(filter=><button type="button" key={filter.id} aria-pressed={eventFilters.includes(filter.id)} onClick={()=>toggleEventFilter(filter.id)} className={cn('inline-flex min-h-8 items-center gap-1.5 rounded-md border px-2.5 py-1 font-semibold transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary',filter.color,eventFilters.includes(filter.id) ? 'ring-2 ring-primary ring-offset-1' : 'border-transparent hover:border-current')}>
        {eventFilters.includes(filter.id) && <CheckIcon className="size-3.5"/>}{filter.label}
      </button>)}
      <Link href="/app/priorities" className="ml-auto font-semibold text-primary">All priorities →</Link>
    </div>
    ...
    <div style={splitStyle} className="grid min-h-0 flex-1 lg:grid-cols-[minmax(0,1fr)_1px_var(--right-width)] xl:grid-cols-[var(--left-width)_1px_minmax(0,1fr)_1px_var(--right-width)]">
      ...
      <main className="bg-card min-h-0 min-w-0 overflow-hidden" data-fc><CalendarGrid apiRef={apiRef} view={view} events={events} onSelectEvent={(event) => { setRightOpen(true); setAttendanceError(null); setSelected(event); setSelectedDate(calendarLocalDay(event)) }} onSelectDate={(day) => { setRightOpen(true); setSelected(null); setSelectedDate(day) }} onTitle={setTitle} onReady={() => setReady(true)} /></main>
