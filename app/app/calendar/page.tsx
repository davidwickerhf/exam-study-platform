...
export default function CalendarPage() {
  ...
  const [obligationsOnly, setObligationsOnly] = useState(false)
  ...
  return <div className="flex h-[calc(100dvh-4rem)] min-h-0 flex-col md:h-dvh">
    ...
    <div className="flex shrink-0 flex-wrap items-center gap-x-5 gap-y-2 border-b px-4 py-2 text-xs sm:px-6" aria-label="Calendar obligations">
      <span className="border-l-4 border-primary pl-2 font-semibold text-primary">Mandatory attendance</span><span className="rounded bg-[#e0f2e8] px-2 py-1 text-[#24553c]">Attended</span><span className="rounded bg-[#fbe5e8] px-2 py-1 text-[#822d42]">Missed</span><span className="rounded bg-[#fff0d9] px-2 py-1 text-[#75521b]">Excused</span><span className="border-l-4 border-destructive pl-2 font-semibold text-destructive">Exams &amp; deadlines</span>
      <Button size="sm" variant={obligationsOnly ? 'secondary' : 'ghost'} aria-pressed={obligationsOnly} onClick={()=>setObligationsOnly(value=>!value)}>Obligations only</Button><Link href="/app/priorities" className="ml-auto font-semibold text-primary">All priorities →</Link>
    </div>
    ...
    <div style={splitStyle} className="grid min-h-0 flex-1 lg:grid-cols-[minmax(0,1fr)_1px_var(--right-width)] xl:grid-cols-[var(--left-width)_1px_minmax(0,1fr)_1px_var(--right-width)]">
      ...
      <main className="bg-card min-h-0 min-w-0 overflow-hidden" data-fc><CalendarGrid apiRef={apiRef} view={view} events={obligationsOnly ? events.filter(event=>['required','deadline'].includes(calendarEventEmphasis(event).tone)) : events} onSelectEvent={(event) => { setRightOpen(true); setAttendanceError(null); setSelected(event); setSelectedDate(event.start.slice(0, 10)) }} onSelectDate={(day) => { setRightOpen(true); setSelected(null); setSelectedDate(day) }} onTitle={setTitle} onReady={() => setReady(true)} /></main>
