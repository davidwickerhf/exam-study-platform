...
import { calendarLocalDay, isCalendarDeadline, calendarDeadlineTime } from '@/lib/calendar-display.mjs'
      ...
      events.map((event) => ({
        ...
        title:
          event.courseCode && event.category === 'timetable'
            ...
            : event.title,
        start: isCalendarDeadline(event) ? calendarLocalDay(event) : event.start,
        end: isCalendarDeadline(event) ? undefined : event.end ?? undefined,
        allDay: isCalendarDeadline(event) || event.allDay,
        extendedProps: event
      })),
    [events]
  )
    ...
    <FullCalendar
      ...
      eventContent={info => {
        ...
        return <div className="h-full min-w-0 overflow-hidden" data-calendar-event-id={event.id}>
          {isCalendarDeadline(event) && calendarDeadlineTime(event) && <span className="fc-event-time block text-[10px]">Due {calendarDeadlineTime(event)}</span>}
