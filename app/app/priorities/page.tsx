...
export default function PrioritiesPage() {
  ...
  return <main className="mx-auto w-full max-w-[1180px] px-4 py-6 sm:px-8 sm:py-8">
    ...
    <section aria-label="Priority filters" className="my-6 grid gap-3 sm:grid-cols-[minmax(0,1fr)_12rem_12rem]">
      ...
      <div className="space-y-2"><Label id="priority-course-label">Course</Label><Select value={course} onValueChange={v=>v&&setCourse(v)}><SelectTrigger aria-labelledby="priority-course-label" className="w-full"><SelectValue>{course==='all' ? 'All courses' : course}</SelectValue></SelectTrigger><SelectContent><SelectItem value="all">All courses</SelectItem>{codes.map(code=><SelectItem key={code} value={code}>{code}</SelectItem>)}</SelectContent></Select></div>
      <div className="space-y-2"><Label id="priority-kind-label">Type</Label><Select value={kind} onValueChange={v=>v&&setKind(v)}><SelectTrigger aria-labelledby="priority-kind-label" className="w-full"><SelectValue>{{all:'All priorities',attendance:'Attendance',assignment:'Assignments',exam:'Exams',project:'Milestones'}[kind] || kind}</SelectValue></SelectTrigger><SelectContent>{[['all','All priorities'],['attendance','Attendance'],['assignment','Assignments'],['exam','Exams'],['project','Milestones']].map(([id,title])=><SelectItem key={id} value={id}>{title}</SelectItem>)}</SelectContent></Select></div>
    </section>
