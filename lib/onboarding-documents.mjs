import { removeOriginal } from './academic-originals.mjs'
import { discardDocumentReviews } from './academic-document-review.mjs'
import { repairTranscriptDuplicates } from './academic-record-repair.mjs'
...
export async function removeOnboardingDocument(kind) {
  ...
  for (const entry of [...history].reverse()) next = undoDocumentImport(next, repairTranscriptDuplicates(entry.before), repairTranscriptDuplicates(entry.after))
  ...
  if (otherHistory.length) await writeDocument(NAMESPACE, otherKey, otherHistory.map((entry) => ({
    before: undoDocumentImport(repairTranscriptDuplicates(entry.before), next, workspace),
    after: undoDocumentImport(repairTranscriptDuplicates(entry.after), next, workspace)
  })))
  await removeOriginal(kind)
