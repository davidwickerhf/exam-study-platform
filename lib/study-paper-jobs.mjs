...
import { canonicalPaperIdentity, deduplicatePaperSources, paperKind, preferredPaperJob, sameCourseEdition } from './study-paper-identity.mjs'
...
export function paperJobRecord(owner, programmeId, course, source) {
  if (!['paper', 'exercises'].includes(source.paperKind || paperKind(source))) return null
  return { id: `pap-${uuid([owner, programmeId, course.courseCode, course.academicYear, course.period, source.sha256 || source.key])}`, programmeId,
...
export async function enqueuePaper(course, source, programmeId = null) {
  const programme = programmeId || await activeProgrammeId()
  const record = paperJobRecord(currentUserId(), programme, course, source)
  if (!record) return null
  const existing = (await listDocuments(PAPER_JOBS))
    .map(row => row.value)
    .filter(job => job.programmeId === programme && sameCourseEdition(job.course, course) && canonicalPaperIdentity({key:job.sourceKey,sha256:job.sha256}) === canonicalPaperIdentity(source))
  if (existing.length) return preferredPaperJob(existing)
...
export async function queueCoursePapers(course, { sourceOptions = {} } = {}) {
  ...
  const sources = deduplicatePaperSources(await listStudySources(course, sourceOptions)), programme = await activeProgrammeId()
  const jobs = []
  for (const source of sources) {
    // The source carries the authoritative Canvas edition. A course page can
    // have no period metadata yet; using that empty shell would create a
    // second job for the same document.
    const sourceCourse={...course,academicYear:course.academicYear || source.academicYear,period:course.period || source.period}
    const job = await enqueuePaper(sourceCourse, source, programme)
...
export async function retryPaperJob(id) {
  ...
  // A worker records the paused state before its final lease cleanup. Explicit
  // retry must invalidate that lease so the old worker cannot later overwrite
  // the resumed job with stale state.
  const next = { ...old, revision: randomUUID(), status: 'queued', error: null, billing: null, retry: true, runAfter: 0, queueDeliveryUntil: 0, lease: null }
...
export async function processPaperJob(id, { sourceOptions = {}, platform = {}, generate } = {}) {
  if(!await recurringEnabled('papers'))return {again:false}
  ...
  try {
    ...
    const currentSource=available.find(s=>s.key===job.sourceKey && s.sha256===job.sha256) || available.find(s=>s.sha256===job.sha256)
    if(!currentSource) throw new StudyVersionError('The original changed or access was removed. Sync again to process the current paper.',409)
    job.sourceKey=currentSource.key
    job.title=currentSource.title
    if (!job.snapshot) {
    ...
    if(job.completedSections===job.sections.length) {
      ...
      if(!current.some(s=>s.sha256===job.sha256)) throw new StudyVersionError('The original changed during processing. Sync again.',409)
...
export async function resolvePaperJob(id) {
  const rows=sql ? await sql`SELECT user_id AS owner,document_key AS key,value FROM user_documents jobs WHERE namespace=${PAPER_JOBS}
    AND NOT EXISTS(SELECT 1 FROM user_documents policy WHERE policy.user_id=jobs.user_id AND policy.namespace='study-recurring-pipelines' AND policy.document_key='settings' AND policy.value->'settings'->>'papers'='false') AND document_key=${id} LIMIT 1`
...
export async function claimPaperDispatch() {
  const now=Date.now()
  const rows=sql ? await sql`SELECT user_id AS owner,document_key AS key,value FROM user_documents jobs WHERE namespace=${PAPER_JOBS}
    AND NOT EXISTS(SELECT 1 FROM user_documents policy WHERE policy.user_id=jobs.user_id AND policy.namespace='study-recurring-pipelines' AND policy.document_key='settings' AND policy.value->'settings'->>'papers'='false')
  ...
  for(const row of rows) {
    ...
    try { await asStudyOwner(row.owner,async()=>{
      if(!await recurringEnabled('papers'))return
