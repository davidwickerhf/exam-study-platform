...
export function tutorResearchRequirements(message, history = []) {
  ...
  if (/\b(class|session|meeting|representative|reps|teammates)\b/i.test(subject) && /\b(prepar\w*|bring\w*|advance|need to|what.*doing|check again)\b/i.test(text)) {
...
export function createTutorGrounding({ message, history = [] } = {}) {
  ...
  const dateConfirmation = String(message || '').length < 180 && /\b(so|confirm|right|correct|is it|its|it's)\b/i.test(String(message || '')) && /\b(tomorrow|tonight|night|today|due|deadline|date|time)\b/i.test(String(message || '')) && history.filter(item=>item.role==='user').slice(-3).some(item=>/\b(due|deadline|when)\b/i.test(item.content || ''))
  const focused = (preparation || dateConfirmation) && !/\b(make|create|save|track|give me)\b.{0,40}\b(plan|tasks|checklist)\b/i.test(String(message || ''))
  const learningQuestion = /ask me\b[^.!?]{0,90}\bquestion/i.test(String(message || ''))
  const attempted = new Set()
  const evidence = new Map()
  let linkedAssignment = false
  let hasAssignments = false
  const requireTool = (name, reason) => {
    if (!requirements.some(rule => rule.anyOf.length === 1 && rule.anyOf[0] === name)) requirements.push({anyOf:[name],reason})
  }
  return {
    requiredTools() {
    ...
    record(name, result, sources = []) {
      ...
      if (name === 'get_canvas_assignments' && result?.assignments?.length) hasAssignments = true
      ...
      if (linkedAssignment && hasAssignments) {
    ...
    responseFormat(base) {
      ...
      if (focused) {
        ...
        format.json_schema.schema.properties.summary.description = dateConfirmation ? 'Confirm the date/time directly with no plan or task widgets. Anchor relative dates to the current clock and verify current source details as needed.' : 'Answer this preparation question directly and briefly. State only preparation established by the actual instructions; if unspecified, say "I found no stated preparation requirement". Missing instructions do not establish that nothing is required: never say "the brief states there is no deliverable" unless it explicitly grants that exemption. Do not infer required preparation from a deadline or invent a checklist.'
      }
      if(learningQuestion) format.json_schema.schema.properties.summary.description = (format.json_schema.schema.properties.summary.description || '') + ' Include the requested learning question in this visible summary, ending with a question mark. Ask for a prediction or explanation; do not merely offer more help.'
      return format
    },
    review(content) {
      ...
      if (focused && Object.entries(answer).some(([key,value])=>key!=='evidenceIds' && Array.isArray(value) && value.length)) return 'Answer this narrow question directly. Keep all widget arrays empty; do not add unrequested tasks or plans.'
      if(learningQuestion && (!String(answer.summary || '').includes('?') || /do you want|would you like/i.test(answer.summary))) return 'Include a learner prediction question about a changed case in summary, with a question mark. Do not hide the question in detail or substitute an offer of more help.'
      return null
    }
  }
}
...
Separate documented requirements, the student’s reports, and optional advice. Do not invent a draft, template, completed review, one-page status, customer/supervisor meeting or debrief duration as required preparation. If instructions do not establish preparation, say what you checked and what remains unspecified. Absence of a stated requirement is not an explicit exemption: never turn "does not state a pre-class deliverable" into "explicitly says none is required". Say "I found no stated pre-class deliverable in the brief" instead. You may offer one brief suggestion clearly labelled optional. Do not turn it into a tracked task unless the student asks for a plan or to save it.
