...
export async function compareAndSwapDocument(namespace, key, value, expectedRevision) {
  const userId = currentUserId()
  if (sql) {
  ...
  const pending = previous.catch(() => {}).then(async () => {
    ...
    if ((current?.revision ?? null) !== expectedRevision) throw new DocumentConflictError()
    await mkdir(dirname(path), { recursive: true })
