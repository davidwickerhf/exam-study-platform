import { sanitizeDocumentHtml } from "./document-html.mjs";
...
export async function previewCourseBytes(bytes, filename, member = "") {
  ...
  try {
    ...
    if (preview.kind === "html") preview.text = sanitizeDocumentHtml(preview.text);
    return preview;
  } finally {
    await rm(directory, { recursive: true, force: true });
  }
}
export async function readCourseAssetBytes(asset) {
...
export async function previewCourseAsset(asset, member = "") {
  const bytes = await readCourseAssetBytes(asset);
