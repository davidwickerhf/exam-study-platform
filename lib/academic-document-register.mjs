// Programme-scoped derived evidence/version metadata. Original files, when
// retained by setup, live separately in academic-originals and never in prompts.
...
export async function deleteAcademicDocumentVersion({ kind: rawKind, versionId } = {}) {
  ...
  if (['transcript','academic-overview'].includes(kind)) await (await import('./academic-originals.mjs')).removeOriginal(kind === 'transcript' ? 'transcript' : 'record', `${programmeId}:${kind === 'transcript' ? 'transcript' : 'record'}:${id}`)
...
export async function deleteAcademicDocumentRecord({ kind: rawKind } = {}) {
  const programmeId = await activeProgrammeId()
  const kind = normalizedKind(rawKind)
  if (['transcript','academic-overview'].includes(kind)) await (await import('./academic-originals.mjs')).removeOriginal(kind === 'transcript' ? 'transcript' : 'record')
