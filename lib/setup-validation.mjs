...
export function inferEntryCurriculum(courses = [], programmeVersions = []) {
  const available = new Set(programmeVersions.map((version) => String(version.id || '').replace(/[–—/]/g, '-')))
  const yearOneCodes = new Set(programmeVersions.flatMap((version) => (version.courses || [])
    .filter((course) => course.yearLevel === 'Year 1')
    .map((course) => code(course.code))))
  const candidates = []
  for (const course of courses || []) {
    for (const attempt of course.attempts || []) {
      const attemptCode = code(attempt.courseCode || course.code)
      const level = String(attempt.yearLevel || course.yearLevel || '')
      if (level !== 'Year 1' && !yearOneCodes.has(attemptCode)) continue
      const academicYear = String(attempt.curriculumVersion || attempt.academicYear || '').replace(/[–—/]/g, '-')
      if (/^20\d{2}-20\d{2}$/.test(academicYear) && available.has(academicYear)) candidates.push(academicYear)
    }
  }
