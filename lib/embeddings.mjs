...
export async function embedTexts(values = []) {
  ...
  return trackAiCall({provider:'openai',model:MODEL,feature:'search-indexing',operation:'embeddings',firstParty:new URL(BASE_URL).hostname==='api.openai.com'},async observe=>{
  ...
  if (!response.ok) {
    ...
    throw Object.assign(new Error(messages[failure.code]), {
      providerRequestId:response.headers.get('x-request-id'), code: failure.code, status: response.status, retryable: failure.retryable,
      blockedReason: failure.retryable ? null : failure.code
    })
  }
  const body = await response.json()
  observe({...body,providerRequestId:response.headers.get('x-request-id')})
