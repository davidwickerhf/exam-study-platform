...
export const STUDY_STANDARD = 'student-source-teaching-v5'
...
export const lessonSchema = z.object({
  ...
  learningGoals: z.array(z.string().min(1).max(180)).max(8).default([]),
  title: z.string().min(1).max(180),
  sections: z
    .array(
...
export const teachingSchema = lessonSchema.omit({ teachingPlan: true }).extend({
  ...
  questions: z.array(lessonSchema.shape.questions.element.extend(diagnosticQuestionFields)).min(8).max(24),
...
export function teachingResponseSchema(plan, evidenceIds) {
  ...
  if(!simpleIds.length)result.properties.questions.items=diagnostic
  else {
    const simple=structuredClone(question)
    simple.properties.objectiveIds.items.enum=simpleIds
    result.properties.questions.items={anyOf:[simple,diagnostic]}
  }
  return result
}
...
export function pedagogicalResponseSchema(chapter) {
  ...
  const choices = chapter.sections.map(section => {
    ...
    return {type:'object',additionalProperties:false,required:['sectionId','quote'],properties:{
      sectionId:{type:'string',enum:[section.id]}, quote:{type:'string',enum:[...new Set(quotes)]}
    }}
...
export const inputHash = (topic, chunks) =>
  digest([
    STUDY_STANDARD,
    topic.title,
    chunks.filter(c=>topic.sourceIds.includes(c.id) || c.scopeContext).sort((a,b)=>a.id.localeCompare(b.id))
  ])
