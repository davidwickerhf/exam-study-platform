...
export const STUDY_STANDARD = 'student-source-teaching-v2'
...
Use the supplied structured schema. All factual items and visuals carry evidence through sourceIds; never print internal citation IDs in prose. The walkthrough is optional; do not duplicate the lesson in it. Caveats contain only genuine source/coverage limits, not boilerplate. Return JSON only.`
}
export function reviewPrompt(course, sources, chunks, chapter) {
  return `${evidencePrompt(course, sources, chunks)}\nIndependently check this generated chapter against evidence. When evidence marks a visual as not analyzed, reject interpretations or numerical readings of that visual unless the supplied text independently establishes them. Check unsupported claims, incorrect answers/calculations, contradictions, notes presented as official, historical rules presented as current, and substantial copied passages. Check every diagram arrow, set membership, comparison and plotted value; illustrative inputs must be explicitly labelled and derived consistently. Check that the summary actually explains the core relationships and that practice tests different skills, including application and diagnosis rather than repeated recall. Do not flag clearly labelled illustrative examples merely because their inputs are invented. Treat rules from different explicit years as historical changes, not same-edition conflicts. Citation presence is not proof of support. Flag substantive problems as error, minor caveats as warning. Shape: {"issues":[{"topicId":"${chapter.id}","detail":"","severity":"error|warning"}]}. Chapter: ${JSON.stringify(chapter)}`
}
