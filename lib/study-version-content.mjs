...
// plan's gaps, or its exclusions notes) that was never part of this
// chapter's supplied evidence set — usually carried over from a
// neighbouring chapter, a stale correction, or an invented citation.
// assertEvidence above only checks structured sourceIds arrays, so a prose
// citation like this previously reached review untouched, as a genuine and
// expensive factual-review finding. Strip only the unsupported identifier
// token itself; the surrounding prose, and every other (supported) citation,
// is left exactly as generated. Every removal is recorded on
// chapter.evidenceIdRepairs for visibility, mirroring the linkRepairs
// pattern in study-pedagogy.mjs. If removing every unsupported identifier
// from one entry would leave it empty, that entry is left untouched so
// review still flags the real problem instead of receiving an invalid,
// empty required field.
const EVIDENCE_ID_PATTERN = /\be-[0-9a-f]{6,}\b/g
function stripEntryEvidenceIds(entry, allowed, onRemove) {
  if (typeof entry !== 'string' || !entry) return entry
  const unsupported = [...new Set((entry.match(EVIDENCE_ID_PATTERN) || []).filter(id => !allowed.has(id)))]
  if (!unsupported.length) return entry
  if (!unsupported.reduce((text, id) => text.split(id).join(''), entry).trim()) return entry
  let next = entry
  for (const id of unsupported) {
    if (!next.includes(id)) continue
    next = next.split(id).join('')
    onRemove(id)
  }
...
function stripListEvidenceIds(list, allowed, field, repairs) {
  if (!Array.isArray(list)) return list
  let changed = false
  const next = list.map((entry, index) => {
    const cleaned = stripEntryEvidenceIds(entry, allowed, (ref) => repairs.push({ field, index, ref }))
    if (cleaned !== entry) changed = true
    return cleaned
  })
  return changed ? next : list
}
export function stripUnsupportedEvidenceIds(chapter, evidence) {
  const allowed = new Set(evidence.map((c) => c.id))
  const repairs = []
  const caveats = stripListEvidenceIds(chapter.caveats, allowed, 'caveats', repairs)
  const plan = chapter.teachingPlan
  const gaps = plan ? stripListEvidenceIds(plan.gaps, allowed, 'teachingPlan.gaps', repairs) : undefined
  const exclusions = plan ? stripListEvidenceIds(plan.exclusions, allowed, 'teachingPlan.exclusions', repairs) : undefined
