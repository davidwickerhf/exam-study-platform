...
// The acceptance shape for a MERGED chapter (a bounded patch or structural
// fill applied to saved content). Coverage rows keep their shape but may be
// empty: whether an objective is explained and assessed is the chapter
// contract's decision (contractIssues), which reports it as a located finding
// the caller can act on, instead of an unlocatable schema rejection.
export const mergedTeachingSchema = teachingSchema.extend({
  objectiveCoverage: z.array(coverageSchema.element.extend({explanationSectionIds: coverageSchema.element.shape.workedExampleSectionIds, independentQuestionKeys: coverageSchema.element.shape.workedExampleSectionIds})).min(1).max(8)
})
...
// DRAFT SCHEMA KEYED BY OBJECTIVE AND STAGE. The per-objective practice
// obligations are made impossible to omit at generation time: practice is an
// object keyed by every planned objective, each holding its guided,
// independent, transfer and remediation arrays with the contract's minimums
// (raised to the blueprint's planned counts for a first draft). Difficult
// objectives' core questions require a misconception, guided questions two
// hints, remediation none. The server flattens the response into the ordinary
// questions array (flattenDraftPractice) before the unchanged acceptance
// schema and checks run, so saved chapters keep their existing shape.
export const PRACTICE_STAGES = ['guided', 'independent', 'transfer', 'remediation']
export function draftResponseSchema(plan, evidenceIds, blueprint = null) {
  const base = studyResponseSchema(teachingSchema, evidenceIds, plan.objectives.map(o => o.id))
  const question = base.properties.questions.items
  delete question.properties.practiceStage
  question.required = question.required.filter(key => key !== 'practiceStage')
  const simpleIds = plan.objectives.filter(o => o.complexity === 'simple').map(o => o.id)
  const planned = (objectiveId, stage) => (Array.isArray(blueprint) ? blueprint : []).filter(row => row.objectiveId === objectiveId && row.stage === stage).length
  // At most six question variants, each defined once and referenced by every
  // objective's stage arrays, so the schema grows with the objective count by
  // references only (never by repeated evidence-id enums).
  const defs = {}
  const variant = (difficult, stage) => {
    const kind = stage === 'remediation' ? 'remediation' : stage === 'guided' ? 'guided' : 'core'
    const name = `question_${difficult ? 'difficult' : 'simple'}_${kind}`
    if (!defs[name]) {
      const item = structuredClone(question)
      // A simple objective's questions may only co-assess simple objectives, so
      // they never silently acquire a difficult objective's obligations.
      if (!difficult && simpleIds.length) item.properties.objectiveIds.items.enum = simpleIds
      if (kind === 'remediation') item.properties.misconceptions.maxItems = 0
      else if (difficult) item.properties.misconceptions.minItems = 1
      if (kind === 'guided') item.properties.hints.minItems = 2
      defs[name] = item
    }
    return {$ref: `#/$defs/${name}`}
  }
  const practice = {type: 'object', additionalProperties: false, required: plan.objectives.map(o => o.id), properties: {}}
  for (const objective of plan.objectives) {
    const difficult = objective.complexity === 'difficult'
    const contractMinimum = {guided: difficult ? 1 : 0, independent: 1, transfer: difficult ? 1 : 0, remediation: 0}
    const stages = {}
    for (const stage of PRACTICE_STAGES) {
      const minimum = Math.max(contractMinimum[stage], planned(objective.id, stage))
      stages[stage] = {type: 'array', items: variant(difficult, stage), ...(minimum ? {minItems: minimum} : {})}
    }
    practice.properties[objective.id] = {type: 'object', additionalProperties: false, required: [...PRACTICE_STAGES], properties: stages}
  }
  // Keep the chapter's field order: practice takes the questions position.
  base.properties = Object.fromEntries(Object.entries(base.properties).map(([key, value]) => key === 'questions' ? ['practice', practice] : [key, value]))
  base.required = Object.keys(base.properties)
  // One shared definition for the evidence-id enum wherever it occurs.
  if (evidenceIds?.length) {
    defs.evidence_id = {type: 'string', minLength: 1, maxLength: 120, enum: [...new Set(evidenceIds)]}
    const visit = node => {
      if (!node || typeof node !== 'object') return
      if (Array.isArray(node)) { node.forEach(visit); return }
      for (const key of ['sourceIds', 'answerSourceIds']) if (node.properties?.[key]?.items?.enum) node.properties[key].items = {$ref: '#/$defs/evidence_id'}
      Object.values(node).forEach(visit)
    }
    visit(base.properties)
    visit(defs)
  }
  base.$defs = defs
  return base
}
// Flatten a keyed draft response into the ordinary questions array: each
// question takes its stage from the array it was written in and always lists
// the objective it was written under. A response already carrying questions
// (a saved or scripted draft, a legacy client) is returned untouched.
export function flattenDraftPractice(raw) {
  let value = raw
  if (typeof raw === 'string') {
    try { value = parseUniqueJson(raw.replace(/^\s*```(?:json)?\s*/, '').replace(/\s*```\s*$/, '')) } catch { return raw }
  }
  if (!value || typeof value !== 'object' || Array.isArray(value) || value.questions !== undefined || !value.practice || typeof value.practice !== 'object' || Array.isArray(value.practice)) return raw
  const {practice, ...rest} = value
  const questions = []
  for (const [objectiveId, stages] of Object.entries(practice)) {
    for (const stage of PRACTICE_STAGES) for (const question of Array.isArray(stages?.[stage]) ? stages[stage] : []) {
      questions.push({...question, practiceStage: stage, objectiveIds: [...new Set([objectiveId, ...(Array.isArray(question?.objectiveIds) ? question.objectiveIds : [])])]})
    }
  }
  return JSON.stringify({...rest, questions})
}
...
export function outlinePrompt(course, maps, previous, capacity = null, {compact=false}={}) {
  const candidates=outlineCandidates(maps)
  // A whole-course scope policy already carries exact evidence size per
  // concept. Resending every concept's citation array, every passage size and
  // hundreds of mapper gap paragraphs adds no planning signal: the response
  // uses refs only, and resolveOutlineGroups restores every citation and gap
  // from the immutable maps. Keep the detailed packet for legacy single-guide
  // planning, where no per-concept policy accompanies the prompt.
  const mapped=compact?candidates.map(({ref,id,title})=>({ref,id,title})):candidates
  const promptCapacity=compact && capacity ? Object.fromEntries(Object.entries(capacity).filter(([key])=>!['evidenceSizes','scopeEvidenceIds'].includes(key))) : capacity
  const gaps=compact?[]:maps.flatMap(map=>map.gaps||[])
  const gapNote=compact?' Source-map gaps remain saved and are restored by the server; do not repeat them. Add only a new grouping-related gap if necessary.':''
  return `${RULES}\nCourse ${JSON.stringify(course)}. These are topic maps from ALL selected source batches. Plan coherent chapters around what the learner must understand or do, with related subtopics taught as sections. These maps contain candidate concepts, not a required chapter for every entry. Consolidate overlapping explanations across batches and years; the number of occurrences is not a reason to repeat a chapter. Keep all supporting evidence IDs and the supported learning goals. Combine interacting mechanisms into one instructional sequence when that improves learning. Do not add standalone chapters for incidental environment setup, tool installation or optional background unless it is itself a supported course learning objective. Preserve that useful background within its dependent chapter. Do not omit difficult content, merge unrelated goals, or weaken diagnostic practice just to reduce counts. Use the smallest coherent set of chapters that can teach the objectives well; capacity ceilings are not chapter-count targets. Chapter evidence capacity: ${JSON.stringify(promptCapacity)}. Scope evidence is shared context, not additional teaching weight for every topic. Respect the actual remaining evidence capacity when grouping; a mapping batch boundary is not a teaching boundary. Preserve earlier topic IDs for the same concepts. Earlier outline: ${JSON.stringify(previous.map(({ id, title }) => ({ id, title })))}\nMapped concepts: ${JSON.stringify(mapped)}\nSource gaps: ${JSON.stringify(gaps)}.${gapNote}\nReturn groups using topicRefs, not sourceIds. Assign EVERY mapped ref exactly once. The server retains the complete union of evidence IDs, concept titles and saved source gaps for each group; never omit a mapped concept to shrink the outline. Shape: {"topics":[{"id":"stable-topic-slug","title":"","topicRefs":["map-0-topic-0"]}],"gaps":["missing or conflicting evidence"]}. At most ${GUIDE_TOPIC_LIMIT} chapters. If there are more, keep separate parts with evidence represented and report the scope limit. Never claim completeness.`
}
