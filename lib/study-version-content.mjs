...
export function resolveOutlineGroups(result,maps) {
  ...
  const topics=result.topics.map(group=>{
    const selected=group.topicRefs.map(ref=>{
      if(!byRef.has(ref))throw new StudyVersionError(`Outline references an unknown mapped concept: ${ref}`,502)
      if(used.has(ref))throw new StudyVersionError(`Outline assigns a mapped concept twice: ${ref}`,502)
      used.add(ref);return byRef.get(ref)
    })
    return {id:group.id,title:group.title,sourceIds:[...new Set(selected.flatMap(item=>item.sourceIds))],concepts:[...new Set(selected.map(item=>item.title))],conceptEvidence:selected.map(item=>({title:item.title,sourceIds:[...item.sourceIds]}))}
  })
  const missing=candidates.filter(item=>!used.has(item.ref))
  if(missing.length)throw new StudyVersionError(`Outline omitted ${missing.length} mapped concepts: ${missing.slice(0,5).map(item=>item.ref).join(', ')}. Consolidate their teaching without dropping coverage.`,502)
  return {topics,gaps:[...new Set([...maps.flatMap(map=>map.gaps||[]),...(result.gaps||[])])]}
}
