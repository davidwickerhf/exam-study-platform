...
export const createCanvasConsumer = (visibilityTimeoutSeconds = 300) => handleCallback(async (message: { version: number; jobId: string; probe?: string }) => {
...
}, { visibilityTimeoutSeconds, retry: (_error, metadata) => ({ afterSeconds: Math.min(300, 15 * metadata.deliveryCount) }) })
