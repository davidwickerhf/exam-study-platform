...
export function legacyHashTarget(hash = '') {
  ...
  if (parts[0] === 'settings' || parts[0] === 'account') {
    ...
    return tab === 'profile' ? '/app/account' : `/app/settings?tab=${tab}`
  }
  ...
  if (parts[0] === 'planning') {
    if (parts[1] === 'documents') return '/app/documents'
    const aliases = { curriculum: 'courses', credits: 'progress', requirements: 'progress' }
    const requested = aliases[parts[1]] || parts[1] || 'overview'
    const tab = ['overview', 'courses', 'progress', 'planner', 'settings'].includes(requested) ? requested : 'overview'
