...
export function paperSelection(sets, jobs, keys, selectedId) {
  const sourceKeys = new Set(Array.isArray(keys) ? keys : [keys])
  const matches = sets.filter(s => sourceKeys.has(s.questionSourceKey))
  const job = jobs.find(j => sourceKeys.has(j.sourceKey))
