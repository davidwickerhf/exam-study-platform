...
export type PlannerAcademicPeriod = { id: string; title: string; date: string; endDate?: string | null; kind?: string; period?: number | null; semester?: number | null; resit?: boolean; cohorts?: string[]; academicYear?: string; sourceLabel?: string }
export type PlanningOffering = { sourceId: string; label: string; kind: string; period: number | null; semester: number | null; resit: boolean; cohorts: string[]; sourceLabel: string }
export type PlanningSession = { id: string; aliases?: string[]; sourceIds?: string[]; label: string; academicYear: string | null; period: number | null; semester?: number | null; startsAt: string | null; endsAt: string | null; kind: string; resit: boolean; hasResit?: boolean; offerings?: PlanningOffering[] }
...
export declare function planningSessions(workspace: PlannerWorkspace, options?: { academicYear?: string | null; courses?: PlannerCourse[] }): PlanningSession[]
export declare function planningDestinations(workspace: PlannerWorkspace, courseId: string): { courseId: string; courseCode: string; teachingPeriod: string | null; evidenceSource: string; period: number | null; semester: number | null; resitRules: Array<string | { text?: string }>; allowedSessionIds: string[]; destinations: Array<PlanningSession & { allowed: boolean; role: 'primary' | 'resit' | 'continuous' | 'carry' | null; reason: string | null }> }
