...
export function canvasCorpusSummary(status) {
  ...
  for (const job of jobs) {
    ...
    // The API sends newest first; keep the first reading for each course.
    if (job?.courseCode && !latest.has(job.courseCode)) latest.set(job.courseCode, job)
  }

  // A retry supersedes only the same Canvas shell. A completed older year
  // must never conceal a pending/failed current sitting of the same course.
  for (const job of Array.isArray(status?.latestJobs) ? status.latestJobs : jobs) {
    if (!job?.courseCode) continue
    const edition = job.bindingId || `${job.origin || ''}:${String(job.courseCode).trim().toUpperCase()}:${job.academicYear || ''}`
    if (!latestEditions.has(edition)) latestEditions.set(edition, job)
  }
  return {
    jobs,
    active,
    ...
    latestByEdition: [...latestEditions.values()],
    failureGroups: [...failures.entries()],
    courseEditions: courses.reduce((total, course) => total + Math.max(1, Number(course?.editionCount) || 0), 0),
    storedMaterials: courses.reduce((total, course) => total + (Number(course?.sources) || 0), 0)
  }
}

/**
 * The current visible Canvas state, reduced for Home and the detail page.
 *
 * More than one persisted batch can still contain an old pending/running row
 * after a retry. Those rows belong in Run history, but counting them here made
 * the headline disagree with the current-by-course table. Use only the newest
 * job for each Canvas course edition plus the newest catalogue job.
 */
export function canvasSyncProgress(status) {
  const jobs = Array.isArray(status?.jobs) ? status.jobs : []
  const summary = canvasCorpusSummary(status)
  const courses = summary.latestByEdition.filter((job) => job.type === 'course' || job.courseCode)
  const activeCourses = courses.filter((job) => RUNNING.has(job?.status))
  const latestCatalog = jobs.find((job) => job.type === 'catalog')
  const catalogActive = !activeCourses.length && latestCatalog && RUNNING.has(latestCatalog.status) ? latestCatalog : null
  const activeJobs = catalogActive ? [catalogActive, ...activeCourses] : activeCourses
  if (!activeJobs.length) return {
    active: false, percent: null, stage: null, activeJobs: [], jobs: [], totalCourses: 0,
    settledCourses: 0, completedCourses: 0, failedCourses: 0, indexedFiles: 0
  }
  const completed = courses.filter((job) => job.status === 'completed')
  const failed = courses.filter((job) => job.status === 'failed')
  const settled = courses.filter((job) => ['completed', 'failed', 'cancelled'].includes(job.status)).length
  const runningCourses = activeCourses.filter((job) => job.status === 'running')
  const queuedCourses = activeCourses.filter((job) => job.status === 'pending')
  const hasCourseLedger = Array.isArray(status?.courses)
  const storedFiles = hasCourseLedger
    ? status.courses.reduce((sum, course) => sum + (Number(course?.sources) || 0), 0)
    : 0
  return {
    active: true,
    percent: courses.length ? Math.round((settled / courses.length) * 100) : null,
    stage: catalogActive
      ? 'Reading the Canvas course list'
      : runningCourses.length
        ? `Indexing ${runningCourses.map((job) => job.courseCode || job.courseName || 'course material').slice(0, 2).join(' and ')}`
        : queuedCourses.length
          ? `Waiting to index ${queuedCourses.map((job) => job.courseCode || job.courseName || 'course material').slice(0, 2).join(' and ')}`
          : 'Waiting for the next course import',
    activeJobs,
    jobs: [catalogActive, ...courses].filter(Boolean),
    totalCourses: courses.length,
    settledCourses: settled,
    completedCourses: completed.length,
    failedCourses: failed.length,
    indexedFiles: hasCourseLedger ? storedFiles : completed.reduce((sum, job) => sum + (Number(job.result?.indexed) || 0), 0)
  }
}
