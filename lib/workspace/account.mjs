...
/** One real server-side Canvas batch, reduced for Home and the detail page. */
export function canvasSyncProgress(status) {
  const jobs = Array.isArray(status?.jobs) ? status.jobs : []
  const activeJobs = jobs.filter((job) => RUNNING.has(job?.status))
  if (!activeJobs.length) return {
    active: false, percent: null, stage: null, activeJobs: [], jobs: [], totalCourses: 0,
    completedCourses: 0, failedCourses: 0, indexedFiles: 0
  }
  const syncIds = new Set(activeJobs.map((job) => job.syncId || job.id).filter(Boolean))
  const batch = jobs.filter((job) => syncIds.has(job.syncId || job.id))
  const courses = batch.filter((job) => job.type === 'course')
  ...
  const catalogActive = activeJobs.some((job) => job.type === 'catalog')
  const runningCourses = activeJobs.filter((job) => job.type === 'course' && job.status === 'running')
  return {
    active: true,
    ...
    indexedFiles: completed.reduce((sum, job) => sum + (Number(job.result?.indexed) || 0), 0)
  }
}
