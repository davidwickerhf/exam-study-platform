...
/**
 * The control a question can honestly support. Closed questions only become
 * choosers when their published data is complete; a broken option list falls
 * back to a written response instead of drawing unusable controls.
 */
export function questionAnswerMode(question) {
  const type = String(question?.type ?? '').trim()
  if (type === 'tf') return 'true-false'
  const options = usableOptions(question)
  if (type === 'mc' && options.length > 1) return 'single-choice'
  if (type === 'multi' && options.length > 1) return 'multiple-choice'
  return 'written'
}

/** Multi-select answers keep the source order so grading is deterministic. */
export function formatChoiceAttempt(options, selected) {
  const held = new Set(Array.isArray(selected) ? selected : [])
  return (Array.isArray(options) ? options : [])
    .filter((option) => held.has(option))
    .join('\n')
}
...
export function practiceHeadline(state = {}) {
  ...
  return `${plural(questionCount, 'question')} across ${plural(courseCount, 'active course')}.`
}
