...
export function createResourceCache({ fetchImpl = (...args) => fetch(...args), now = Date.now, maxAge = 300_000 } = {}) {
  ...
  function invalidate(key, { discard = false } = {}) {
    const keys = key == null ? new Set([...entries.keys(), ...listeners.keys()]) : typeof key === 'function' ? [...new Set([...entries.keys(), ...listeners.keys()])].filter(key) : [key]
  ...
  function load(key, { force = false, requestUrl = key } = {}) {
    const current = read(key)
    if (current.pending) return current.pending
    if (!force && current.data !== undefined && current.fetchedAt > now() - (/\/(corpus|sync|events)(?:[/?]|$)/.test(key) ? Math.min(maxAge, 30_000) : maxAge)) return Promise.resolve(current.data)
    const entry = { ...current, error: null, pending: null }
    const pending = Promise.resolve().then(() => fetchImpl(requestUrl, { headers: { accept: 'application/json' }, signal: AbortSignal.timeout(30_000) }))
      .then(async response => {
...
export function workspaceWriteAffectsReads(path, method) {
  ...
  if (workspaceInvalidationTargets(path)) return true
  if (path.startsWith('/api/tutor/') && path !== '/api/tutor/actions' && !/\/diagnostics\//.test(path)) return false
  return true
}

/** Known mutations invalidate their dependencies, not unrelated course/Canvas payloads. */
export function workspaceInvalidationTargets(path) {
  if (path.startsWith('/api/onboarding/tour')) return key => key.startsWith('/api/onboarding')
  if (path === '/api/tutor') return key => key.startsWith('/api/tutor')
  if (path.startsWith('/api/tutor/attachments') || path.startsWith('/api/tutor/conversations') || path.startsWith('/api/tutor/plans') || path.startsWith('/api/tutor/preferences')) return key => key.startsWith('/api/tutor')
  if (path.startsWith('/api/account/integrations/canvas') || path.startsWith('/api/integrations/canvas')) return key => /canvas|workspace-shell|academics|calendar|tutor/.test(key)
  // Unknown writes retain conservative invalidation until their dependencies are declared.
  return null
}
