...
function actionIdentity(value) {
  return String(value || '').toLowerCase().replace(/^(?:submit|upload|complete|hand in|turn in|prepare)\s+(?:the\s+)?/,'').replace(/[^a-z0-9]+/g,' ').trim()
}
function obligationDay(value) {
  if (!value) return ''
  if (String(value).length<=10) return String(value)
  const date=new Date(value)
  return Number.isFinite(date.getTime()) ? new Intl.DateTimeFormat('en-CA',{timeZone:'Europe/Amsterdam',year:'numeric',month:'2-digit',day:'2-digit'}).format(date) : ''
}
...
export function homePriorities({ events = [], assignments = [], courses = [], now = Date.now(), limit = 4 } = {}) {
  ...
  for (const event of events) {
    if (event.category !== 'timetable' || hasPassed(event.end || event.start, now) || seenEvents.has(event.id)) continue
    ...
    if (!policy?.required && !policy?.participationAssessed) continue
    ...
    const item = {
      ...
      status: policy.participationAssessed ? 'Assessed participation' : 'Required',
  ...
  for (const assignment of assignments) {
    ...
    const canvasDate = obligationDay(assignment.dueAt)
    const ruleDate = obligationDay(matched?.component?.deadline)
  ...
  for (const course of courses) {
    ...
    for (const [index, action] of actions.entries()) {
      // Match only the actual deliverable, never every step of a shared project.
      // A completed final report must not mark its separate presentation done.
      const equivalent = assignments.find(assignment=>String(assignment.courseCode || '').toLowerCase()===String(course.code || '').toLowerCase() && actionIdentity(action.title)===actionIdentity(deadlineTitle(assignment)))
      if (equivalent) continue
      const past=hasPassed(action.deadline,now)
      priorities.push({
        id:`step:${course.id}:${index}`,kind:'project',title:action.title,
        detail:[action.parent,action.prerequisite || null,action.notes].filter(Boolean).join(' · '),
        courseCode:course.code ?? null,dueAt:action.deadline || null,
        dueText:action.deadlineText || (action.deadline ? null : 'Timing not specified'),
        href:`/app/courses/${encodeURIComponent(course.id)}?tab=materials`,
        status:past ? 'Check completion' : 'Project step',
        source:'Verified course rule',evidence:action.evidence || [],rank:past ? 0 : action.deadline ? 3 : 4
      })
    }
    for (const [index, component] of (assessment.components ?? []).entries()) {
      if (actions.some(action=>actionIdentity(action.title)===actionIdentity(component.name) && obligationDay(action.deadline)===obligationDay(component.deadline))) continue
      ...
      if (!component.deadline) continue
      const dueAt = component.deadline
      if (hasPassed(dueAt, now)) continue
      priorities.push({
        id: `project:${course.id}:${index}`,
        kind: component.type === 'exam' ? 'exam' : component.type === 'assignment' ? 'assignment' : 'project',
        ...
        dueAt,
        dueText: component.deadlineText || null,
        href: `/app/courses/${encodeURIComponent(course.id)}?tab=materials`,
        evidence: component.evidence || [],
        status: component.type === 'exam' ? 'Exam' : 'Milestone',
