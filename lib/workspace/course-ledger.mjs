 ...
 * proves they passed or are taking them. Failed past options remain history,
 * not an additional selection. A passed course is joined by stable code to today's
...
export function degreeRunwayYears({ programme = null, version = null, programmeTemplate = null, academic = [], currentCodes = new Set() } = {}) {
  ...
  return labels.map((label, index) => {
    ...
    const isHistoricalOption = (course) => {
      const recorded = academicByCode.get(key(course.code))
      return Boolean(recorded?.attempts?.length) && !recorded.hiddenFromStats
        && courseStatus(recorded) !== 'passed'
        && !recorded.attempts.some((attempt) => attempt.status === 'upcoming' || attempt.registered)
        && !selectedIds.has(String(course.id)) && !canonicalCurrentCodes.has(key(course.code))
    }
    const selected = options.filter((course) => {
      const recorded = academicByCode.get(key(course.code))
      return selectedIds.has(String(course.id))
        || Boolean(recorded && !recorded.hiddenFromStats && (courseStatus(recorded) === 'passed'
          || recorded.attempts?.some((attempt) => attempt.status === 'upcoming' || attempt.registered)))
    ...
    const passed = courses.filter((course) => {
      const recorded = academicByCode.get(key(course.code))
      return recorded && !recorded.hiddenFromStats && courseStatus(recorded) === 'passed'
    })
    ...
    const earnedEcts = Number(passed.reduce((sum, course) => sum + courseEarnedCredits(academicByCode.get(key(course.code))), 0).toFixed(1))
    ...
    return {
      ...
      historicalOptions: options.filter(isHistoricalOption).map((course) => ({ code: course.code, name: course.name, ects: Number(course.ects) || 0 })),
      running,
...
export function rowDestination(entry) {
  ...
  // The course record is useful even before chapters exist. Use its code so
  // catalogue-only and Canvas-only rows have the same detail destination.
  return { kind: 'study', href: `/app/courses/${encodeURIComponent(entry.code)}`, action: 'Open course', chapters: 0 }
}
