...
export function resumableAuthSessions(sessions, selectedSessionId) {
  // A selected pending session belongs to Clerk's task/MFA flow. Never replace
  // it with another account, or try to revive an expired/revoked session.
  if (selectedSessionId) return []
  return (sessions || []).filter(session => session.status === 'active' && !session.currentTask)
}

export function safeAuthDestination(value, origin) {
  try {
    const url = new URL(value || '/app', origin)
    if (url.origin !== origin || !/^\/app(?:\/|$)/.test(url.pathname) && url.pathname !== '/connect' && url.pathname !== '/connect/remote') return '/app'
