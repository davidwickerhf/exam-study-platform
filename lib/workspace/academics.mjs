...
export function planningTab(value) {
  const aliases = { curriculum: 'courses', credits: 'progress', requirements: 'progress', documents: 'overview' }
  const requested = aliases[String(value ?? '')] ?? String(value ?? '')
  return ['overview', 'courses', 'progress', 'planner', 'settings'].includes(requested) ? requested : 'overview'
}
