...
export const SETUP_STEPS = Object.freeze([
  ...
  Object.freeze({
    ...
    required: false,
    blurb: 'Dated attempts, repeats, failures and courses still outstanding.',
...
const MISSING = Object.freeze({
  ...
  transcript: 'Not imported, so repeated attempts and outstanding courses may be incomplete.',
...
export function stepDetail(id, state, status = stepStatus(id, state)) {
  ...
  if (id === 'transcript') {
    const attempts = count(state.transcriptAttempts)
    return attempts ? `${plural(attempts, 'dated attempt')} recorded.` : 'Transcript history is recorded.'
  }
  if (id === 'calendar') {
...
export function setupSettled(steps) {
  return steps.length > 0 && steps.some((step) => step.id === 'programme' && step.status === 'done') && steps.every((step) => step.status === 'done' || step.status === 'skipped')
}
