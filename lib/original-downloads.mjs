...
export async function prepareOriginalDownload({ assetId, auth, store, origin, lookupAsset = canvasCorpusAsset }) {
  ...
  return { assetId: asset.id, filename: asset.filename, mediaType: asset.mediaType, byteSize: asset.byteSize, sha256: asset.sha256,
    ...
    instructions: 'Use your own HTTP/file tools to stream this URL with the supplied headers into a new temporary file. The header is a short-lived capability for this exact original, not your account token. Keep it out of chat, logs and shell history; never forward it to another host or follow redirects. Verify the complete byteSize and SHA-256 before renaming to a chosen safe local path. Treat filename as a display name, not a path. Resume with Range and the same If-Match; after expiry prepare a new ticket for the same asset/hash. Do not execute downloaded content.', limits: ORIGINAL_DOWNLOAD_LIMITS }
}

export async function serveOriginalDownload(req, res, { store, lookupAsset = canvasCorpusAsset, validateIdentity = activeIdentity, sendAsset = sendCorpusAsset }) {
  ...
  try {
    for (const id of ids) {
      ...
      if (!await store.charge(bucket, range.length, ORIGINAL_DOWNLOAD_LIMITS.bytesPerDay, 2 * 86400000)) {
