import { evidencePrompt, lessonPrompt, reviewPrompt, teachingSchema, reviewSchema } from './study-version-content.mjs'
...
export function evaluationStep(row) {
  ...
  if (stage === 0) return { schema: teachingPlanSchema, prompt: teachingPlanPrompt(context, topic), tokens: 6000, kind: 'plan' }
  if (stage === 1) return { schema: teachingSchema, prompt: lessonPrompt(course, snapshot.sources, snapshot.chunks, topic, row.teachingPlan), tokens: 20000, kind: 'lesson' }
  ...
  return { schema: pedagogyReviewSchema, prompt: pedagogyPrompt(fixture ? evidencePrompt(fixture.course, fixture.sources, fixture.chunks) : context, chapter), tokens: 10000, kind: stage === 3 ? 'pedagogy' : stage === 5 ? 'iot-teaching' : 'iot-shallow', chapter }
}
