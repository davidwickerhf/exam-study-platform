...
function compactEvent(event) {
  return {
    ...
    notes: clean(event.notes, 2000),
    type: clean(event.type, 40) || null,
...
// Only reuse the current account's snapshot for the exact configured feed.
// This is a saved copy, not evidence of a successful refresh.
export async function savedCalendarFeeds(workspaceId, links) {
  const held = await readDocument(NAMESPACE, clean(workspaceId, 100) || 'default', { feeds: {} })
  return links.flatMap((link) => {
    const saved = held?.feeds?.[link.id]
    return saved?.url === link.url && Array.isArray(saved.events)
      ? [{ link, events: saved.events, savedAt: saved.observedAt || null }]
      : []
  })
}
