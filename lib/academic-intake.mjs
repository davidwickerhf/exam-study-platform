import { documentRows, validateDocumentRows } from './academic-document-check.mjs'
import { parseAcademicWork } from './academic-work.mjs'
import { transcriptCourseTitle } from './academic-record-repair.mjs'
...
const normalizedName = (value) => cleanText(value, 240).normalize('NFKD').replace(/[\u0300-\u036f]/g, '').toLocaleLowerCase().replace(/\bai\b/g, 'artificial intelligence').replace(/[^\p{L}\p{N}]+/gu, ' ').trim()
const decimal = (value) => {
...
function parseAcademicOverview(source) {
  ...
  const parsed = parseAcademicWork(source)
  return inferAttemptTypes(parsed.courses.map((row) => ({
    code: row.code, name: row.name, ects: row.creditsTotal, yearLevel: '', period: row.period || '', passMark: 5.5,
    programmeRequirement: row.status === 'upcoming' ? null : 'historical',
    attempts: [{ academicYear: row.academicYear.replace('-', '–'), type: 'first', examDate: null, grade: row.grade, status: row.status === 'exempt' ? 'passed' : row.status, creditsEarned: row.creditsEarned }]
  })))
}
...
function parseOfficialTranscript(source, codeByName = new Map()) {
  ...
  for (let index = 0; index < rawLines.length; index += 1) {
    ...
    if (/^(?:ECTS\s*)+$/i.test(line)) continue
    if (!transcriptRow.test(line) && !transcriptBoundary.test(line)) {
      ...
      for (let lookahead = index + 1; lookahead < rawLines.length && lookahead <= index + 2; lookahead += 1) {
        if (transcriptBoundary.test(rawLines[lookahead]) || /^(?:ECTS\s*)+$/i.test(rawLines[lookahead])) break
  ...
  for (const line of lines) {
    ...
    const name = transcriptCourseTitle(row[1])
    ...
    courses.push({
      ...
      attempts: [{ academicYear: academicYearForDate(examDate), type: 'first', examDate, grade, creditsEarned: earnedCredits, status: earnedCredits > 0 ? 'passed' : /^NG$/i.test(row[2]) ? 'no-show' : 'failed' }]
    })
  }
...
export function normalizeAcademicIntakeDraft(value, editorialCourses = [], { kind = 'auto', identityCourses = [] } = {}) {
  ...
  if ((value?.courses || []).length > 500 || (value?.courses || []).some((course) => (course.attempts || []).length > 50)) throw new Error('This import exceeds the programme record capacity. No partial results were imported.')
  ...
  if (deduped.some((course) => course.attempts.length > 50)) throw new Error('This import exceeds the attempt history capacity for a course. No partial results were imported.')
...
export function fallbackAcademicIntake(rawText, editorialCourses = [], { kind = 'auto', identityCourses = [] } = {}) {
  ...
  if (transcriptCourses.length || overviewCourses.length) {
    const rows = documentRows({ courses: transcriptCourses.length ? transcriptCourses : overviewCourses })
    const declared = transcriptCourses.length ? source.match(/^\s*(?:Total\s+)?(?:earned credits|credits earned|earned ECTS)\s*[:=]?\s*(\d+(?:[.,]\d+)?)\s*(?:ECTS)?\s*$/mi) : null
    const expectedRows = transcriptCourses.length ? source.split('\n').filter((line) => /\b\d{2}\.\d{2}\.\d{4}\b/.test(line) && !/^(?:printed|date|birth|issued|geboorte)/i.test(line.trim())).length : rows.length
    const validation = validateDocumentRows(rows, { expectedRows, declaredCredits: declared ? decimal(declared[1]) : null })
    if (validation.issues.length) throw new Error(validation.issues.join(' '))
    sourceEvidence = { kind: transcriptCourses.length ? 'transcript' : 'academic-overview', rows, validation }
  } else if (detected) {
    throw new Error('The document layout was recognised, but its result rows could not be read completely. Upload a text-based export from the student portal.')
  }
  return normalizeAcademicIntakeDraft({ profile, courses, events: [], warnings, sourceEvidence }, editorialCourses, { kind: effectiveKind, identityCourses })
}
