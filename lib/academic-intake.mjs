...
function uniqueAttempts(items = []) {
  ...
  for (const attempt of items) {
    ...
    if (compatible) {
      if (!compatible.examDate && attempt.examDate) Object.assign(compatible, attempt)
      continue
    }
    merged.push({ ...attempt })
  }
...
function parseOfficialTranscript(source, codeByName = new Map()) {
  ...
  for (const line of lines) {
    ...
    const row = line.match(/^(.+?)\s+(NG|\d+(?:[.,]\d+)?)\s+(\d{2}\.\d{2}\.\d{4})\s+(\d+(?:[.,]\d+)?)\s+(\d+(?:[.,]\d+)?)(?:\s+\d+)?$/i)
...
export function normalizeAcademicIntakeDraft(value, editorialCourses = [], { kind = 'auto' } = {}) {
...
export function fallbackAcademicIntake(rawText, editorialCourses = [], { kind = 'auto' } = {}) {
  ...
  return normalizeAcademicIntakeDraft({ profile, courses, events: [], warnings }, editorialCourses, { kind: effectiveKind })
}
