...
import { attendanceOverview } from './attendance.mjs'
import { savedCalendarFeeds } from './calendar-changes.mjs'
...
export async function calendarFeedsWithFallback(workspaceId, links, { readFeed = feedEvents } = {}) {
  const outcomes = await Promise.allSettled(links.map((link) => readFeed(link)))
  const failed = links.filter((_, index) => outcomes[index].status === 'rejected')
  const feeds = [], freshFeeds = [], problems = []
  let saved = []
  if (failed.length) {
    try { saved = await savedCalendarFeeds(workspaceId, failed) }
    catch {
      problems.push({ id: 'calendar-snapshot', label: 'Saved timetable', error: 'The saved timetable could not be loaded. Try again later.' })
    }
  }
  for (const [index, outcome] of outcomes.entries()) {
    const link = links[index]
    if (outcome.status === 'fulfilled') {
      const feed = { link, events: outcome.value }
      feeds.push(feed)
      freshFeeds.push(feed)
    } else {
      const fallback = saved.find((feed) => feed.link.id === link.id)
      if (fallback) feeds.push(fallback)
      problems.push({ id: link.id, label: link.label, error: outcome.reason instanceof Error ? outcome.reason.message : String(outcome.reason), usingSaved: Boolean(fallback), savedAt: fallback?.savedAt || null })
    }
  }
  return { feeds, freshFeeds, problems }
}
...
function timetableActivity(event) {
  ...
  const source = /\bproject[ -]+opening\b/i.test(event?.title || '') ? String(event.title) : labelled || String(event?.title || '')
...
export function aggregateCalendar({ workspace, editorialCourses = [], ruleCourses = editorialCourses, institutionCalendar = [], feeds = [], canvas = null, date = new Date() }) {
  const events = []
  const selectedCourses = workspace.courses || []
  ...
  for (const event of workspace.planning?.calendarEvents || []) {
    const course = selectedCourses.find((item) => item.id === event.courseId)
    const reference = course ? { course, code: course.code || event.courseCode } : courseReferenceInText(`${event.courseCode || ''} ${event.courseName || ''} ${event.title || ''}`, selectedCourses)
    const editorial = reference?.code ? editorialByCode.get(String(reference.code).toUpperCase()) : null
    events.push({
      id: `personal:${event.id}`,
      managedEventId: event.id,
      managedCalendarId: event.calendarId || 'wicker',
      editable: true,
      title: event.title,
      start: event.start,
      end: event.allDay && event.end ? nextDay(event.end) : event.end || null,
      allDay: Boolean(event.allDay),
      category: ['deadline', 'study', 'appointment'].includes(event.type) ? event.type : 'other',
      courseId: reference?.course?.id || event.courseId || null,
      courseCode: reference?.code || event.courseCode || null,
      courseName: reference?.course?.name || event.courseName || null,
      editorialCourseId: editorial?.id || reference?.course?.editorialCourseId || null,
      colour: editorial?.accent || null,
      source: `personal:${event.calendarId || 'wicker'}`,
      feedLabel: 'Wicker calendar',
  ...
  for (const feed of feeds) {
    for (const event of feed.events || []) {
      ...
      if (event.cancelled || event.status === 'CANCELLED') continue
      const timed = /^\d{2}:\d{2}/.test(event.notes || '')
      const time = event.startTime ? [null, event.startTime, event.endTime] : timed ? event.notes.match(/^(\d{2}:\d{2})(?:–(\d{2}:\d{2}))?/) : null
      const reference = courseReferenceInText(`${event.title || ''} ${event.notes || ''}`, selectedCourses)
      const editorial = reference?.code ? editorialByCode.get(reference.code) : null
      ...
      events.push({
        ...
        end: time?.[2] ? `${event.endDate || event.date}T${time[2]}:00` : event.endDate ? nextDay(event.endDate) : null,
        allDay: !time,
        category: 'timetable',
        subtype: event.type,
        courseId: reference?.course?.id || null,
        ...
        courseName,
        activity,
        attendanceEligible: Boolean(reference?.code && isTeachingAppointment(event)),
        editorialCourseId: editorial?.id || reference?.course?.editorialCourseId || null,
        colour: editorial?.accent || null,
        ...
        ...(feed.savedAt !== undefined ? { stale: true, savedAt: feed.savedAt } : {}),
        href: null
      })
    }
  }
  ...
  const attendance = attendanceOverview(events, workspace.planning?.attendanceRecords || [], ruleCourses, { now: date ? new Date(date).getTime() : Date.now() })
  return { events: attendance.events, categories: CALENDAR_CATEGORIES, courses, reconciliation, academicContext, periodCourses, currentCourses, examWindow, attendance: { summary: attendance.summary, courses: attendance.courses } }
}
