import { validateDocumentRows, documentCredits } from './academic-document-check.mjs'
...
export function parseAcademicWork(input) {
  ...
  // Long descriptions may wrap before the result/credits columns. Join only
  // a row that already starts with a Maastricht identity, and stop as soon as
  // its terminal result + earned/available credit cells are present. This
  // keeps headings and the code-only line used by older printouts separate.
  const hasResultCells = (line) => /\s(-|NG|\d+(?:[.,]\d+)?)\s+\d+(?:[.,]\d+)?\s*\/\s*\d+(?:[.,]\d+)?\s*$/i.test(line)
  const assembled = []
  for (let index = 0; index < joined.length; index += 1) {
    const current = joined[index]
    let line = current.line
    if (!line || !/^\d{4}\s*[-–]\s*\d{4}\s*-\s*\d{3}\s*-/.test(line) || hasResultCells(line)) {
      assembled.push(current)
      continue
    }
    let lookahead = index + 1
    while (lookahead < joined.length && lookahead <= index + 3 && !hasResultCells(line)) {
      const continuation = joined[lookahead]?.line
      if (!continuation || SECTIONS.some(([pattern]) => pattern.test(continuation))) break
  ...
  const expectedRows = lines.filter((line) => /^\s*\d{4}\s*[-–]\s*\d{4}\s*-\s*\d{3}\s*-/.test(line)).length
  const validation = validateDocumentRows(rows, { expectedRows })
  if (validation.issues.length) throw new AcademicWorkError(validation.issues.join(' '))
...
export function summariseAcademicWork(courses = []) {
  const passed = [...new Map(courses.filter((course) => ['passed', 'exempt'].includes(course.status)).sort((a, b) => String(a.academicYear).localeCompare(String(b.academicYear))).reverse().map((course) => [course.code, course])).values()]
  ...
  return {
    earnedEcts: documentCredits(courses),
...
const comparableYear = (value) => String(value || '').replace(/[–—/]/g, '-').trim()

function recordAttempt(row, index, previous = null) {
  const academicYear = comparableYear(row.academicYear)
  const previousYear = comparableYear(previous?.academicYear)
  return {
    academicYear,
    type: index === 0 ? 'first' : previousYear === academicYear ? 'resit' : 'carry-over',
    examDate: null,
    grade: row.grade == null ? null : Number(row.grade),
    status: row.status === 'exempt' ? 'passed' : row.status,
    courseCode: row.code,
    courseName: row.name,
    ects: row.creditsTotal ?? null,
    creditsEarned: row.creditsEarned ?? null,
    yearLevel: '',
    period: row.period || '',
    curriculumVersion: academicYear
  }
}

function sameRecordedAttempt(left, right) {
  return comparableYear(left?.academicYear) === comparableYear(right?.academicYear)
    && String(left?.status || '') === String(right?.status || '')
    && (left?.grade == null ? null : Number(left.grade)) === (right?.grade == null ? null : Number(right.grade))
}

/**
 * Academic Work is an authoritative, user-uploaded record, not merely a chart
 * data source. Older uploads were saved as version snapshots but were not
 * joined back onto the academic workspace, which made passed courses render
 * as "not yet recorded". Merge the derived rows by stable course code while
 * preserving any richer dated transcript attempt already on the course.
 *
 * This is idempotent so existing snapshots can repair the workspace on its
 * next read without creating duplicate attempts.
 */
export function mergeAcademicWorkIntoWorkspace(workspace, rows = []) {
  ...
  for (const row of rows || []) {
    ...
    if (!code || !row?.name) continue
    const held = grouped.get(code) || []
    held.push({ ...row, code })
    grouped.set(code, held)
  }

  for (const [code, recordRows] of grouped) {
    recordRows.sort((left, right) => comparableYear(left.academicYear).localeCompare(comparableYear(right.academicYear)))
    let course = next.courses.find((entry) => String(entry.code || '').trim().toUpperCase().replace(/\s+/g, '') === code)
    if (!course) {
      const latest = recordRows.at(-1)
      course = {
        id: `record-${code.toLowerCase()}`,
        code,
        name: latest.name || code,
        ects: Number(latest.creditsTotal) || 0,
        yearLevel: '',
        period: latest.period || '',
        passMark: 5.5,
        notes: 'Reconciled from the uploaded Academic Work record.',
        programmeRequirement: latest.status === 'upcoming' ? null : 'historical',
        attempts: []
      }
    ...
    recordRows.forEach((row, index) => {
      const incoming = recordAttempt(row, index, recordRows[index - 1])
      const existing = course.attempts.find((attempt) => sameRecordedAttempt(attempt, incoming))
      if (existing) {
        existing.courseCode ||= incoming.courseCode
        existing.courseName ||= incoming.courseName
        existing.ects ??= incoming.ects
        existing.creditsEarned ??= incoming.creditsEarned
        existing.period ||= incoming.period
        existing.curriculumVersion ||= incoming.curriculumVersion
        return
      }
      course.attempts.push(incoming)
    })
    if (!course.ects) course.ects = Number(recordRows.find((row) => Number(row.creditsTotal) > 0)?.creditsTotal) || 0
    if (recordRows.some((row) => row.status === 'upcoming') && course.programmeRequirement === 'historical') course.programmeRequirement = null
  }

  return normalizeAcademicWorkspace(next)
}
