...
async function scopedLocalRows(table, programmeId) {
  const rows = await localRows(table)
  let changed = false
  for (const row of rows) if (!row.programmeId) { row.programmeId = programmeId; changed = true }
  if (changed) await saveLocalRows(table, rows)
  return rows.filter((row) => row.programmeId === programmeId)
}

async function scopedLocalUpsert(table, programmeId, key, entries) {
  return localUpsert(table, (row) => `${row.programmeId || programmeId}:${key(row)}`, entries.map((row) => ({ ...row, programmeId })))
}

async function scopedLocalDelete(table, programmeId, predicate) {
  return localDelete(table, (row) => (row.programmeId || programmeId) === programmeId && predicate(row))
}
...
export async function listCourseSettings() {
  ...
  if (sql) {
    const rows = await sql`SELECT course_id, archived, sort_order FROM course_settings WHERE user_id = ${userId()} AND programme_id = ${programmeId}`
  ...
  return scopedLocalRows('course_settings', programmeId)
}
...
export async function upsertCourseSettings(entries) {
  ...
  if (sql) {
    for (const entry of list) {
      await sql`INSERT INTO course_settings (user_id, programme_id, course_id, archived, sort_order, updated_at)
        VALUES (${userId()}, ${programmeId}, ${entry.courseId}, ${entry.archived === true}, ${entry.order == null ? null : Math.trunc(entry.order)}, now())
        ON CONFLICT (user_id, programme_id, course_id) DO UPDATE SET
  ...
  const existing = new Map((await scopedLocalRows('course_settings', programmeId)).map((row) => [row.courseId, row]))
  ...
  await scopedLocalUpsert('course_settings', programmeId, (row) => row.courseId, merged)
}
...
export async function listItemProgress() {
  const programmeId = await activeProgrammeId()
  if (sql) {
    const rows = await sql`SELECT item_id, course_id, mastery, mastery_updated_at, notes, priority, review_log FROM item_progress WHERE user_id = ${userId()} AND programme_id = ${programmeId}`
    return rows.map((row) => {
  ...
  return scopedLocalRows('item_progress', programmeId)
}
...
export async function upsertItemProgress(courseId, item) {
  ...
  if (sql) {
    await sql`INSERT INTO item_progress (user_id, programme_id, item_id, course_id, mastery, mastery_updated_at, notes, priority, review_log, updated_at)
      VALUES (${userId()}, ${programmeId}, ${entry.itemId}, ${courseId}, ${entry.mastery ?? null}, ${iso(entry.masteryUpdatedAt)}, ${entry.notes ?? null}, ${entry.priority ?? null}, ${jsonb(entry.reviewLog || [])}::jsonb, now())
      ON CONFLICT (user_id, programme_id, item_id) DO UPDATE SET course_id = excluded.course_id, mastery = excluded.mastery, mastery_updated_at = excluded.mastery_updated_at,
        notes = excluded.notes, priority = excluded.priority, review_log = excluded.review_log, updated_at = now()`
    return entry
  }
  await scopedLocalUpsert('item_progress', programmeId, (row) => row.itemId, [entry])
  return entry
}
...
export async function listPersonalExercises(courseId, chapterId) {
  const programmeId = await activeProgrammeId()
  if (sql) {
    const rows = await sql`SELECT body FROM personal_exercises WHERE user_id = ${userId()} AND programme_id = ${programmeId} AND course_id = ${courseId} AND chapter_id = ${chapterId} ORDER BY created_at, id`
    return rows.map((row) => row.body)
  }
  return (await scopedLocalRows('personal_exercises', programmeId)).filter((row) => row.courseId === courseId && row.chapterId === chapterId).map((row) => row.body)
}
...
export async function addPersonalExercises(courseId, chapterId, questions) {
  ...
  if (sql) {
    for (const question of questions) {
      await sql`INSERT INTO personal_exercises (user_id, programme_id, id, course_id, chapter_id, type, difficulty, body, created_at)
        VALUES (${userId()}, ${programmeId}, ${question.id}, ${courseId}, ${chapterId}, ${question.type ?? null}, ${question.difficulty ?? null}, ${jsonb(question)}::jsonb, ${now}::timestamptz)
        ON CONFLICT (user_id, programme_id, id) DO UPDATE SET body = excluded.body, type = excluded.type, difficulty = excluded.difficulty`
    }
    return
  }
  await scopedLocalUpsert('personal_exercises', programmeId, (row) => row.id, questions.map((question) => ({ id: question.id, courseId, chapterId, body: question, createdAt: now })))
}

export async function deletePersonalExercise(courseId, chapterId, questionId) {
  const programmeId = await activeProgrammeId()
  if (sql) {
    const rows = await sql`DELETE FROM personal_exercises WHERE user_id = ${userId()} AND programme_id = ${programmeId} AND course_id = ${courseId} AND chapter_id = ${chapterId} AND id = ${questionId} RETURNING id`
    return rows.length > 0
  }
  return (await scopedLocalDelete('personal_exercises', programmeId, (row) => row.id === questionId && row.courseId === courseId && row.chapterId === chapterId)) > 0
}
...
export async function listFlashcardRows() {
  const programmeId = await activeProgrammeId()
  if (sql) {
    const rows = await sql`SELECT * FROM flashcards WHERE user_id = ${userId()} AND programme_id = ${programmeId} ORDER BY created_at, id`
    return rows.map(cardFromRow)
  }
  return scopedLocalRows('flashcards', programmeId)
}
...
export async function upsertFlashcards(cards) {
  ...
  if (sql) {
    for (const card of cards) {
      ...
      await sql`INSERT INTO flashcards (user_id, programme_id, id, course_id, chapter_id, front, back, source, ease, interval_days, repetitions, last_reviewed, due_at, history, extra, created_at, updated_at)
        VALUES (${userId()}, ${programmeId}, ${card.id}, ${card.courseId || ''}, ${card.chapterId ?? null}, ${card.front || ''}, ${card.back || ''}, ${card.source ?? null},
        ...
        ON CONFLICT (user_id, programme_id, id) DO UPDATE SET course_id = excluded.course_id, chapter_id = excluded.chapter_id, front = excluded.front, back = excluded.back, source = excluded.source,
  ...
  await scopedLocalUpsert('flashcards', programmeId, (row) => row.id, cards)
}
...
export async function deleteFlashcards(ids) {
  ...
  if (sql) {
    const rows = await sql`DELETE FROM flashcards WHERE user_id = ${userId()} AND programme_id = ${programmeId} AND id = ANY(${ids}) RETURNING id`
    return rows.length
  }
  const set = new Set(ids)
  return scopedLocalDelete('flashcards', programmeId, (row) => set.has(row.id))
}
...
export async function listSrCards() {
  const programmeId = await activeProgrammeId()
  if (sql) {
    const rows = await sql`SELECT question_id, ease, interval_days, repetitions, last_reviewed, due_at, history FROM sr_cards WHERE user_id = ${userId()} AND programme_id = ${programmeId}`
  ...
  return Object.fromEntries((await scopedLocalRows('sr_cards', programmeId)).map((row) => [row.questionId, row.card]))
}
...
export async function upsertSrCards(entries) {
  ...
  if (sql) {
    for (const [questionId, card] of entries) {
      await sql`INSERT INTO sr_cards (user_id, programme_id, question_id, ease, interval_days, repetitions, last_reviewed, due_at, history, updated_at)
        VALUES (${userId()}, ${programmeId}, ${questionId}, ${num(card.ease, 2.5)}, ${num(card.interval, 0)}, ${Math.trunc(num(card.repetitions, 0))}, ${iso(card.lastReviewed)}, ${iso(card.dueAt)}, ${jsonb(card.history || [])}::jsonb, now())
        ON CONFLICT (user_id, programme_id, question_id) DO UPDATE SET ease = excluded.ease, interval_days = excluded.interval_days, repetitions = excluded.repetitions,
          last_reviewed = excluded.last_reviewed, due_at = excluded.due_at, history = excluded.history, updated_at = now()`
    }
    return
  }
  await scopedLocalUpsert('sr_cards', programmeId, (row) => row.questionId, entries.map(([questionId, card]) => ({ questionId, card })))
}
...
export async function deleteSrCards(ids) {
  ...
  if (!ids.length) return 0
  if (sql) {
    const rows = await sql`DELETE FROM sr_cards WHERE user_id = ${userId()} AND programme_id = ${programmeId} AND question_id = ANY(${ids}) RETURNING question_id`
    return rows.length
  }
  const set = new Set(ids)
  return scopedLocalDelete('sr_cards', programmeId, (row) => set.has(row.questionId))
}
...
export async function listMistakes(filter = {}) {
  ...
  let all
  if (sql) {
    const rows = await sql`SELECT * FROM mistakes WHERE user_id = ${userId()} AND programme_id = ${programmeId} ORDER BY created_at DESC`
    all = rows.map(mistakeFromRow)
  } else {
    all = (await scopedLocalRows('mistakes', programmeId)).slice().sort((a, b) => String(b.createdAt).localeCompare(String(a.createdAt)))
  }
  return all.filter((m) => {
...
export async function insertMistake(record) {
  ...
  if (sql) {
    await sql`INSERT INTO mistakes (user_id, programme_id, id, course_id, chapter_id, question_id, type, difficulty, question, options, expected, source, attempt, correction, score, created_at, resolved_at)
      VALUES (${userId()}, ${programmeId}, ${record.id}, ${record.courseId}, ${record.chapterId ?? null}, ${record.questionId ?? null}, ${record.type ?? null}, ${record.difficulty ?? null},
      ...
      ON CONFLICT (user_id, programme_id, id) DO UPDATE SET correction = excluded.correction, score = excluded.score, attempt = excluded.attempt, resolved_at = excluded.resolved_at`
    return record
  }
  await scopedLocalUpsert('mistakes', programmeId, (row) => row.id, [record])
  return record
}
...
export async function updateMistake(id, patch) {
  ...
  if (sql) {
    const rows = await sql`UPDATE mistakes SET resolved_at = CASE WHEN ${'resolvedAt' in patch} THEN ${iso(patch.resolvedAt)}::timestamptz ELSE resolved_at END
      WHERE user_id = ${userId()} AND programme_id = ${programmeId} AND id = ${id} RETURNING *`
    return rows[0] ? mistakeFromRow(rows[0]) : null
  }
  const rows = await localRows('mistakes')
  const found = rows.find((row) => (row.programmeId || programmeId) === programmeId && row.id === id)
...
export async function deleteMistakesWhere({ id, courseId, chapterId, questionId, open } = {}) {
  const programmeId = await activeProgrammeId()
  if (sql) {
    const rows = await sql`DELETE FROM mistakes WHERE user_id = ${userId()} AND programme_id = ${programmeId}
  ...
  return scopedLocalDelete('mistakes', programmeId, (row) => (id == null || row.id === id) && (courseId == null || row.courseId === courseId)
...
export async function listMockSessions() {
  ...
  if (sql) {
    const rows = await sql`SELECT s.id, s.course_id, s.chapter_id, s.submitted_at, s.total_score, s.total_max, s.duration_seconds,
        (SELECT count(*) FROM mock_session_answers a WHERE a.user_id = s.user_id AND a.programme_id = s.programme_id AND a.session_id = s.id)::int AS count
      FROM mock_sessions s WHERE s.user_id = ${userId()} AND s.programme_id = ${programmeId} ORDER BY s.submitted_at DESC NULLS LAST`
  ...
  return (await scopedLocalRows('mock_sessions', programmeId))
...
export async function getMockSession(id) {
  const programmeId = await activeProgrammeId()
  if (sql) {
    const [session] = await sql`SELECT * FROM mock_sessions WHERE user_id = ${userId()} AND programme_id = ${programmeId} AND id = ${id}`
    if (!session) return null
    const answers = await sql`SELECT * FROM mock_session_answers WHERE user_id = ${userId()} AND programme_id = ${programmeId} AND session_id = ${id} ORDER BY position`
    return {
  ...
  return (await scopedLocalRows('mock_sessions', programmeId)).find((s) => s.id === id) || null
}

export async function saveMockSession(session) {
  const programmeId = await activeProgrammeId()
  if (sql) {
    await sql`INSERT INTO mock_sessions (user_id, programme_id, id, course_id, chapter_id, started_at, submitted_at, duration_seconds, total_score, total_max)
      VALUES (${userId()}, ${programmeId}, ${session.id}, ${session.courseId || ''}, ${session.chapterId ?? null}, ${iso(session.startedAt)}, ${iso(session.submittedAt)}, ${session.duration == null ? null : Math.trunc(num(session.duration, 0))}, ${num(session.totalScore)}, ${num(session.totalMax)})
      ON CONFLICT (user_id, programme_id, id) DO UPDATE SET submitted_at = excluded.submitted_at, duration_seconds = excluded.duration_seconds, total_score = excluded.total_score, total_max = excluded.total_max`
    await sql`DELETE FROM mock_session_answers WHERE user_id = ${userId()} AND programme_id = ${programmeId} AND session_id = ${session.id}`
    const questions = Array.isArray(session.questions) ? session.questions : []
    for (const [position, q] of questions.entries()) {
      const { attempt, attemptImages, correction, score, ...question } = q
      await sql`INSERT INTO mock_session_answers (user_id, programme_id, session_id, position, question_id, question, attempt, attempt_images, correction, score)
        VALUES (${userId()}, ${programmeId}, ${session.id}, ${position}, ${q.id ?? null}, ${jsonb(question)}::jsonb, ${attempt == null ? null : String(attempt)}, ${attemptImages ? jsonb(attemptImages) : null}::jsonb, ${correction ?? null}, ${num(score)})`
    }
    return session
  }
  await scopedLocalUpsert('mock_sessions', programmeId, (row) => row.id, [session])
  return session
}
...
export async function deleteMockSessionsWhere({ courseId } = {}) {
  const programmeId = await activeProgrammeId()
  if (sql) {
    const rows = await sql`DELETE FROM mock_sessions WHERE user_id = ${userId()} AND programme_id = ${programmeId} AND (${courseId ?? null}::text IS NULL OR course_id = ${courseId ?? null}) RETURNING id`
    return rows.length
  }
  return scopedLocalDelete('mock_sessions', programmeId, (row) => courseId == null || row.courseId === courseId)
}
...
export async function getBrowserState() {
  const programmeId = await activeProgrammeId()
  if (sql) {
    const rows = await sql`SELECT key, value FROM browser_state WHERE user_id = ${userId()} AND programme_id = ${programmeId}`
    return Object.fromEntries(rows.map((row) => [row.key, row.value]))
  }
  return Object.fromEntries((await scopedLocalRows('browser_state', programmeId)).map((row) => [row.key, row.value]))
}
...
export async function putBrowserState(value) {
  ...
  if (sql) {
    for (const [key, text] of changed) {
      await sql`INSERT INTO browser_state (user_id, programme_id, key, value, updated_at) VALUES (${userId()}, ${programmeId}, ${key}, ${text}, now())
        ON CONFLICT (user_id, programme_id, key) DO UPDATE SET value = excluded.value, updated_at = now()`
    }
    if (removed.length) await sql`DELETE FROM browser_state WHERE user_id = ${userId()} AND programme_id = ${programmeId} AND key = ANY(${removed})`
    return next
  }
  const all = await localRows('browser_state')
  await saveLocalRows('browser_state', [...all.filter((row) => (row.programmeId || programmeId) !== programmeId), ...Object.entries(next).map(([key, text]) => ({ key, value: text, programmeId }))])
  return next
}
...
export async function exportStudyTables() {
  ...
  return {
    ...
    programmeId,
    personalExercises: sql ? (await sql`SELECT body FROM personal_exercises WHERE user_id = ${userId()} AND programme_id = ${programmeId} ORDER BY created_at`).map((row) => row.body) : (await scopedLocalRows('personal_exercises', programmeId)).map((row) => row.body),
    flashcards,
