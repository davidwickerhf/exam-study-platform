...
export function renderingPreflight(chapter) {
  ...
  const visit=(value,path)=>{
    ...
    else if(value && typeof value==='object')for(const [key,item] of Object.entries(value))if(!['factualAudit','evidenceReview','pedagogyAudit','pedagogicalReview','reviewBaseline','reviewFocus'].includes(key))visit(item,`${path}.${key}`)
  }
...
// Deterministic repair for transport-corrupted invisible characters. Hosted
// structured output has occasionally emitted a truncated Unicode escape as
// U+0002 plus the remaining suffix (for example "re\u000209ranking"). The
// local context distinguishes the observed safe reconstructions: a spaced
// "12" separator is an arrow in a pipeline; an embedded suffix is a hyphen.
// Other invisible format/control characters carry no visible meaning and are
// removed. Every changed field remains inspectable in renderRepairs.
const INVISIBLE_CONTROL=/[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f\u200b\u202a-\u202e\u2066-\u2069\ufeff]/gu
function repairRenderingString(value){
  return value
    .replace(/\u000212(?=\s)/gu,'→')
    .replace(/\u0002(?:09|02)/gu,'-')
    .replace(/\t(?=heta\b)/gu,'θ')
...
export function repairRenderingControls(chapter){
  const repairs=[]
  const visit=(value,path)=>{
    if(typeof value==='string'){
      const repaired=repairRenderingString(value)
      if(repaired!==value)repairs.push({path,reason:'invisible-control'})
      return repaired
    }
    if(Array.isArray(value))return value.map((item,index)=>visit(item,`${path}[${index}]`))
    if(value && typeof value==='object')return Object.fromEntries(Object.entries(value).map(([key,item])=>[key,['factualAudit','evidenceReview','pedagogyAudit','pedagogicalReview','reviewBaseline','reviewFocus'].includes(key)?item:visit(item,`${path}.${key}`)]))
    return value
  }
  const repaired=visit(chapter,'chapter')
  return repairs.length ? {...repaired,renderRepairs:[...(chapter.renderRepairs || []),...repairs]} : chapter
}
...
// Schema vocabulary a plan or chapter generation can leak into free prose in
// place of a sentence (observed: seven of nine gaps entries in one saved plan
// were bare field names such as prerequisites or teachingApproach, plus a
// snake_case placeholder). An entry that is only such a token, contains a
// snake_case token, names a camelCase schema field, or is placeholder text is
// scaffolding, not a gap statement.
const SCHEMA_FIELD_TOKENS = new Set(['sourceIds', 'objectiveIds', 'teachingApproach', 'practiceStage', 'followUpKey', 'objectiveCoverage', 'explanationSectionIds', 'workedExampleSectionIds', 'guidedQuestionKeys', 'independentQuestionKeys', 'transferQuestionKeys', 'learningGoals', 'correctOptions', 'teachingPlan', 'answerSourceIds', 'changedCondition', 'formatVersion'])
const SNAKE_CASE_TOKEN = /\b[A-Za-z][A-Za-z0-9]*(?:_[A-Za-z0-9]+)+\b/
// A scope patch can preserve a generated citation label after its source-id
// slots have collapsed (observed: `Sources: and.`). Keep any real sentence
// before the suffix, but never send the empty citation scaffold to review.
const EMPTY_SOURCE_TAIL = /\s*(?:sources?|evidence(?: ids?)?):\s*(?:(?:and|or)\s*)?[.,;:]?\s*$/i
// A sentence that merely mentions a placeholder (say, placeholder images in
// a lab pack) is prose; a short entry that is itself a placeholder is not.
const PLACEHOLDER_TEXT = /\blorem ipsum\b|\b(?:TODO|TBD|FIXME)\b|^(?:n\/a|none|null|undefined|string|\.\.\.)\.?$/i
const placeholderEntry = text => PLACEHOLDER_TEXT.test(text) || (/\bplaceholders?\b/i.test(text) && (text.split(/\s+/).length < 8 || /\bschema\b|\bcompliance\b/i.test(text)))
export const SCAFFOLDING_REASONS = Object.freeze(['bare identifier', 'schema token', 'schema field name', 'placeholder text'])
...
export function incoherentProseReason(entry) {
  ...
  if (EMPTY_SOURCE_TAIL.test(text)) return 'empty source placeholder'
  ...
  if (!/\s/.test(text)) return 'bare identifier'
  if (placeholderEntry(text)) return 'placeholder text'
  if (SNAKE_CASE_TOKEN.test(text)) return 'schema token'
  if (text.split(/[^A-Za-z]+/).some(word => SCHEMA_FIELD_TOKENS.has(word))) return 'schema field name'
  return null
}
...
function repairProseEntry(entry) {
  ...
  text = tidyProse(dropUnmatchedQuotes(dropUnmatchedBrackets(text.replace(JSON_OPENER_START, '').replace(EMPTY_SOURCE_TAIL, ''))))
...
function repairProseList(list, field, repairs) {
  ...
  for (const [index, entry] of list.entries()) {
    ...
    changed = true
    // Leaked schema vocabulary is never repaired into a sentence: there is no
    // prose to recover, so the entry is dropped and the drop recorded.
    const repaired = SCAFFOLDING_REASONS.includes(reason) ? null : repairProseEntry(entry)
    if (repaired) {
...
const CORRUPT_PROSE_COMPLAINT = /malformed JSON|invalid JSON|JSON fragment|corrupt|stray (?:JSON|text|characters|punctuation)|truncat|unbalanced|(?:incomplete|empty|unpopulated) (?:source )?placeholder|not (?:coherent|valid) (?:prose|text)/i
