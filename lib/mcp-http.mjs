...
export function createRemoteMcpServer({api,auth}) {
...
export async function handleMcpRequest(req,res,{api,auth,store,body}) {
  ...
  for(const [period,windowMs,limit]of [['minute',60000,MCP_LIMITS.tokenUnitsPerMinute],['day',86400000,MCP_LIMITS.tokenUnitsPerDay]]){
    for(const id of identities){
      ...
      if(!await store.charge(bucket,cost,limit,windowMs*2)){
  ...
  const server=createRemoteMcpServer({api,auth})
