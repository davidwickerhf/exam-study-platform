...
function stateFor(context,chapter) {
  ...
  return {fingerprint:hash,dependencies,reviews:Object.fromEntries(Object.entries(dependencies).filter(([id,hash])=>saved?.reviews[id]&&(legacy||saved.dependencies?.[id]===hash || saved.dependencies?.[id]===legacyDependencyHash(values[id]))).map(([id])=>[id,structuredClone(saved.reviews[id])]))}
}
