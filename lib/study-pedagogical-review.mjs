...
const fingerprint = chapter => digest({sections:chapter.sections,questions:chapter.questions,teachingPlan:chapter.teachingPlan,objectiveCoverage:chapter.objectiveCoverage})
...
export function acceptPedagogicalReview(chapter,step,raw) {
  ...
  const combine=(field,positive)=>{
    const rows=new Map()
    for(const review of reviews)for(const row of review[field] || [])if(!rows.has(row.questionKey) || !row[positive])rows.set(row.questionKey,row)
    return [...rows.values()]
  }
  return {objectives:reviews.flatMap(r=>r.objectives),issues:reviews.flatMap(r=>r.issues),transferChecks:combine('transferChecks','requiresNewReasoning'),followUpChecks:combine('followUpChecks','useful')}
}
