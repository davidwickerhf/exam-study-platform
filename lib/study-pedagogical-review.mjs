import { dependencyHash, boundedReviewItems } from './study-review-dependencies.mjs'
...
const fingerprint = chapter => digest({reviewVersion:6,sections:chapter.sections,questions:chapter.questions,teachingPlan:chapter.teachingPlan,objectiveCoverage:chapter.objectiveCoverage})
function objectiveSlice(chapter, objectives) {
  const ids=new Set(objectives.map(o=>o.id))
  return {...chapter,teachingPlan:{...chapter.teachingPlan,objectives},objectiveCoverage:chapter.objectiveCoverage.filter(item=>ids.has(item.objectiveId)),questions:chapter.questions.filter(q=>q.objectiveIds.some(id=>ids.has(id)))}
}
function stateFor(context,chapter) {
  const hash=fingerprint(chapter), saved=chapter.pedagogyAudit
  const dependencies=Object.fromEntries(chapter.teachingPlan.objectives.map(o=>{
    const slice=objectiveSlice(chapter,[o])
    const related=new Set(slice.questions.flatMap(q=>(q.misconceptions||[]).map(m=>m.followUpKey)))
    return [o.id,dependencyHash({context,rules:[pedagogyPrompt.toString(),pedagogyReviewIssues.toString()],schema:pedagogicalResponseSchema(slice),sections:slice.sections,questions:slice.questions,related:chapter.questions.filter(q=>related.has(q.key)),objective:o,coverage:slice.objectiveCoverage})]
  }))
  const legacy=saved && !saved.dependencies && saved.fingerprint===hash
  return {fingerprint:hash,dependencies,reviews:Object.fromEntries(Object.entries(dependencies).filter(([id,hash])=>saved?.reviews[id]&&(legacy||saved.dependencies?.[id]===hash)).map(([id])=>[id,structuredClone(saved.reviews[id])]))}
}
...
export function nextPedagogicalReview(context, chapter) {
  ...
  const objectives=boundedReviewItems(chapter.teachingPlan.objectives.filter(item=>!state.reviews[item.id]),4,12000)
  if(!objectives.length){chapter.pedagogyAudit=state;return null}
  const slice=objectiveSlice(chapter,objectives)
  ...
  return {objectiveId:objectives[0].id,objectiveIds:objectives.map(o=>o.id),state,quoteReferences,chapter:slice,schema:pedagogyReviewSchema,responseSchema,tokens:generationLimits.pedagogicalReviewTokens,prompt:pedagogyPrompt(context+'\nReview all selected objectives and questions. Other sections and relatedQuestions provide prerequisite and linked-follow-up context. A follow-up listed in relatedQuestions exists; do not flag it as missing merely because it belongs to another objective.',slice,{quoteReferences})}
}
export function acceptPedagogicalReview(chapter,step,raw) {
  ...
  for(const id of ids) {
    ...
    state.reviews[id]={...review,objectives:review.objectives.filter(o=>o.objectiveId===id),transferChecks:review.transferChecks.filter(row=>keys.has(row.questionKey)),followUpChecks:review.followUpChecks.filter(row=>keys.has(row.questionKey))}
  }
