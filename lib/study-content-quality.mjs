...
import { objectiveCoverageIssues } from './study-pedagogy.mjs'
...
export function studyLessonQuality(chapter, evidence = []) {
  ...
  visit(chapter)
  const hasNullPadding=value=>typeof value==='string' ? value.includes('\u0000') : Array.isArray(value) ? value.some(hasNullPadding) : value && typeof value==='object' ? Object.entries(value).some(([key,item])=>!['factualAudit','evidenceReview','pedagogyAudit','pedagogicalReview','reviewBaseline','reviewFocus'].includes(key) && hasNullPadding(item)) : false
  if(hasNullPadding(chapter))issues.push('The lesson contains null-character corruption. Reconstruct affected text and formulas from the evidence; deleting invisible characters alone may leave missing operators or terms.')
  for (const q of chapter.questions || []) {
    issues.push(...intersectionRangeIssues(q))
    const choice = ['mc', 'multi', 'tf'].includes(q.type)
    const options = q.options || [], correct = q.correctOptions || []
    if (choice && (options.length < 2 || options.some(o => !o.trim() || /^string\d*$/i.test(o)) || new Set(options).size !== options.length || !correct.length || new Set(correct).size !== correct.length || correct.some(i => !Number.isInteger(i) || i < 0 || i >= options.length) || (q.type !== 'multi' && correct.length !== 1))) issues.push('Choice questions need distinct usable options and a valid answer key matching their question type.')
    if (q.type === 'tf' && JSON.stringify(options) !== '["True","False"]') issues.push('True/false questions must use the options True and False in that order.')
    if (!choice && (options.length || correct.length)) issues.push('Written, calculation and pseudocode problems must not carry placeholder choices.')
  }
  if ([2, 3].includes(chapter.formatVersion)) {
    const words = value => String(value || '').trim().split(/\s+/).filter(Boolean).length
    if (chapter.sections.some(s => !s.takeaway)) issues.push('Give each explanation a clear takeaway.')
    issues.push(...objectiveCoverageIssues(chapter))
    ...
    if (chapter.questions.length < 8 || chapter.questions.filter(q => q.kind !== 'recall').length < 4
      ...
      || new Set(chapter.questions.map(q => q.skill)).size < 3) issues.push('Practice needs varied skills, progressive challenge, useful hints and reasoned solutions covering the learning goals.')
    ...
    for (const q of chapter.questions) if (!q.hint || !q.objective || (chapter.formatVersion !== 3 && words(q.answer) < 20)) issues.push(`${q.key}: Practice needs a useful first hint, a stated objective and a reasoned solution.`)
    ...
    const offending = (matches, message) => {
      ...
      for (const key of [...keys, ...cards]) issues.push(`${key}: ${message}`)
    }
    const administrative = /exam rules?|exam duration|current exam is|how many ects|(?:lecturer|professor).{0,20}name/i
    offending(text => administrative.test(text), 'Practice and flashcards must test academic concepts, not course administration or exam-policy trivia.')
    ...
    offending(text => sourceRecall.test(text), 'Ask about the academic concept directly, not what a slide, lecture or chapter says, lists or mentions. Replace source-wording recall with understanding, application or a useful definition.')
  ...
  if (
    chapter.formatVersion !== 3 && (chapter.questions || []).some(
