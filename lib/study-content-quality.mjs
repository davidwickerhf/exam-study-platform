...
export function studyLessonQuality(chapter, evidence = []) {
  ...
  visit(chapter)
  const hasNullPadding=value=>typeof value==='string' ? value.includes('\u0000') : Array.isArray(value) ? value.some(hasNullPadding) : value && typeof value==='object' ? Object.entries(value).some(([key,item])=>!['factualAudit','evidenceReview','pedagogyAudit','pedagogicalReview'].includes(key) && hasNullPadding(item)) : false
  ...
  if ([2, 3].includes(chapter.formatVersion)) {
    ...
    if (chapter.questions.length < 8 || chapter.questions.filter(q => q.kind !== 'recall').length < 4
      ...
      || chapter.questions.some(q => !q.hint || !q.objective || (chapter.formatVersion !== 3 && words(q.answer) < 20))) issues.push('Practice needs varied skills, progressive challenge, useful hints and reasoned solutions covering the learning goals.')
