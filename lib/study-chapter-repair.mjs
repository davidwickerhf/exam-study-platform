...
export function repairScopeDecision(chapter, issues, step) {
  ...
  if (step) return {path: 'patch', repair: repairPhase(step), findings: errors.length, targets: [...new Set([
    ...errors.map(issue => issue.itemKey).filter(Boolean),
    ...(step.keys || []).map(key=>`question:${key}`),
    ...(step.sectionIds || []).map(id=>`section:${id}`),
    ...(step.planObjectiveIds || []).map(id=>`objective:${id}`),
    ...(step.cardIndexes || []).map(index=>`cards:${Math.floor(index/4)}`),
...
export function questionRepairStep(course,sources,evidence,chapter,issues,{expandObjectives=true}={}) {
  ...
  // Warnings remain recorded but never enlarge a mandatory correction. A
  // chapter with one blocking question and several advisory section/scope
  // notes must still receive the one bounded question patch; promoting those
  // warnings here previously exceeded the patch partition and bought a whole
  // rewrite. The independent re-review may report any still-relevant warning.
...
export function questionRepairSteps(course,sources,evidence,chapter,issues,{maxPatches=maxRepairPatches()}={}) {
  ...
  // A diagnostic finding often names both the faulty question and its linked
  // remediation target, so it cannot honestly receive one itemKey. Exact IDs
  // are still a safe bounded location: singleRepairStep deliberately selects
  // both questions. Only genuinely unlocated findings require a full rewrite.
  if(errors.some(issue=>issue.scope==='chapter' || (!issue.itemKey && !questionKeysForIssue(chapter,issue).length) || isFillableFinding(issue) || /Missing related practice/i.test(issue.detail)))return []
