...
export function applyQuestionRepair(chapter,step,raw) {
  ...
  if(step.scope){
    ...
    next.teachingPlan={...next.teachingPlan,...scope,objectives:next.teachingPlan.objectives.map(objective=>objectives[objective.id])}
    return next
  }
  if(step.cardIndexes) {
...
function scopeRepairStep(course,sources,evidence,chapter,issues){
  ...
  const objectives=z.object(Object.fromEntries(chapter.teachingPlan.objectives.map(objective=>[objective.id,teachingPlanSchema.shape.objectives.element.extend({id:z.literal(objective.id),complexity:z.enum(objective.complexity==='simple'?['simple','difficult']:['difficult'])})]))).strict()
