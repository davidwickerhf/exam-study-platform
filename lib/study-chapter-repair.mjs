...
// copy this text out of the chapter.
const QUOTED = /"([^"]{16,400})"|“([^”]{16,400})”/g
...
export function locateReviewIssues(chapter, issues) {
  return issues.map(issue => {
    ...
    const named = [...new Set(String(issue.detail || '').split(/[^A-Za-z0-9_-]+/).filter(word => IDENTIFIER.test(word)).flatMap(id => itemKeyFor(chapter, id)))]
