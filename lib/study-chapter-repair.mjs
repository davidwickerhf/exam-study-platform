...
export function questionRepairStep(course,sources,evidence,chapter,issues) {
  const direct=singleRepairStep(course,sources,evidence,chapter,issues)
  if(direct)return direct
  const groups={questions:[],sections:[],cards:[]}
  for(const issue of issues.filter(i=>i.severity==='error')){
    if(chapter.questions.some(q=>issue.itemKey===`question:${q.key}` || issue.detail.startsWith(q.key+':')))groups.questions.push(issue)
  ...
  const parts=selected.map(rows=>singleRepairStep(course,sources,evidence,chapter,rows))
