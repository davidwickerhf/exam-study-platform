...
export class CanvasConnectionError extends Error {
  constructor(message, code = 'CANVAS_CONNECTION_INVALID') {
    super(message)
    this.name = 'CanvasConnectionError'
    this.code = code
    // Waiting does not repair missing, malformed or differently encrypted credentials.
    this.retryable = false
  }
}
export const CANVAS_DECRYPTION_ERROR = 'This server cannot decrypt the saved Canvas connection. Check that the app and sync worker use the same encryption key. Reconnect only if the original key cannot be restored.'
...
function decrypt(value) {
  ...
  if (version !== VERSION || !ivEncoded || !tagEncoded || !dataEncoded || extra.length) throw new CanvasConnectionError(CANVAS_DECRYPTION_ERROR, 'CANVAS_CONNECTION_UNREADABLE')
  try {
    ...
    throw new CanvasConnectionError(CANVAS_DECRYPTION_ERROR, 'CANVAS_CONNECTION_UNREADABLE')
  }
}
