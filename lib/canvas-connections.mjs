...
async function rowFor(origin) {
  if (sql) {
    const rows = await sql`SELECT origin, encrypted_token, created_at, updated_at, last_used_at FROM canvas_connections WHERE user_id = ${userId()} AND origin = ${origin} LIMIT 1`
    return rows[0] || null
  }
  return (await localRows(TABLE)).find((row) => row.userId === userId() && row.origin === origin) || null
}
