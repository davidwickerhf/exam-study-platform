...
const TEACHING_KIND = /\b(lecture|tutorial|practical|lab(?:oratory)?|workshop|seminar|debate|class|colloquium|exercise(?: class)?)s?\b/gi
...
export function attendancePolicyForEvent(event, course) {
  ...
  for (const policy of confirmedPolicies(course)) {
    ...
    const required = policy.participationAssessed ? null : requirementFor(policy.text, event.activity || eventText)
