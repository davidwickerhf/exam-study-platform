...
export function attendancePolicyForEvent(event, course) {
  ...
  for (const policy of confirmedPolicies(course)) {
    ...
    const named = teachingKinds(policy.activity).length ? teachingKinds(policy.activity) : teachingKinds(policy.text)
    if (!named.length || !named.some((kind) => eventKinds.has(kind))) continue
    const required = requirementFor(policy.text, event.activity || eventText)
    if (required !== null) matches.push({ ...policy, required, source: 'Verified course rule' })
  }
  return matches.length && new Set(matches.map(p => p.required)).size === 1 ? matches[0] : null
}
...
export function attendanceOverview(events = [], records = [], courses = [], { now = Date.now() } = {}) {
  ...
  const courseRows = [...byCourse.values()].map((course) => {
    ...
    return {
      ...course,
      unknownRequirementSessions: eligible.filter(event => event.courseCode === course.courseCode && event.attendanceRequired == null).length,
      unmatchedRules: confirmedPolicies(courseByCode.get(String(course.courseCode || '').toUpperCase())).filter(policy =>
        requirementFor(policy.text, policy.activity) === true &&
      ...
      ).map(policy => ({ text: policy.text, activity: policy.activity, source: 'Verified course rule' })),
