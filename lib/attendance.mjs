...
export function attendancePolicyForEvent(event, course, {sessions=[]} = {}) {
  ...
  if(new Set(mappings.map(mapping=>mapping.activity)).size>1) return null
  const mapping=mappings[0]
  if(mapping) eventKinds=new Set([mapping.activity])
  const matches = []
  for (const policy of confirmedPolicies(course)) {
    ...
    const required = policy.participationAssessed ? null : policy.requirement === 'required' ? true : policy.requirement === 'optional' ? false : requirementFor(policy.text, mapping?.activity || activity)
    if (required !== null || policy.participationAssessed) matches.push({ ...policy, required, evidence:[...policy.evidence,...(mapping?.evidence || [])], source: 'Verified course rule' })
  }
  ...
  if(!applicable.length) return mapping ? null : directPolicy
