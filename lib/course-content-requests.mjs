...
export async function createCourseContentRequest(payload, { requesterEmail = null } = {}) {
  ...
  if (sql) {
    ...
    if (existing) {
      ...
      await sql`UPDATE course_content_requests SET requester_email=${base.requester_email}, programme_id=${base.programme_id}, course_code=${base.course_code}, course_name=${base.course_name}, academic_year=${base.academic_year}, period=${base.period}, categories=${JSON.stringify(mergedCategories)}::jsonb, urls=${JSON.stringify(mergedUrls)}::jsonb, notes=${mergedNotes}, updated_at=now() WHERE id=${requestId}`
    } else {
      requestId = randomUUID()
      await sql`INSERT INTO course_content_requests (id, user_id, requester_email, programme_id, academic_course_id, course_code, course_name, academic_year, period, categories, notes, urls)
        VALUES (${requestId}, ${owner}, ${base.requester_email}, ${base.programme_id}, ${base.academic_course_id}, ${base.course_code}, ${base.course_name}, ${base.academic_year}, ${base.period}, ${JSON.stringify(categories)}::jsonb, ${notes}, ${JSON.stringify(urls)}::jsonb)`
    }
    try {
  ...
  const created = !row
  if (!row) {
    row = { id: randomUUID(), user_id: owner, ...base, categories, notes, urls, status: 'submitted', pipeline_stage: 'collection', admin_note: '', created_at: now, updated_at: now }
    requests.push(row)
  } else {
