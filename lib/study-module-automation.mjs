import { reconcileMaintainedGuides } from './study-guide-maintenance.mjs'
...
import { resolveGuideBilling } from './study-ai-budget.mjs'
...
const settingsSchema = z.object({enabled:z.boolean(),execution:z.enum(['hosted','local']),rule:z.enum(['auto','weekly','topic','textbook']),historicalMode:z.enum(['none','suggest','supplement']).default('suggest'),
  ...
  billing:z.object({source:z.enum(['platform','personal']),quality:z.enum(['standard','enhanced','sol','astra']),maxJobUsd:z.number().min(0.05).max(generationLimits.maxJobUsd)})})
const defaults = {enabled:false,execution:'local',rule:'auto',historicalMode:'suggest',supportingSourceKeys:[],billing:{source:'platform',quality:'astra',maxJobUsd:generationLimits.defaultJobUsd}}
export async function moduleGuideStatus(course, {sourceOptions={}}={}) {
  ...
  return {settings:own?.settings || defaults, checkedAt:own?.checkedAt || null, error:own?.error || null, modules, maintainedGuides:own?.maintainedGuides || [],
...
export async function saveModuleGuideSettings(course, input, options={}) {
  ...
  if (!parsed.success) throw new StudyVersionError('Choose valid automatic guide settings and a per-revision budget ($0.05–$50).')
  ...
  if(settings.enabled && settings.execution==='hosted') await resolveGuideBilling(settings.billing,options.platform)
...
export async function reconcileModuleGuides(key, {sourceOptions={},platform={},now=Date.now()}={}) {
  ...
  if(!await recurringEnabled('guides') || (!row?.settings.enabled && !row?.maintainedGuides?.some(g=>g.enabled)) || row.programmeId!==await activeProgrammeId() || row.leaseUntil>now) return
  ...
  try {
    ...
    const maintenanceEvents=await reconcileMaintainedGuides(row,{sourceOptions,sources,inventories})
    const modules=[]
    let prepared=0
    ...
    for(const inventory of row.settings.enabled ? inventories : []) {
      ...
      for(const module of inventory.modules) {
        // An enrolled manual guide owns these module boundaries. Do not make
        // duplicate automatic guides when course-wide generation is enabled.
        if(row.maintainedGuides?.some(g=>g.enabled&&g.moduleRefs.some(r=>r.bindingId===inventory.bindingId&&r.moduleId===String(module.id))))continue
        ...
        if(candidate.status==='candidate' && (!prior?.versionId || prior.appliedSignature!==candidate.signature)) {
          ...
          try {
            ...
            const billing=row.settings.execution==='local'?{source:'local',model:'local-agent',provider:'local'}:await resolveGuideBilling(row.settings.billing,platform)
...
export async function scheduleModuleGuides(options={}) {
  ...
  const rows=sql?await sql`SELECT d.user_id AS owner,d.document_key AS key,d.value FROM user_documents d
    WHERE namespace=${MODULE_SETTINGS} AND (value->'settings'->>'enabled'='true' OR EXISTS(SELECT 1 FROM jsonb_array_elements(COALESCE(value->'maintainedGuides','[]'::jsonb)) g WHERE g->>'enabled'='true'))
