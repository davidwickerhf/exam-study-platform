...
const settingsSchema = z.object({enabled:z.boolean(),execution:z.enum(['hosted','local']),rule:z.enum(['auto','weekly','topic','textbook']),historicalMode:z.enum(['none','suggest','supplement']).default('suggest'),
  ...
  billing:z.object({source:z.enum(['platform','personal']),quality:z.enum(['standard','enhanced']),maxJobUsd:z.number().min(0.05).max(10)})})
const defaults = {enabled:false,execution:'local',rule:'auto',historicalMode:'suggest',supportingSourceKeys:[],billing:{source:'platform',quality:'standard',maxJobUsd:1}}
