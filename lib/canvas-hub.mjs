...
import { cachedCanvasResponse, canvasCachePartition, invalidateCanvasSharedCache } from './canvas-shared-cache.mjs'
...
function cacheKey(parts) {
  return [currentUserId(), canvasCachePartition(), ...parts].join('|')
}
...
async function cached(key, produce, { force = false } = {}) {
  const previous = hubCache.get(key)
  if (previous?.pending) return previous.pending
  if (!force && previous && previous.at > Date.now() - HUB_TTL_MS) return previous.value
  const entry = { at: 0, pending: null }
  const pending = Promise.resolve().then(produce).then(value => {
    // Revoking/reconnecting during a read must not repopulate its old cache.
    if (hubCache.get(key) === entry) hubCache.set(key, { at: Date.now(), value })
    return value
  }, error => {
    if (hubCache.get(key) === entry) hubCache.delete(key)
    throw error
  })
  entry.pending = pending
  hubCache.set(key, entry); prune()
  return pending
}
...
export async function clearCanvasHubCache({ all = false } = {}) {
  ...
  await invalidateCanvasSharedCache()
}
...
export async function fetchCanvasHub(options = {}) {
  return cachedCanvasResponse({ ...options, parts: options.parts || CANVAS_HUB_PARTS }, () => buildCanvasHub(options))
}

async function buildCanvasHub({
  origin,
  token,
  scope = 'current',
  courseIds = [],
