...
//     conversation is not memory. External study checkpoints are separately labelled.
...
export const MAX_STUDY_SESSIONS = 80
...
export async function searchTutorHistory({ query = '', excludeConversationId = '', currentConversationId = '', limit = 6, userOnly = false } = {}) {
  ...
  if (sql) {
    ...
    matches = await sql`SELECT d.value->>'id' AS conversation_id, d.value->>'title' AS title,
      ...
      WHERE d.user_id=${currentUserId()} AND d.namespace=${NAMESPACE}
        ...
        AND m.message->>'role' IN ('user','assistant') AND (${userOnly}=false OR m.message->>'role'='user') AND CASE WHEN jsonb_typeof(m.message->'tool_calls')='array' THEN jsonb_array_length(m.message->'tool_calls') ELSE 0 END=0
        AND length(trim(m.message->>'content'))>0
        AND (${tsQuery}='' OR to_tsvector('simple',m.message->>'content') @@ to_tsquery('simple',${tsQuery}))
    ...
    }))).filter(item => !item.currentTurn && (!userOnly || item.role === 'user') && (!terms.length || item.score > 0) && ['user', 'assistant'].includes(item.role) && !item.toolCalls?.length && item.content.trim())
...
export async function readTutorMemory() {
  ...
  return {
    ...
    studySessions: (stored.studySessions || []).slice(0, MAX_STUDY_SESSIONS),
...
// Checkpoints are reported learning context, never authoritative course evidence
// or automatically scored practice. They share memory's account/programme scope,
// export and erasure lifecycle. Each request ID identifies one immutable checkpoint.
export const studySessionSchema = z.object({
  requestId: z.string().regex(/^[\w-]{8,100}$/),
  sessionId: z.string().trim().min(1).max(100),
  courseCode: z.string().trim().min(1).max(40).transform(value => value.toUpperCase()),
  academicYear: z.string().regex(/^(20\d{2}-20\d{2}|undated)$/),
  summary: z.string().trim().min(1).max(1200),
  topics: z.array(z.string().trim().min(1).max(160)).max(12),
  observations: z.array(z.object({
    topic: z.string().trim().min(1).max(160),
    detail: z.string().trim().min(1).max(500),
    basis: z.enum(['student_report', 'observed_answer', 'not_assessed']),
    studentAnswer: z.string().trim().max(800).optional()
  }).strict().refine(item => item.basis !== 'observed_answer' || !!item.studentAnswer,
    'An observed answer needs the actual student response.')).max(8),
  nextSteps: z.array(z.string().trim().min(1).max(300)).max(8),
  confirmed: z.literal(true)
}).strict()

export async function saveStudySession(input) {
  const parsed = studySessionSchema.safeParse(input)
  if (!parsed.success) throw new TutorStoreError(`Invalid study checkpoint: ${parsed.error.issues.map(i => i.message).join('; ')}`)
  const { confirmed, ...payload } = parsed.data
  return updateMemory(memory => {
    const sessions = memory.studySessions || []
    const existing = sessions.find(entry => entry.requestId === payload.requestId)
    if (existing) {
      if (!isDeepStrictEqual(existing.payload, payload)) throw new TutorStoreError('This request ID already saved different context. Use a new request ID for a new checkpoint.', 409)
      return { result: { checkpoint: existing, duplicate: true } }
    }
    const checkpoint = { id: randomUUID(), origin: 'external_study_session', at: new Date().toISOString(), requestId: payload.requestId, payload }
    return { next: { ...memory, studySessions: [checkpoint, ...sessions].slice(0, MAX_STUDY_SESSIONS) }, result: { checkpoint, duplicate: false } }
  })
}

export async function readStudySessions({ courseCode = '', academicYear = '', limit = 20 } = {}) {
  const memory = await readTutorMemory()
  return memory.studySessions.filter(entry => (!courseCode || entry.payload.courseCode === courseCode.trim().toUpperCase())
    && (!academicYear || entry.payload.academicYear === academicYear)).slice(0, Math.min(80, Math.max(1, Number(limit) || 20)))
}

export async function forgetStudySession(id) {
  return updateMemory(memory => {
    const sessions = memory.studySessions || []
    const studySessions = sessions.filter(entry => entry.id !== id)
    return studySessions.length === sessions.length ? { result: false } : { next: { ...memory, studySessions }, result: true }
  })
}
