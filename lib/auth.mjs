...
export async function authenticate(req) {
  ...
  if (localAccount) {
    return { userId: localAccount.userId, mode: 'local-login', authenticated: true, admin: isAdminUser(localAccount.userId), email: localAccount.email, memberships: await membershipsFor(localAccount.userId), trusted: true }
  }
  ...
  return { userId, clerkUserId: auth.userId, sessionId: auth.sessionId, mode: 'clerk', authenticated: true, admin: isAdministrator(userId, identity.admin), email: identity.email, memberships, trusted: identity.trusted }
}
...
export function isPublicApi(pathname) {
  ...
  return pathname === '/api/health' || pathname === '/api/auth/config' || pathname === '/api/public/course-repository' || pathname === '/api/agent/authorize/exchange' || pathname.startsWith('/api/pdf/')
}
