...
export async function authenticateKeyIdentity(key) {
...
export async function authenticate(req) {
  ...
  if (bearer && bearer.startsWith(KEY_PREFIX)) {
    ...
    return authenticateKeyIdentity(key)
  }
...
export function authorise(auth, { method, pathname }) {
  ...
  const readPost = method === 'POST' && ['/api/retrieve', '/api/programme-policies/retrieve'].includes(pathname)
  if (method !== 'GET' && method !== 'HEAD' && !readPost && !auth.scopes.includes('write')) return 'This key is read-only.'
  return null
}
