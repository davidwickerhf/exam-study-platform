...
export async function exportPersonalData(identity = null) {
  ...
  return {
    ...
    studentStudy: Object.fromEntries(await Promise.all(['study-versions','study-revisions','study-notes','study-version-progress','study-version-exams','study-publications'].map(async namespace => [namespace, await listDocuments(namespace)]))),
