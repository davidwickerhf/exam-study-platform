...
import { deleteAllTutorData, exportTutorData, listConversations, readTutorMemory } from './tutor-store.mjs'
import { exportTutorAttachments, listTutorAttachments } from './tutor-attachments.mjs'
...
export async function summarisePersonalData() {
  ...
  const [tutorConversations, tutorMemory, tutorAttachments] = await Promise.all([listConversations(), readTutorMemory(), listTutorAttachments()])
  const namespaces = [
    ...study,
    ...
    { namespace: 'tutorAttachments', label: RECORD_LABELS.tutorAttachments, count: tutorAttachments.length, bytes: tutorAttachments.reduce((sum, item) => sum + Number(item.size || 0), 0), updatedAt: tutorAttachments[0]?.updatedAt || null, study: false,
      detail: tutorAttachments.length ? 'Files and pictures stored privately for Tutor retrieval. They are removed with account deletion or data erasure.' : null }
