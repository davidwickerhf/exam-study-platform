...
export async function exportPersonalData(identity = null) {
  ...
  return {
    ...
    studentStudy: Object.fromEntries(await Promise.all(['study-versions','study-revisions','study-notes','study-version-progress','study-version-exams','study-publications','study-quality-evaluations'].map(async namespace => [namespace, await listDocuments(namespace)]))),
