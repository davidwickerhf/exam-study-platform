...
export async function deletePersonalData() {
  ...
  if (sql) {
    ...
    return { documents: documents.length, tables, programmes, aiUsageEvents: aiUsage.length, activityEvents }
  }
  const target = localUserRoot(id)
  if (existsSync(target)) await rm(target, { recursive: true, force: true })
  return { documents: 0, tables, programmes, aiUsageEvents: 0, activityEvents }
}
