import { exportFeedback, eraseFeedback } from './feedback-store.mjs'
import { exportAgentActivity } from './agent-activity.mjs'
...
const ACCOUNT_DELETION_TABLES = Object.freeze([
  ...
  'academic_snapshots',
  'user_documents',
  'ai_usage_events', 'ai_call_events'
])
...
export async function summarisePersonalData() {
  ...
  const namespaces = [
    ...
    { namespace: 'academicSnapshots', label: RECORD_LABELS.academicSnapshots, count: academicSnapshots.length, bytes: null, updatedAt: academicSnapshots[0]?.createdAt || null, study: false,
      detail: academicSnapshots.length ? 'Course results and totals read from transcripts you uploaded, kept so progress can be compared over time. Originals saved through setup are retained privately for viewing and download and are removed by deleting uploaded data.' : null },
    { namespace: 'academicDocumentRegister', label: RECORD_LABELS.academicDocumentRegister,
      ...
      study: false, detail: academicDocumentRegister.length ? 'Names, fingerprints and approved changes retained for version history. Setup originals are stored separately and privately for viewing and download.' : null },
...
// Removes derived import metadata and retained setup/Tutor originals, including
// incomplete original uploads.
export async function deleteUploadedData() {
  ...
  const documents = await deleteNamespaces(['tutor-attachments', 'academic-document-register', 'onboarding-document-context', 'academic-originals', 'academic-original-chunks'])
...
export async function exportPersonalData(identity = null) {
  const id = userId()
  let aiUsage
  const aiCalls = sql ? (await sql`SELECT event FROM ai_call_events WHERE user_id=${id} ORDER BY created_at DESC`).map(row=>row.event) : (await listDocuments('ai-call-events')).map(row=>row.value)
  if (sql) {
  ...
  return {
    ...
    studentStudy: Object.fromEntries(await Promise.all(['study-versions','study-revisions','study-notes','study-version-progress','study-version-exams','study-publications','study-quality-evaluations','priority-evidence-cache'].map(async namespace => [namespace, await listDocuments(namespace)]))),
    academicOriginals: Object.fromEntries(await Promise.all(['academic-originals','academic-original-chunks'].map(async namespace => [namespace, await listDocuments(namespace)]))),
    tutorAttachments: await exportTutorAttachments(),
    activity: await readActivity({ since: '1970-01-01T00:00:00.000Z' }),
    agentActivity: await exportAgentActivity(),
    aiUsage, aiCalls
  }
}
...
export async function deletePersonalData() {
  ...
  try {
    ...
    await (await import('./mcp-service.mjs')).deleteRemoteMcpData(id)
    ...
    const academicSnapshots = await deleteAllAcademicSnapshots()
    if (sql) {
      const [documents, aiUsage, aiCalls] = await sql.transaction([
        ...
        sql`DELETE FROM ai_call_events WHERE user_id = ${id} RETURNING id`
      ])
      return { documents: documents.length, tables, programmes, courseContentRequests, editorialContributions, canvasConnections, academicSnapshots, tutor, agentAuthorizations, accessRecords, aiUsageEvents: aiUsage.length + aiCalls.length, activityEvents }
    }
