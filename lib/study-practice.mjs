...
export function localPaperRequest(record) {
  ...
  return {
    ...
    stage: record.stage,
    prompt: record.stage === 'review' ? practiceReviewPrompt(record) : setPrompt(record),
    responseSchema: studyResponseSchema(record.stage === 'review' ? reviewSchema : extractedSetSchema, record.snapshot.chunks.map(c => c.id)),
...
export async function submitLocalPaper(versionId, id, body, sourceOptions = {}) {
  ...
  if (record.stage === 'generate') {
    next.result = validatePracticeSet(parseStudyJson(response, extractedSetSchema), record)
    next.stage = 'review'
  } else {
    const review = parseStudyJson(response, reviewSchema)
    ...
    if (review.issues.length) {
      ...
      next.result = validatePracticeSet(record.result, record)
