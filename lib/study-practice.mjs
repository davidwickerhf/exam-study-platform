...
export function validatePracticeSet(raw, record) {
  const set = (record.mode === 'extract' ? extractedSetSchema : practiceSetSchema).parse(raw),
...
export async function createStudyPractice(versionId, body, options = {}) {
  ...
  if (options.execution === 'local') cacheParts.push('local-paper-v1')
...
export function localPaperRequest(record) {
  ...
  return {
    ...
    evidenceManifest: { hash: digest(record.snapshot), sources: record.snapshot.sources, chunks: record.snapshot.chunks },
    stage: record.stage,
    ...
    responseSchema: studyResponseSchema(record.stage === 'review' ? localPaperReviewSchema(record) : extractedSetSchema, record.snapshot.chunks.map(c => c.id)),
  ...
  for(const [index,q] of raw.questions.entries()) {
    ...
    try {
      ...
      if(q.originalTranscription) {
        originalTranscription=z.object({sourceKey:z.string(),sha256:z.string(),page:z.number().int().positive(),reason:z.enum(['image','fragmented-text'])}).strict().parse(q.originalTranscription)
...
export async function validateLocalPaper(versionId,id,body,sourceOptions={}) {
  ...
  const response=typeof body.response==='string'?JSON.parse(body.response):body.response
...
export async function submitLocalPaper(versionId, id, body, sourceOptions = {}) {
  ...
  if (record.stage === 'generate') {
    ...
    const checked=validateLocalPaperImport(JSON.parse(response),record)
