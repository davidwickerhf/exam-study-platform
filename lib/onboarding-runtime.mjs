...
import { academicSetupPresence, readAcademicState, saveActiveAcademicWorkspace, normalizeAcademicWorkspace } from './academics.mjs'
...
async function callModel(messages) {
  try { return await sharedCallModel(messages, { usageFeature:'onboarding', tools: ONBOARDING_TOOLS, maxOutputTokens: 1200 }) }
...
// Sign-in only needs a completion verdict. Do not reconcile uploaded documents,
// load Canvas connections or inspect course-rule scans on the entry path.
export async function onboardingStatus({ readStored = storedConversation, readPresence = academicSetupPresence } = {}) {
  const stored = await readStored()
  if (stored.finished) return { finished: true }
  return { finished: establishedRecord(await readPresence()) }
}
