...
export async function chooseElectives({ groupId, courseIds = [] } = {}) {
  const state = await readAcademicState()
  const workspace = state.workspace
  const template = templateOf(workspace)
  const { version } = resolveVersion(template)
  if (!version) throw new OnboardingError('No programme is set yet.')
  const group = (version.choiceGroups || []).find((entry) => entry.id === groupId)
  if (!group) throw new OnboardingError(`No elective group "${groupId}" in this curriculum.`)

  const wanted = [...new Set((Array.isArray(courseIds) ? courseIds : [courseIds]).map((value) => String(value || '').trim()).filter(Boolean))]
  const unknown = wanted.filter((courseId) => !group.courseIds.includes(courseId))
  ...
  // place in the plan.
  const previous = template?.selectedChoices?.[groupId] || []
  ...
  const next = normalizeAcademicWorkspace({
    ...workspace,
    programmeTemplate: { ...template, selectedChoices: { ...(template?.selectedChoices || {}), [groupId]: wanted } },
    courses
  })
...
export async function applySecureValue(kind, value) {
  ...
  if (kind === 'timetable') {
    ...
    outcome = `Canvas is connected: ${courses} current course${courses === 1 ? '' : 's'} are visible. Confirm this briefly and move on.`
  } else {
