...
const OPENING_PLACEHOLDERS = {
  ...
  record: 'Say go and I will show you the upload',
...
export async function openingMessage() {
  ...
  if (!state.record) {
    return {
      step: 'record',
      heading: 'Next: your academic record.',
      body: 'Your Academic Work PDF from the student portal lists every course attempt you have made, which is what tells the dashboard how many credits you already hold and what is still outstanding. Say go and I will show you where to upload it — or skip, and it will wait for you on the dashboard.'
    }
  }
  ...
  return {
    ...
    body: 'Your programme, electives, record, calendar, timetable and Canvas are all set up. Ask me to change any of them, or head back to your dashboard.'
  }
}
...
export async function applySecureValue(kind, value, options = {}) {
  ...
  if (kind === 'timetable') {
    ...
    outcome = `Canvas is connected: ${courses} current course${courses === 1 ? '' : 's'} are visible.${collection} Confirm this briefly and move on.`
  } else {
