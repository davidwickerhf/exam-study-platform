...
export async function handleRemoteMcp(req,res,{handler,ip,dependencies=remoteMcpService()}) {
  ...
  try{
    ...
    if(path==='/api/mcp/consent'||path==='/api/mcp/connections'||path.startsWith('/api/mcp/connections/')){
      ...
      if(path==='/api/mcp/consent'&&req.method==='GET'){json(200,{...await oauth.pending(url.searchParams.get('request')),account:{id:auth.userId,email:auth.email||null}});return}
      if(path==='/api/mcp/consent'&&req.method==='POST'){
        const body=await readRequest(req,4096)
        if(body.approved===true&&body.accountId!==auth.userId){json(409,{error:'account_changed',error_description:'Your signed-in account changed. Reload the connection to review the account before approving.'});return}
      ...
      if(path==='/api/mcp/connections'&&req.method==='GET'){json(200,{connections:await oauth.connections(auth.userId),account:{id:auth.userId,email:auth.email||null}});return}
