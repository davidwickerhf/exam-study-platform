...
export async function fetchCalendar(url, { fetchImpl = fetch } = {}) {
  const response = await fetchImpl(url, { headers: { accept: 'text/calendar, text/plain;q=0.9, */*;q=0.5' }, redirect: 'follow', signal: AbortSignal.timeout(15000) })
  ...
  return parseIcs(text.slice(0, 4 * 1024 * 1024))
}
