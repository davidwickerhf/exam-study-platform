...
function sameSitting(left, right) {
  ...
  return left?.status === 'upcoming' || right?.status === 'upcoming'
}
  ...
  for (const draftCourse of draft?.courses || []) {
    ...
    if (!match) {
      ...
      changes.push({
        id,
        kind: 'new-course',
        label: `${historicalEvidence ? 'Add to academic record' : 'Add to selected courses'}: ${code(draftCourse.code) || draftCourse.name}${code(draftCourse.code) && draftCourse.name && draftCourse.name !== code(draftCourse.code) ? ` — ${draftCourse.name}` : ''}`,
    ...
    for (const attempt of draftCourse.attempts || []) {
      ...
      if (!hasFact) continue
      if (stagedAttempts.some((item) => sameAttempt(item, attempt))) continue
      ...
      if (existing) {
        ...
        if (attempt.status === 'upcoming' && completed.length) addConflict({ id: `${changeId}:conflict`, kind: 'attempt-conflict', label: `${label}: scheduled although already completed`, detail: `Selected plan: ${completed.map(describeAttempt).join(', ')} · ${sourceLabel}: ${describeAttempt(attempt)}. Add a new attempt only if this is intentional.`, payload: { courseId: match.id, attempt }, issue: 'scheduled-completed-course' })
        else {
