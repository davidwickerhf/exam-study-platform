...
export async function fetchCalendar(url, { fetchImpl = fetch } = {}) {
  ...
  let result
  try {
    result = await safeFetch(url, { fetchImpl, headers: { accept: 'text/calendar, text/plain;q=0.9, */*;q=0.5' }, maxBytes: 4 * 1024 * 1024, timeoutMs: 15_000 })
  } catch (error) {
    if (error.name === 'TimeoutError' || error.cause?.code === 'UND_ERR_CONNECT_TIMEOUT') {
      throw new Error('The timetable server did not respond in time. Try refreshing again later.', { cause: error })
    }
    if (error.message === 'fetch failed') {
      throw new Error('The timetable server could not be reached. Try refreshing again later.', { cause: error })
    }
    throw error
  }
...
export function buildChangeSet(workspace, draft, { source = 'document', sourceLabel = 'Uploaded source', kind = 'auto' } = {}) {
  ...
  for (const draftCourse of draft?.courses || []) {
    ...
    const match = (draftCode ? byCode.get(draftCode) : null) || (!draftCode || !codeStrict || !nameMatch?.code ? nameMatch : null) || null
