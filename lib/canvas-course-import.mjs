...
export async function importCanvasCourse({ courseUrl, accessToken, outputFolder, maxResources = CANVAS_IMPORT_LIMITS.maxResources, maxFileBytes = CANVAS_IMPORT_LIMITS.maxFileBytes, fetchImpl = fetch } = {}) {
  ...
  try {
    ...
    if (error instanceof CanvasCourseImportError && /HTTP (403|404) for \/api\/v1\/courses\/.+\/files/.test(error.message)) {
  ...
  const staleLocalResources = [...priorPaths].filter((path) => !currentPaths.has(path)).sort((left, right) => left.localeCompare(right, undefined, { numeric: true }))
  const summary = {
    schemaVersion: 1,
