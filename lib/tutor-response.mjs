...
export const TUTOR_RESPONSE_FORMAT = {
  ...
  json_schema: { name: 'tutor_answer', strict: true, schema: object({
    ...
    drafts: array(object({ title: string, recipient: string, subject: string, body: string })),
...
export function parseTutorResponse(content, proposals = [], attendanceReports = [], artifacts = {}) {
  ...
  const drafts = rows(value.drafts, 'drafts', 2).map(item => Object.fromEntries(['title', 'recipient', 'subject', 'body'].map(key => [key, requiredText(item[key], key, key === 'body' ? 4000 : 240)])))
