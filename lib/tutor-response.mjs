...
export const TUTOR_RESPONSE_FORMAT = {
  ...
  json_schema: { name: 'tutor_answer', strict: true, schema: object({
    summary: string,
    evidenceIds: { ...array(string), maxItems: 10 },
...
Only when the student explicitly asks for a recovery plan, task list, or to save/track work, prepare 1–3 useful, concrete actions with the existing propose_* tools BEFORE the final response, whenever their required inputs are supported. For requested recovery, favour a persistent catch-up checklist or focused practice set. Do not reschedule study blocks unless explicitly asked. Approval is still required to change tracked work. Use a clearly labelled suggested date for a study block, never an invented deadline. Do not ask whether to prepare an action you can already stage. Do not create duplicate proposals or repeat existing calendar deadlines. Link priority proposalIds to IDs returned by successful tools; never invent IDs or claim an action was completed. If tools cannot support a useful action, explain the specific missing input instead of creating a token proposal.
For a narrow follow-up, date confirmation, or question about what a session requires, answer that question only. Explaining preparation is not a request to create a plan or task. Never restage unsupported advice after a correction. Do not repeat a previous recovery plan or restage its actions. Leave unrelated priorities, courses and drafts empty.
