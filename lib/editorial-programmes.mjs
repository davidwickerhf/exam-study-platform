...
function normalizeVersion(value, programmeId) {
  ...
  const choiceGroups = (Array.isArray(value.choiceGroups) ? value.choiceGroups : []).map((group, index) => {
    ...
    const offered = ids.map((courseId) => courses.find((course) => course.id === courseId)).filter(Boolean)
    const yearLevels = [...new Set(offered.map((course) => course.yearLevel).filter(Boolean))]
    const periods = [...new Set(offered.map((course) => course.period).filter(Boolean))]
    return {
      id: groupId,
      ...
      courseIds: ids,
      derived: false,
      yearLevel: text(group.yearLevel, 40) || (yearLevels.length === 1 ? yearLevels[0] : ''),
      period: text(group.period, 40) || (periods.length === 1 ? periods[0] : '')
    }
  })
...
// The repository catalogue is the maintained baseline, while the hosted copy
// may contain later administrator edits. When a deployment adds a programme or
// curriculum version, keep the stored definition for every matching version
// and add only what the hosted catalogue has never seen. Seed order remains
// authoritative so the current curriculum stays the default in pickers.
export function mergeEditorialProgrammeSeed(storedValue, seededValue) {
  const stored = normalizeEditorialProgramme(storedValue)
  const seeded = normalizeEditorialProgramme(seededValue)
  if (stored.id !== seeded.id) throw new Error(`Cannot merge programme ${seeded.id} into ${stored.id}.`)

  const storedVersions = new Map(stored.versions.map((version) => [version.id, version]))
  const seededVersionIds = new Set(seeded.versions.map((version) => version.id))
  const versions = [
    ...seeded.versions.map((version) => storedVersions.get(version.id) || version),
    ...stored.versions.filter((version) => !seededVersionIds.has(version.id))
  ]
  const domains = [...new Set([
    ...(seeded.institution?.domains || []),
    ...(stored.institution?.domains || [])
  ])]

  return normalizeEditorialProgramme({
    ...seeded,
    ...stored,
    institution: { ...seeded.institution, ...stored.institution, domains },
    versions
  })
}
...
/** Course navigation needs placements and historical identities, not every coursebook. */
export function workspaceProgrammeCatalogue(catalogue, programmeId) {
  return {
    schemaVersion: catalogue.schemaVersion,
    programmes: catalogue.programmes.filter(programme => programme.id === programmeId).map(({ calendar, ...programme }) => ({
      ...programme,
      versions: programme.versions.map(({ sources, ...version }) => ({
        ...version,
        courses: version.courses.map(({ description, coordinator, department, prerequisites, reading, teachingMethods, assessmentMethods, ...course }) => course)
      }))
    }))
  }
}
