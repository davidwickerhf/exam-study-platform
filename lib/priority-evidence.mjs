...
export const PRIORITY_EXTRACTION_VERSION = 6
      ...
      properties: {
        ...
        attendanceRules: {
          ...
          items: {
            ...
            required: ['text', 'activity', 'requirement', 'allowedMisses', 'minimumAttendancePercent', 'excusedPolicy', 'evidence'],
            properties: {
              text: { type: 'string' },
...
export function attendanceEvidenceCandidates(rows = []) {
  ...
  for (const file of files.values()) {
    ...
    file.forEach((row,index) => {
      if (ATTENDANCE_SIGNAL.test(row.content) || /\b(?:labs?|tutorials?|lectures?|workshops?|seminars?|debates?)\b.{0,100}\b(?:mandatory|required|optional|compulsory)\b/is.test(row.content) || (['syllabus','requirements'].includes(row.sourceType) && Number(row.page || 1)<=2) || (INTRODUCTION.test(row.filename || '') && Number(row.page || 1)<=8)) {
...
${attendanceOnly ? `This is the dedicated attendance pass. Read the syllabus, announcements and opening lecture material together. Return attendanceRules and attendance conflicts only; leave all assessment component and pass-rule arrays empty. Treat an explicit announcement that a rule was changed from X to Y as an amendment: retain citations for the announcement and affected original rule, and report Y as the current requirement. Do not report the explicitly superseded value as a conflict. Preserve true unresolved disagreements. Grade participation as assessed unless compulsory presence is explicit. Preserve limitations to named/numbered sessions, weeks and student groups in the rule text; never generalize these to all lectures or tutorials. Return separate rules for each named activity, e.g. both labs and tutorials. A sign-up or submission task is not an attendance rule. State optional attendance when the sources explicitly establish it, never from silence. Do not count quiz repairs as permitted attendance absences.` : 'Attendance is handled in a separate pass when supplied; focus on other obligations.'}
...
async function extractPriorityBatch(binding, rows, modelCall, state, depth = 0, attendanceOnly = false) {
  ...
  try {
    ...
    ], { maxOutputTokens: attendanceOnly ? 4500 : 7000, responseFormat: PRIORITY_RESPONSE_SCHEMA })
...
export async function extractPriorityEvidence(binding, candidates, modelCall = callModel, {maxCalls=4,cache,attendanceRows=[]} = {}) {
  ...
  const obligationBatches=priorityEvidenceBatches(candidates)
  ...
  const result=mergePriorityExtractions([...attendanceResults,...results])
...
export async function scanCanvasPriorityEvidence({ bindingId, accountId, force = false, assertActive = () => {}, onProgress = async () => {}, commit = null } = {}) {
  ...
  if (candidates.length && chatAvailable()) {
    extracted = await extractPriorityEvidence(binding, candidates, priorityModelCall(accountId,bindingId,evidenceHash), {cache:priorityBatchCache(accountId,bindingId),attendanceRows})
...
export function recoverLiteralAttendance(courseProfile, rows) {
  ...
  const combined=[...(a.attendanceEvidence || []),...literal.attendanceEvidence]
  const byText=new Map(combined.map(rule=>[rule.text,rule]))
