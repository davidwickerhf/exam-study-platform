...
export async function scanCanvasPriorityEvidence({ bindingId, accountId } = {}) {
  ...
  if (candidates.length && chatAvailable()) {
    ...
    ], { maxOutputTokens: 4000, responseFormat: { type: 'json_object' } })
    try {
      extracted = jsonObject(message.content)
    } catch {
      // The course archive is already complete at this point. A provider that
      // spends its output budget without returning JSON must not make the
      // worker download and index the whole course again. Keep the evidence
      // visible for review and let a later refresh retry the derived scan.
      extracted = {
        ...extracted,
        status: 'needs-review',
        conflicts: [{
          title: 'Automatic priority extraction needs another pass',
          detail: `${candidates.length} relevant source passages were indexed, but the language model did not return structured priority data. The stored course material remains available.`,
          chunkIds: candidates.slice(0, 5).map((row) => row.chunkId)
        }]
      }
    }
