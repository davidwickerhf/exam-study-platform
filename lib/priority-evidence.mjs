...
export async function scanCanvasPriorityEvidence({ bindingId, accountId, assertActive = () => {} } = {}) {
  ...
  if (held && (held.status !== 'needs-review' || Date.now() - new Date(held.scanned_at).getTime() < 6 * 60 * 60_000)) return { status: held.status, courseProfile: held.course_profile, conflicts: held.conflicts, candidates: candidates.length, cached: true, scannedAt: held.scanned_at }
