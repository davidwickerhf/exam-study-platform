...
export const PRIORITY_EXTRACTION_VERSION = 7
...
const PRIORITY_RESPONSE_SCHEMA = {
  ...
  json_schema: {
    ...
    schema: {
      ...
      required: ['status', 'attendanceRules', 'components', 'actions', 'overallPassRules', 'resitRules', 'conflicts'],
      properties: {
        ...
        attendanceRules: {
          ...
          items: {
            ...
            properties: {
              ...
              activity: { type: 'string', enum: ['lecture', 'tutorial', 'lab', 'workshop', 'seminar', 'debate', 'other'] },
              allowedMisses: { type: ['integer', 'null'] },
              minimumAttendancePercent: { type: ['number', 'null'] },
              excusedPolicy: { type: 'string' },
              scope: {type:'object',additionalProperties:false,required:['kind','labels','dates'],properties:{
                kind:{type:'string',enum:['all','specific']}, labels:{type:'array',items:{type:'string'}},dates:{type:'array',items:{type:'string'}}
              }},
...
const emptyExtraction = () => ({ status: 'not-found', attendanceRules: [], components: [], actions: [], overallPassRules: [], resitRules: [], conflicts: [] })
...
export function mergePriorityExtractions(values = []) {
  ...
  const hasClaims = merged.attendanceRules.length || merged.components.length || merged.actions.length || merged.overallPassRules.length || merged.resitRules.length
...
function prioritySource(row) {
  if (/(?:announcement|discussion)/i.test(row.sourcePath || row.filename || '')) return {...row,sourceType:'announcements'}
...
export function priorityEvidenceCandidates(rows = [], limit = 100) {
  const ranked = rows.map(prioritySource).filter(row => !sourceCode(row) && (SIGNAL.test(row.content) || ['syllabus', 'requirements'].includes(row.sourceType) || INTRODUCTION.test(row.filename || '')))
...
export function attendanceEvidenceCandidates(rows = []) {
  const ranked = [...new Map(rows.map(prioritySource).filter(row => !sourceCode(row) && !/--links-| linked file--/i.test(row.filename || '')).map(row=>[Number(row.chunkId),row])).values()]
...
Return JSON only:
{"status":"confirmed|needs-review|not-found","attendanceRules":[{"text":"","activity":"lecture|tutorial|lab|workshop|seminar|debate|other","requirement":"required|optional|assessed","allowedMisses":null,"minimumAttendancePercent":null,"excusedPolicy":"","scope":{"kind":"all|specific","labels":[],"dates":[]},"evidence":[{"chunkId":1}]}],"components":[{"name":"","type":"exam|project|presentation|assignment|participation|other","weightPercent":null,"minimumPercent":null,"deadline":null,"deadlineText":"","notes":"","evidence":[{"chunkId":1}]}],"actions":[{"title":"","parent":"","kind":"team|preparation|approval|submission|presentation|other","deadline":null,"deadlineText":"","prerequisite":"","notes":"","evidence":[{"chunkId":1}]}],"overallPassRules":[],"resitRules":[],"conflicts":[{"title":"","detail":"","chunkIds":[1,2]}]}
...
async function extractPriorityBatch(binding, rows, modelCall, state, depth = 0, attendanceOnly = false) {
  ...
  try {
    ...
    return {
      ...
      status: 'needs-review',
      conflicts: [{
        title: error.code === 'SCAN_CALL_LIMIT' ? 'Priority scan allowance reached' : error.status === 429 ? 'Priority AI provider unavailable' : 'Automatic priority extraction needs another pass',
...
${JSON.stringify(extracted)}`}
    ...
    return {...extracted,status:'needs-review',conflicts:[...(extracted.conflicts || []),{
      title:error.code==='SCAN_CALL_LIMIT' ? 'Priority scan allowance reached' : 'Automatic priority extraction needs another pass',
      detail:'The extracted steps still need their final cross-source check. Saved batches will be reused.',chunkIds:evidence.map(row=>Number(row.chunkId))
    }]}
  }
}
...
export async function extractPriorityEvidence(binding, candidates, modelCall = callModel, {maxCalls=4,cache,attendanceRows=[]} = {}) {
  ...
  for (const rows of obligationBatches) {
    ...
    results.push(attendanceRows.length ? {...result,attendanceRules:[]} : result)
  }
  let result=mergePriorityExtractions([...attendanceResults,...results])
  const operational = result.conflicts.some(conflict=>/^(Priority scan allowance reached|Priority AI provider unavailable|Automatic priority extraction needs another pass)$/.test(conflict.title))
  if (!operational && candidates.length && (result.actions.length || result.components.length || result.attendanceRules.length)) {
    ...
    if (!reconciled.conflicts?.some(conflict=>/^(Priority scan allowance reached|Automatic priority extraction needs another pass)$/.test(conflict.title))) {
      ...
      attendanceResults.splice(0,attendanceResults.length,{...reconciled,status:conflicts.length ? 'needs-review' : reconciled.attendanceRules?.length ? 'confirmed' : 'not-found',components:[],actions:[],conflicts})
    }
  }
  ...
  if (result.conflicts?.some(conflict=>/^(Priority scan allowance reached|Priority AI provider unavailable|Automatic priority extraction needs another pass)$/.test(conflict.title))) result={...result,actions:[],components:[]}
...
export function normalizeScan(value = {}, rows = []) {
  ...
  const attendanceRules = [...supplied.filter(rule => !literal.some(found => found.text === clean(rule?.text,500))), ...literal].map((rule) => ({
    ...
    activity: ['lecture', 'tutorial', 'lab', 'workshop', 'seminar', 'debate', 'other'].includes(rule?.activity) ? rule.activity : 'other',
    ...
    ...(rule?.scope ? {scope:{kind:rule.scope.kind==='specific' ? 'specific' : 'all',labels:(rule.scope.labels || []).map(label=>clean(label,100)).filter(Boolean),dates:(rule.scope.dates || []).map(validPriorityDeadline).filter(date=>date?.length===10)}} : {}),
  ...
  const hasClaims = attendanceRules.length || components.length || actions.length
