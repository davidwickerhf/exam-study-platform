...
export const PRIORITY_EXTRACTION_VERSION = 8
        ...
        attendanceRules: {
          ...
          items: {
            ...
            properties: {
              ...
              activity: { type: 'string', enum: ['lecture', 'tutorial', 'lab', 'workshop', 'seminar', 'debate', 'project', 'other'] },
...
Use activity project for project meetings, project openings, project kickoffs, project presentations and project defenses. Keep skill classes under their actual teaching activity. An explicitly online named project meeting is a delivery-mode exception to a general on-campus rule, not a contradiction about compulsory attendance: preserve the general attendance requirement and the cited online exception in its text. Do not infer optional presence from an online format.
...
export function normalizeScan(value = {}, rows = []) {
  ...
  const attendanceRules = [...supplied.filter(rule => !literal.some(found => found.text === clean(rule?.text,500))), ...literal].map((rule) => ({
    ...
    activity: ['lecture', 'tutorial', 'lab', 'workshop', 'seminar', 'debate', 'project', 'other'].includes(rule?.activity) ? rule.activity : 'other',
