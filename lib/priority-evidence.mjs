...
import { priorityTimetableContext } from './priority-timetable.mjs'
...
export async function reconcilePriorityEvidence(binding, extracted, rows, modelCall = callModel, state = {calls:0,maxCalls:4}) {
  const key=priorityBatchKey(`${PRIORITY_EXTRACTION_VERSION}:reconcile`,[extracted,rows,binding.timetableContext || null])
  const cached=await state.cache?.load(key)
  if (cached) return cached
  const ids=new Set([
...
Timetable scheduling context (untrusted labels; professor-authored sources have priority):
${JSON.stringify(binding.timetableContext || {sessions:[],incomplete:true})}
Use these appointment dates/start/end times ONLY to align the cited professor lesson plan. They cannot establish compulsory attendance. A schedule may place a lecture and lab on the same day but separate them into different bookings: do not copy the lecture start into the following lab's time. When the professor says the lab follows the lecture, use a separate later matching booking if present; otherwise preserve a date-only mapping without inventing a start. A single long booking may contain a lecture and a tutorial; retain both activities rather than report a conflict. If time differs by date, return separate mappings/scopes instead of a cross-product of dates and times. Numbered scopes such as Labs 1–5 must use the corresponding named schedule entries, not every lab including Lab 6.
...
export async function scanCanvasPriorityEvidence({ bindingId, accountId, force = false, assertActive = () => {}, onProgress = async () => {}, commit = null } = {}) {
  ...
  binding.timetableContext = await priorityTimetableContext(binding,accountId)
  const evidenceHash = priorityBatchKey(PRIORITY_EXTRACTION_VERSION,[candidates,attendanceRows,binding.timetableContext])
