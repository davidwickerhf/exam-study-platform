    ...
    if(job.job_type==='catalog') {
      ...
      if(resource) {
        ...
        if(!rules) {
          ...
          const result=await scanCanvasPriorityEvidence({bindingId:binding.id,accountId:job.user_id,force:false,assertActive:()=>signal.throwIfAborted(),commit:queries=>fenced(job,queries),onProgress:event=>job.log(event)})
