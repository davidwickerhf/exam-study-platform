  ...
  try {
    ...
    if(job.job_type==='catalog') {
      ...
      if(resource) {
        ...
        await fenced(job,[sql`UPDATE canvas_course_bindings SET last_synced_at=now(),next_sync_at=now()+make_interval(hours=>${CANVAS_MATERIAL_REFRESH_HOURS}),updated_at=now() WHERE id=${binding.id}`,
