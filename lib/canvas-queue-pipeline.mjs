...
import { runCanvasFreshnessCheck } from './canvas-freshness-store.mjs'
import { materialLocations } from './canvas-material-locations.mjs'
import { indexedPaperJob, PAPER_JOBS } from './study-paper-jobs.mjs'
import { queueWorkersEnabled, previewWorkerUsers } from './queue-runtime.mjs'
import { supportsTextExtraction, courseFileMediaType, needsExtractionUpgrade } from './course-file-types.mjs'
import { recordQualityEvent } from './feedback-store.mjs'
...
import { extracted, retrievalRecords, MEDIA_TYPES, sourceTypeForPath, runCatalog, scheduleDueRefreshes, CANVAS_MATERIAL_REFRESH_HOURS } from './canvas-corpus-worker.mjs'
...
export async function dispatchCanvasQueue() {
  if (!sql || !queueWorkersEnabled()) return []
  ...
  const rows = await sql`SELECT j.id FROM canvas_sync_jobs j
    ...
    WHERE j.status IN ('pending','running') AND j.run_after<=now()
      AND (${process.env.VERCEL_ENV !== 'preview'} OR j.user_id=ANY(${previewWorkerUsers()}::text[]))
...
async function restoreRetry(job) {
  ...
  await fenced(job, [
    ...
    sql`INSERT INTO canvas_sync_resources(id,job_id,source_path,payload,stage,asset_id,downloaded_bytes,total_bytes,etag,index_offset)
      SELECT concat(${job.id}::text,':',md5(source_path)),${job.id},source_path,payload,
...
async function discover(job, binding, budget) {
  ...
  const put = async (path, payload) => {
    ...
    check()
    const reusable = !job.payload?.force && (typeof payload.content === 'string' || Boolean(payload.fileId && payload.updatedAt))
    const [unchanged] = !reusable ? [] : await sql`SELECT old.asset_id,old.total_bytes,old.index_offset,asset.extraction_status,asset.metadata FROM canvas_sync_resources old
      JOIN canvas_sync_jobs prior ON prior.id=old.job_id
      JOIN editorial_source_assets asset ON asset.id=old.asset_id AND asset.is_complete=true
      WHERE prior.user_id=${job.user_id} AND prior.binding_id=${binding.id} AND prior.id<>${job.id}
        AND old.source_path=${path} AND old.stage='complete' AND (old.payload-'locations')=${json(payload)}::jsonb
        AND EXISTS(SELECT 1 FROM canvas_source_snapshots snapshot WHERE snapshot.binding_id=${binding.id} AND snapshot.contributor_user_id=${job.user_id}
          AND snapshot.asset_id=old.asset_id AND snapshot.source_path=${path} AND snapshot.retired_at IS NULL)
      ORDER BY old.updated_at DESC LIMIT 1`
    await fenced(job, [sql`INSERT INTO canvas_sync_resources(id,job_id,source_path,payload,stage,asset_id,total_bytes,downloaded_bytes,index_offset)
      VALUES(${resourceId(job.id,path)},${job.id},${path},${json(payload)}::jsonb,${unchanged ? needsExtractionUpgrade(path, unchanged) ? 'extract' : 'complete' : 'download'},${unchanged?.asset_id || null},${unchanged?.total_bytes || null},${unchanged?.total_bytes || 0},${unchanged && !needsExtractionUpgrade(path, unchanged) ? unchanged.index_offset || 0 : 0}) ON CONFLICT DO NOTHING`,
      ...(unchanged ? [sql`UPDATE canvas_source_snapshots SET last_seen_at=now() WHERE binding_id=${binding.id} AND contributor_user_id=${job.user_id} AND source_path=${path} AND asset_id=${unchanged.asset_id} AND retired_at IS NULL`] : [])])
    if (unchanged) job.log({ stage: 'discovery', message: needsExtractionUpgrade(path, unchanged) ? 'Saved original reused; refreshing text extraction for supported file formats.' : 'Unchanged resource: original and search index reused.', item: path })
    known.add(path)
  }
  ...
  await importCanvasCourse({
    ...
    durable: {
      ...
      finish: async summary => {
        ...
        for (const path of new Set(summary.resources.map(r=>r.target || r.path))) {
          const locations=materialLocations(summary.resources,path)
          locationWrites.push(
            sql`UPDATE canvas_sync_resources SET payload=payload || ${json({locations})}::jsonb WHERE job_id=${job.id} AND source_path=${path}`,
            sql`UPDATE canvas_source_snapshots SET metadata=metadata || ${json({locations})}::jsonb WHERE binding_id=${binding.id} AND contributor_user_id=${job.user_id} AND source_path=${path} AND retired_at IS NULL`
          )
        }
        for(let at=0;at<locationWrites.length;at+=64) await fenced(job,locationWrites.slice(at,at+64))
        const reused=await sql`SELECT r.source_path,a.sha256 FROM canvas_sync_resources r JOIN editorial_source_assets a ON a.id=r.asset_id WHERE r.job_id=${job.id} AND r.stage='complete'`
        const paperWrites=[]
        for(const resource of reused) {
          const paperJob=await indexedPaperJob(job.user_id,binding,resource.source_path,resource.sha256)
          if(paperJob) paperWrites.push(sql`INSERT INTO user_documents(user_id,namespace,document_key,value,updated_at) VALUES(${job.user_id},${PAPER_JOBS},${paperJob.id},${json(paperJob)}::jsonb,now()) ON CONFLICT DO NOTHING`)
        }
        for(let at=0;at<paperWrites.length;at+=64)await fenced(job,paperWrites.slice(at,at+64))
        const inventory = { course:summary.course, modules:summary.modules, sourcePaths:[...new Set(summary.resources.map(r=>r.target || r.path))], resources:summary.resources.length, skipped:summary.skipped }
...
async function download(job, binding, resource, budget, signal) {
  ...
  const mediaType=courseFileMediaType(resource.source_path) || resource.payload.mediaType || MEDIA_TYPES.get(extname(resource.source_path).toLowerCase()) || 'application/octet-stream'
  await fenced(job,[
...
async function extractResource(job,resource,budget) {
  ...
  let result
  const needsDatasetProfile = /\.(csv|tsv|xlsx|xls)$/i.test(resource.source_path) && Number(asset.byte_size)>1024*1024 && asset.metadata?.datasetProfileVersion !== 1
  if(asset.extraction_status==='complete' && !needsDatasetProfile && !needsExtractionUpgrade(resource.source_path,asset)) result={text:asset.extracted_text,pages:asset.extracted_pages,status:'complete'}
  else if (!supportsTextExtraction(resource.source_path)) {
    ...
    try {
      ...
      result=supportsTextExtraction(resource.source_path)
    ...
    await fenced(job,[sql`UPDATE editorial_source_assets SET extracted_text=${result.text},extracted_pages=${result.pages?json(result.pages):null}::jsonb,
      content_sha256=${result.text?hash(result.text):null},extraction_status=${result.status},extraction_error=${result.error || null},
      metadata=metadata || ${json(needsDatasetProfile && result.status==='complete'?{fileFormatVersion:5,datasetProfileVersion:1,indexCoverage:'dataset-structure-and-sample'}:{fileFormatVersion:5})}::jsonb,updated_at=now() WHERE id=${asset.id}`])
  }
  if(result.status==='failed') {
    const error=new Error(result.error || 'Text extraction failed; the original is safely stored.')
    error.permanent=/safe.*limit|expansion.*limit|entry limit|expansion ratio|XML entities|Invalid archive|Encrypted archive|Nested archive/i.test(error.message)
    throw error
  }
  job.log({stage:'extraction',message:result.status==='unsupported'?'Original safely stored. This format has no searchable text.':needsDatasetProfile?'Dataset structure and sample saved for search. The full original is preserved.':'Document text saved.',item:resource.source_path})
...
async function indexResource(job,binding,resource) {
  // Upgrade datasets already stuck in the old all-cells embedding path without
  // retrieving their entire extracted text on every callback.
  const [info]=await sql`SELECT byte_size,metadata FROM editorial_source_assets WHERE id=${resource.asset_id}`
  if (/\.(csv|tsv|xlsx|xls)$/i.test(resource.source_path) && Number(info?.byte_size)>1024*1024 && info.metadata?.datasetProfileVersion!==1) {
    await fenced(job,[sql`DELETE FROM canvas_sync_index_staging WHERE resource_id=${resource.id}`,
      sql`UPDATE canvas_sync_resources SET stage='extract',index_offset=0,failures=0,updated_at=now() WHERE id=${resource.id}`])
    job.log({stage:'indexing',message:'Preparing dataset structure and sample; full original retained.',item:resource.source_path})
    return
  }
  ...
  const paperJob=await indexedPaperJob(job.user_id,binding,resource.source_path,asset.sha256)
  await fenced(job,[
    ...
    sql`INSERT INTO canvas_source_snapshots(id,binding_id,asset_id,contribution_id,contributor_user_id,sharing_mode,resource_key,source_path,resource_type,sha256,metadata)
      VALUES(${snapshot},${binding.id},${asset.id},${contribution},${job.user_id},${permission.sharing_mode},${resource.source_path},${resource.source_path},${sourceTypeForPath(resource.source_path)},${asset.sha256},${json({academicYear:binding.academic_year,courseCode:binding.course_code,locations:resource.payload.locations || []})}::jsonb)
    ...
    ...(paperJob ? [sql`INSERT INTO user_documents(user_id,namespace,document_key,value,updated_at) VALUES(${job.user_id},${PAPER_JOBS},${paperJob.id},${json(paperJob)}::jsonb,now()) ON CONFLICT DO NOTHING`] : []),
...
export async function processCanvasQueueStep(jobId) {
  if(!sql || !queueWorkersEnabled()) return {again:false,disabled:true}
  ...
  const [job]=await sql`UPDATE canvas_sync_jobs j SET status='running',lease_token=${token},heartbeat_at=now(),started_at=coalesce(started_at,now()),
      queue_sent_at=null,attempts=CASE WHEN j.error IS NOT NULL THEN greatest(attempts,1)+1 ELSE greatest(attempts,1) END,payload=payload || '{"pipeline":"queue-v1"}'::jsonb
    WHERE j.id=${jobId} AND j.status IN ('pending','running') AND j.run_after<=now()
      AND (${process.env.VERCEL_ENV !== 'preview'} OR j.user_id=ANY(${previewWorkerUsers()}::text[]))
      AND (j.lease_token IS NULL OR coalesce(j.heartbeat_at,j.started_at)<now()-interval '5 minutes')
      AND EXISTS(SELECT 1 FROM canvas_corpus_permissions p WHERE p.user_id=j.user_id AND p.origin=j.origin AND p.collection_enabled=true AND (coalesce(j.payload->>'scheduled','false')<>'true' OR (p.refresh_enabled=true AND p.study_status='studying' AND EXISTS(SELECT 1 FROM academic_programmes ap WHERE ap.user_id=j.user_id AND ap.is_active=true))))
      AND NOT EXISTS(SELECT 1 FROM canvas_corpus_access a WHERE a.user_id=j.user_id AND a.binding_id=j.binding_id AND a.sync_paused=true)
    RETURNING j.*`
  ...
  try {
    ...
    if(job.job_type==='catalog') {
      const result=await runCatalog(job,budget,{commit:queries=>fenced(job,queries)})
      ...
      if(job.payload?.stage==='freshness') {
        const result = await runCanvasFreshnessCheck(job,binding,{commit:queries=>fenced(job,queries)})
        await fenced(job,[sql`UPDATE canvas_sync_jobs SET result=${json(result)}::jsonb WHERE id=${job.id}`])
        terminal=true
      } else {
      ...
      resource=rows[0]
      if(resource) {
        if(Number(resource.failures)>=3) {
          const error='This resource repeatedly stopped before saving a checkpoint. Original retained; retry this sync to resume it.'
          await fenced(job,[sql`UPDATE canvas_sync_resources SET stage='failed',error=${error},updated_at=now() WHERE id=${resource.id}`])
          job.log({stage:'queue',level:'error',message:error,item:resource.source_path})
        } else {
          // Persist the attempt BEFORE potentially CPU-heavy work. Hard runtime
          // termination cannot run catch/finally; the next delivery still sees it.
          await fenced(job,[sql`UPDATE canvas_sync_resources SET failures=failures+1 WHERE id=${resource.id}`])
          ...
          await fenced(job,[sql`UPDATE canvas_sync_resources SET failures=0 WHERE id=${resource.id}`])
        }
      } else {
        ...
        if(!rules) {
          ...
          const result=!await withRequestContext({userId:job.user_id},()=>recurringEnabled('priorities')) ? {status:'paused',candidates:0} : await scanCanvasPriorityEvidence({bindingId:binding.id,accountId:job.user_id,force:false,assertActive:()=>signal.throwIfAborted(),commit:queries=>fenced(job,queries),onProgress:event=>job.log(event)})
          await checkpoint(job,'rules',{status:result.status,candidates:result.candidates})
        }
        // Rules from saved sources remain useful even if an unrelated resource
        // failed. Keep the sync failure visible after the rule checkpoint.
        ...
        if(job.payload?.stage!=='priorities') {
          const moduleInventory={bindingId:binding.id,course:{courseCode:binding.course_code,courseName:binding.course_name,academicYear:binding.academic_year || 'undated',period:String(binding.period||'')},modules:inventory?.value?.modules || [],sourcePaths:inventory?.value?.sourcePaths || [],skipped,updatedAt:new Date().toISOString()}
          await fenced(job,[sql`INSERT INTO user_documents(user_id,namespace,document_key,value,updated_at) VALUES(${job.user_id},'canvas-module-inventories',${binding.id},${json(moduleInventory)}::jsonb,now()) ON CONFLICT(user_id,namespace,document_key) DO UPDATE SET value=excluded.value,updated_at=now()`])
        }
        // Missing resources from a partial/changed listing are NEVER auto-retired.
        await fenced(job,[...(job.payload?.stage==='priorities' ? [] : [sql`UPDATE canvas_course_bindings SET last_synced_at=now(),next_sync_at=now()+make_interval(hours=>${CANVAS_MATERIAL_REFRESH_HOURS}),updated_at=now() WHERE id=${binding.id}`]),
    ...
    await fenced(job,[sql`UPDATE canvas_sync_jobs SET status=${terminal?'completed':'running'},lease_token=null,heartbeat_at=now(),
      error=null,payload=payload || '{"queueFailures":0}'::jsonb,finished_at=${terminal?new Date().toISOString():null}::timestamptz,run_after=now() WHERE id=${job.id}`])
    ...
    await logger.finish()
    if(!yielding)void recordQualityEvent({code:resource?.stage==='extract'?'EXTRACTION_FAILURE':'SYNC_FAILURE',stage:resource?.stage==='download'?'download':resource?.stage==='index'?'index':'extraction',route:'/app/settings/canvas-sync'}, {userId:job.user_id,eventId:`sync-${job.id}-${resource?.id||'queue'}-${resource?.failures||job.payload?.queueFailures||0}`}).catch(()=>{})
    const blocked = error?.retryable === false
    const retry=yielding || (!blocked && (resource ? Number(resource.failures)<2 : Number(job.payload?.queueFailures || 0)<2))
    const again = !blocked && Boolean(retry || resource)
    try {
      await fenced(job,[
        ...(resource ? [yielding
          ? sql`UPDATE canvas_sync_resources SET failures=${Number(resource.failures)} WHERE id=${resource.id}`
          : sql`UPDATE canvas_sync_resources SET error=${message},stage=CASE WHEN failures>=3 OR ${Boolean(error.permanent)} THEN 'failed' ELSE stage END,updated_at=now() WHERE id=${resource.id}`] : []),
        sql`UPDATE canvas_sync_jobs SET lease_token=null,heartbeat_at=now(),error=${yielding?null:message},
          status=${again?'running':'failed'},finished_at=${again?null:new Date().toISOString()}::timestamptz,
          payload=payload || ${json({queueFailures:yielding?0:Number(job.payload?.queueFailures || 0)+1, ...(blocked ? {blockedReason: error.blockedReason || 'canvas-connection'} : {})})}::jsonb,
          run_after=now()+make_interval(secs=>${yielding?0:30}) WHERE id=${job.id}`
      ])
    } catch { return {again:false} /* cancellation or lease transfer */ }
    return {again,delay:yielding?0:30}
  }
}
