...
async function discover(job, binding, budget) {
  ...
  const put = async (path, payload) => {
    ...
    const [unchanged] = !reusable ? [] : await sql`SELECT old.asset_id,old.total_bytes,old.index_offset FROM canvas_sync_resources old
    ...
    await fenced(job, [sql`INSERT INTO canvas_sync_resources(id,job_id,source_path,payload,stage,asset_id,total_bytes,downloaded_bytes,index_offset)
      VALUES(${resourceId(job.id,path)},${job.id},${path},${json(payload)}::jsonb,${unchanged?'complete':'download'},${unchanged?.asset_id || null},${unchanged?.total_bytes || null},${unchanged?.total_bytes || 0},${unchanged?.index_offset || 0}) ON CONFLICT DO NOTHING`,
    ...
    if (unchanged) job.log({ stage: 'discovery', message: 'Unchanged resource: original and search index reused.', item: path })
    known.add(path)
  }
...
async function extractResource(job,resource,budget) {
  ...
  let result
  const needsDatasetProfile = /\.(csv|xlsx)$/i.test(resource.source_path) && Number(asset.byte_size)>1024*1024 && asset.metadata?.datasetProfileVersion !== 1
  if(asset.extraction_status==='complete' && !needsDatasetProfile) result={text:asset.extracted_text,pages:asset.extracted_pages,status:'complete'}
    ...
    await fenced(job,[sql`UPDATE editorial_source_assets SET extracted_text=${result.text},extracted_pages=${result.pages?json(result.pages):null}::jsonb,
      ...
      metadata=metadata || ${json(needsDatasetProfile && result.status==='complete'?{datasetProfileVersion:1,indexCoverage:'dataset-structure-and-sample'}:{})}::jsonb,updated_at=now() WHERE id=${asset.id}`])
  }
...
async function indexResource(job,binding,resource) {
  ...
  if (/\.(csv|xlsx)$/i.test(resource.source_path) && Number(info?.byte_size)>1024*1024 && info.metadata?.datasetProfileVersion!==1) {
