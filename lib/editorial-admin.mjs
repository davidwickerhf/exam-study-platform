...
import { loadEditorialProgrammeFile, mergeEditorialProgrammeSeed, normalizeEditorialProgramme, setEditorialProgrammeCatalogue } from './editorial-programmes.mjs'
...
// The repository catalogue is a guaranteed baseline. Reconcile it on every
// start so later deployments can add maintained programmes and curriculum
// versions to a database that was seeded by an older release. Existing hosted
// definitions win for matching versions, preserving administrator edits.
export async function primeProgrammeCatalogue() {
  if (!sql) return { mode: 'file' }
  const storedRows = await sql`SELECT id, definition FROM editorial_programmes ORDER BY id`
  const storedById = new Map(storedRows.map((row) => [row.id, row.definition]))
  let inserted = 0
  let updated = 0
  let versionsAdded = 0

  for (const programme of loadEditorialProgrammeFile()) {
    const stored = storedById.get(programme.id)
    if (!stored) {
    ...
    const storedVersionIds = new Set((Array.isArray(stored.versions) ? stored.versions : []).map((version) => String(version?.id || '')))
    const missingVersions = programme.versions.filter((version) => !storedVersionIds.has(version.id))
    const storedDomains = new Set(Array.isArray(stored.institution?.domains) ? stored.institution.domains : [])
    const missingDomains = (programme.institution?.domains || []).filter((domain) => !storedDomains.has(domain))
    if (!missingVersions.length && !missingDomains.length) continue

    const merged = mergeEditorialProgrammeSeed(stored, programme)
    await sql`UPDATE editorial_programmes SET definition = ${JSON.stringify(merged)}::jsonb, updated_at = now() WHERE id = ${programme.id}`
    updated++
    versionsAdded += missingVersions.length
  }
  await refreshProgrammeCatalogue()
  return { mode: 'neon', inserted, updated, versionsAdded }
}
