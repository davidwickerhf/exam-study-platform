...
// A staged bundle child is a publication in progress, not a guide: it exists so
// the parent's fenced commit can flip it, and stays unreadable until then. Only
// the publication itself opts in.
export async function ownStudyVersion(id, { includeStaged = false } = {}) {
  ...
  if (!includeStaged && value.courseBundleParent?.state === 'staged')
    throw new StudyVersionError('Study version not found.', 404)
  return value
}
export async function listCourseBundleChildren(versionId) {
  return (await listDocuments(VERSION_NAMESPACE))
    .map((r) => r.value)
    .filter((v) => v.courseBundleParent?.versionId === versionId)
}
export async function mutateStudyVersion(id, change) {
  for (let attempt = 0; attempt < 5; attempt++) {
    const old = await ownStudyVersion(id, { includeStaged: true }),
...
export async function studyRevision(version, id = version.activeRevisionId) {
  ...
  return hydrateStudySnapshot(
    await readDocument('study-revisions', `${version.id}-${id}`, null)
  )
}
// A managed bundle guide references its course run's immutable snapshot instead
// of copying it, and stores only the sources and evidence ids it actually cites.
// Expand that scoped view on read so every reader still sees real chunks; the
// course-level sourceHash and each source's access provenance are stored inline
// and are never recomputed here.
export async function hydrateStudySnapshot(revision) {
  const ref = revision?.snapshot?.ref
  if (!ref) return revision
  const parent = await readDocument(
    'study-revisions',
    `${ref.versionId}-${ref.revisionId}`,
    null
  )
  if (!parent)
    throw new StudyVersionError(
      'The course run this guide was derived from is unavailable.',
      409
    )
  const ids = new Set(revision.snapshot.sourceIds || []),
    keys = new Set((revision.snapshot.sources || []).map((s) => s.key))
  return {
    ...revision,
    snapshot: {
      ...parent.snapshot,
      ...revision.snapshot,
      chunks: parent.snapshot.chunks.filter((c) => ids.has(c.id)),
      excluded: (parent.snapshot.excluded || []).filter((e) => keys.has(e.key))
    }
  }
}
...
export async function pendingStudyVersions() {
  ...
  // Managed bundle guides are published by their parent run and never enter the
  // hosted queue, whatever status a future writer leaves on their derived draft.
  return rows.filter(
    (r) =>
      queueWorkerAllowsUser(r.owner) && !r.value.courseBundleParent && r.value.draft?.execution !== 'local' && ['queued', 'running'].includes(r.value.draft?.status) &&
...
export async function claimStudyDispatch({ owners } = {}) {
  const ids = []
  const allowedOwners = owners ? new Set(owners) : null
  const pending = allowedOwners
    ? (await pendingStudyVersions()).filter((row) => allowedOwners.has(row.owner))
    : await pendingStudyVersions()
