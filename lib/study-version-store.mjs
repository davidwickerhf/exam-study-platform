...
export async function listOwnStudyVersions(courseCode) {
  return (await listDocuments(VERSION_NAMESPACE))
    .map((r) => r.value)
    .filter((v) => !v.practiceHost && (!courseCode || v.course.courseCode === courseCode))
...
export async function createStudyVersion(
  ...
  programmeId,
  snapshot,
  { title = '', parent = null, billing = null, execution = 'hosted' } = {}
) {
  ...
  const version = {
    ...
    draft: { ...newStudyDraft(snapshot, billing), execution, status: execution === 'local' ? 'local-ready' : 'queued' },
    visibility: 'private',
    review: 'unreviewed'
  }
...
export async function saveStudyRevision(version, draft) {
  const value = {
    ...
    generation: { execution: draft.execution || 'hosted', contractId: draft.localContractId || null, semanticReview: draft.execution === 'local' ? 'local-agent' : 'platform-model' },
    maps: draft.maps,
...
export async function discoverStudyDocuments(namespace) {
  if (![VERSION_NAMESPACE, 'study-publications', 'study-paper-jobs'].includes(namespace))
...
export async function pendingStudyVersions() {
  const rows = sql
    ...
    WHERE namespace=${VERSION_NAMESPACE} AND value->'draft'->>'status' IN ('queued','running') AND coalesce(value->'draft'->>'execution','hosted')<>'local'
