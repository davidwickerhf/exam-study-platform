...
export const GUIDE_CONTRACT_FILES = [
  ...
  'study-generation-limits.mjs', 'study-version-sources.mjs', 'study-review-dependencies.mjs', 'study-source-refresh.mjs'
]
export async function guideContractFingerprint(read = file => readFile(new URL(file, import.meta.url), 'utf8')) {
  return digest({ protocol: 1, schema: 2, files: await Promise.all(GUIDE_CONTRACT_FILES.map(async file => [file, await read(file)])) })
}
let contractPromise
export function studyGenerationContract() {
  contractPromise ||= Promise.all([guideContractFingerprint(), Promise.all(['study-factual-review.mjs','study-pedagogical-review.mjs','study-pedagogy.mjs','study-preflight.mjs'].map(file=>readFile(new URL(file,import.meta.url),'utf8'))).then(digest)]).then(([id,reviewRulesId]) => ({
    // schemaVersion 2 adds the `requests` array to next: an additive, backward
    // compatible field. protocol 1 clients that read only `request`/`nextAction`
    // still receive the first outstanding packet and still complete a run.
    protocol: 1, schemaVersion: 2, workflow: 'lesson-guides', id, reviewRulesId, standard: STUDY_STANDARD,
