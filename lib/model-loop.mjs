import { trackAiCall } from './ai-call-tracking.mjs'
...
export async function callModel(messages, { tools = [], toolChoice = 'auto', maxOutputTokens = 1400, responseFormat = null, signal, reasoningEffort = null, model: requestedModel = null, onContent, usageFeature } = {}) {
  ...
  return trackAiCall({provider:'openai',model,...(usageFeature?{feature:usageFeature}:{}),firstParty:new URL(baseUrl).hostname==='api.openai.com'},async observe => {
  ...
  if (onContent && response.headers.get('content-type')?.includes('text/event-stream')) {
    const result=await readModelStream(response,onContent).catch(error=>{error.providerRequestId=response.headers.get('x-request-id');throw error})
    observe({...result,providerRequestId:response.headers.get('x-request-id')})
    await settleRemoteBudget?.(result.usage)
    return result
  }
  const data = await response.json()
  observe({...data,providerRequestId:response.headers.get('x-request-id')})
...
export async function runToolLoop({
  ...
  const complete = async (availableTools, budget) => {
    ...
    const pendingTools = availableTools.filter(tool => required.has(tool.function?.name))
    const result = await abortable(() => modelCall(messages, { tools: pendingTools.length ? pendingTools : availableTools, ...(pendingTools.length ? { toolChoice: 'required' } : {}), maxOutputTokens: budget, signal, reasoningEffort, ...(contentHandler ? { onContent: contentHandler } : {}), ...(format ? { responseFormat: format } : {}) }), signal)
