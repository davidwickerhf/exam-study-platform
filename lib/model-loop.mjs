...
export async function callModel(messages, { tools = [], maxOutputTokens = 1400, responseFormat = null, signal, reasoningEffort = null, onContent } = {}) {
  ...
  if (!response.ok) {
    ...
    throw new ModelError(failure.message, response.status, failure)
  }
  if (onContent && response.headers.get('content-type')?.includes('text/event-stream')) return readModelStream(response, onContent)
...
export async function runToolLoop({
  ...
  const complete = async (availableTools, budget) => {
    onModelStart()
    const result = await abortable(() => modelCall(messages, { tools: availableTools, maxOutputTokens: budget, signal, reasoningEffort, ...(onContent ? { onContent } : {}), ...(responseFormat ? { responseFormat } : {}) }), signal)
