...
export async function runToolLoop({
  ...
  const complete = async (availableTools, budget) => {
    onModelStart()
    const result = await abortable(() => modelCall(messages, { tools: availableTools, maxOutputTokens: budget, signal, reasoningEffort, ...(onContent && !reviewAnswer ? { onContent } : {}), ...(responseFormat ? { responseFormat } : {}) }), signal)
  ...
  if (reviewAnswer?.(message.content)) throw new ModelError('Tutor could not verify the sources needed for this answer. Please retry your question.')
  if (reviewAnswer) onContent?.(message.content)
  remember({ role: 'assistant', content: message.content })
