...
// The HTTP driver carries no timeout of its own, so a stalled request to Neon
// hangs whichever application request issued it — observed while uploading
// large course sources, where a single chunk stopped forever and took the whole
// upload with it. Bound every statement instead.
//
// Deliberately no automatic retry here: a timed-out statement may still have
// been executed, and silently repeating a non-idempotent INSERT is worse than a
// clear failure. Callers that are safe to repeat (the resumable chunk upload)
// retry at their own level.
export const DATABASE_QUERY_TIMEOUT_MS = Number(process.env.DATABASE_QUERY_TIMEOUT_MS || 25_000)

if (process.env.DATABASE_URL) {
  neonConfig.fetchFunction = (input, init = {}) => fetch(input, { ...init, signal: AbortSignal.timeout(DATABASE_QUERY_TIMEOUT_MS) })
}
