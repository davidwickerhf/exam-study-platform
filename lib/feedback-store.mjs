...
export async function submitFeedback({ draftId, revision, confirmed }) {
  ...
  const issueId = `fi-${fingerprint.slice(0, 32)}`,
    title = p.note
      ? p.note.split(/\n/)[0].slice(0, 96)
      : `${p.category.replaceAll("-", " ")} · ${p.subject.kind}${p.subject.courseCode ? " · " + p.subject.courseCode : ""}`;
...
const publicRow = (row) => ({
  ...
  title: row.note ? row.note.split(/\n/)[0].slice(0,96) : `${row.category.replaceAll("-"," ")} · ${row.subject.kind}`,
...
export async function readFeedback(
  ...
  return {
    ...
    ...(admin
      ? {
          reviewJobs:
            await sql`SELECT id,status,attempts FROM feedback_jobs WHERE report_id=${reportId}`,
          canRetryReview: (await feedbackRole()).includes("reliability"),
        }
      : {}),
...
export async function retryFeedbackReview(reportId, jobId) {
  await requireFeedbackRole("reliability");
  const sql = db();
  const rows =
    await sql`WITH changed AS (UPDATE feedback_jobs SET status='pending',attempts=0,run_after=now(),lease_until=null,lease_token=null WHERE id=${jobId} AND report_id=${reportId} AND status='failed' RETURNING id), logged AS (INSERT INTO feedback_admin_audit(id,actor_id,report_id,action) SELECT ${id()},${currentUserId()},${reportId},'review-job-retried' FROM changed) SELECT id FROM changed`;
