...
function observedEvidence(draft = {}, sourceLabel = 'Uploaded source', { kind = 'auto' } = {}) {
  const observed = new Map()
  const add = ({ code, name, evidence, event = null, historical = false }) => {
    ...
    const current = observed.get(key) || { key, code: normalizedCode, name: clean(name || normalizedCode, 200), evidence: [], events: [], historical }
    if (!current.name && name) current.name = clean(name, 200)
    // If any row is current, the combined identity is current. This is how a
    // failed historical attempt and a later carry-over remain one course.
    current.historical = current.historical && historical
  ...
  for (const course of draft?.courses || []) {
    add({ code: course.code, name: course.name, evidence: `${sourceLabel}: course record`, historical: kind === 'transcript' || course.programmeRequirement === 'historical' })
  }
...
function mergeObservedWithPlan(observed, courses, { codeStrict = false } = {}) {
  ...
  return observed.map((item) => {
    ...
    const nameMatch = maps.byName.get(normalizedCourseName(item.name)) || null
    const match = (item.code && maps.byCode.get(item.code)) || (!item.code || !codeStrict || !normalizedCourseCode(nameMatch?.code) ? nameMatch : null) || null
...
export function reconcileAcademicSource(workspace, draft, { kind = 'auto', sourceLabel = 'Uploaded source' } = {}) {
  ...
  const observed = mergeObservedWithPlan(observedEvidence(draft, sourceLabel, { kind }), courses, { codeStrict: kind === 'transcript' || kind === 'academic-overview' })
  ...
  const unmatched = observed.filter((item) => !item.course)
  const unselected = unmatched.filter((item) => !item.historical).map((item) => ({
    key: item.key,
    code: item.code,
  ...
  const historical = unmatched.filter((item) => item.historical).map((item) => ({
    key: item.key,
    code: item.code,
    name: item.name || item.code || 'Unidentified course',
    evidence: item.evidence
  ...
  return {
    kind,
    sourceLabel,
    status: unselected.length ? (kind === 'academic-overview' ? 'review' : 'attention') : missing.length ? 'review' : observed.length ? 'aligned' : 'not-applicable',
