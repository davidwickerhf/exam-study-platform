...
function scopes(value='read') {
  ...
  if(!result.length||result.some(scope=>!SUPPORTED_SCOPES.includes(scope)))fail('invalid_scope','Supported scopes are read and write.')
...
export function createMcpOAuth({store,origin}) {
  ...
  return {
    ...
    async register(body) {
      ...
      const granted=scopes(body.scope||SUPPORTED_SCOPES.join(' '))
