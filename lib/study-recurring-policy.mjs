...
function guidePolicyEnabled(row,automation) {
  return automation.kind==='maintenance' ? row?.maintainedGuides?.some(g=>g.enabled&&!!automation.enrollmentId&&g.enrollmentId===automation.enrollmentId&&g.versionId===automation.versionId&&JSON.stringify(g.moduleRefs)===JSON.stringify(automation.candidate.moduleRefs)) : row?.settings.enabled
}
...
export async function automaticGuideAllowed(automation, execution) {
  ...
  if(!await recurringEnabled('guides') || !guidePolicyEnabled(row,automation) || row.programmeId!==await activeProgrammeId())return false
  if(sql) {
    const bindings=[...new Set(automation.candidate?.moduleRefs?.map(r=>r.bindingId) || [automation.candidate?.bindingId || ''])]
    for(const binding of bindings) {
      const [permission]=await sql`SELECT a.binding_id FROM canvas_corpus_access a JOIN canvas_course_bindings b ON b.id=a.binding_id JOIN canvas_corpus_permissions p ON p.user_id=a.user_id AND p.origin=b.origin WHERE a.user_id=${currentUserId()} AND a.binding_id=${binding} AND NOT a.sync_paused AND a.auto_refresh AND p.collection_enabled AND p.refresh_enabled AND p.study_status='studying'`
