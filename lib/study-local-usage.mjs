...
export async function courseLocalUsage(courseCode) {
  // Spend and review-budget accounting must see the whole-course parent: it
  // holds the receipts, the budget and the planning phase for every managed
  // guide it published. Reading a bundle only through its children would report
  // the course as free.
  const versions=(await listOwnStudyVersions(undefined,{includeBundleParents:true})).filter(v=>!courseCode||v.course.courseCode===courseCode),seen=new Set(),coursePlans=[]
