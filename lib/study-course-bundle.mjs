...
import {outlineSchema,outlineCandidates,resolveOutlineGroups,outlinePrompt,outlineRejection,conceptKey,digest,StudyVersionError} from './study-version-content.mjs'
...
export function resolveCourseBundle(result,maps){
 ...
 if(groups.length>40)throw outlineRejection('The course plan exceeds 40 chapters. Consolidate related concepts before authoring.',
  [{kind:'too-many-chapters',count:groups.length,detail:`The plan has ${groups.length} chapters; at most 40 are allowed across the whole bundle. Consolidate related concepts into fewer chapters without dropping any mapped ref.`}],422)
