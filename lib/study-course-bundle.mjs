...
export function resolveCourseBundle(result,maps,policy){
 // Exact duplicate placement has one deterministic repair: retain the first
 // declared owner and remove later copies. No concept is dropped—the retained
 // ref still resolves to the same immutable mapped evidence—and unknown refs
 // are still rejected below. This avoids spending outline corrections on a
 // one-owner invariant the server can enforce without semantic judgment.
 const claimed=new Map(),deduplicatedRefs=[]
 const groups=result.guides.flatMap(guide=>guide.topics.map(({supportingRefs=[],role,...topic})=>{
  const keep=(refs,kind)=>refs.filter(ref=>{
   const owner=claimed.get(ref)
   if(owner){deduplicatedRefs.push({ref,keptGuideId:owner.guideId,keptChapterId:owner.chapterId,removedGuideId:guide.id,removedChapterId:topic.id,removedKind:kind});return false}
   claimed.set(ref,{guideId:guide.id,chapterId:topic.id});return true
  })
  const core=keep(topic.topicRefs,'core'),supporting=keep(supportingRefs,'supporting')
  return {...topic,topicRefs:[...core,...supporting],guideId:guide.id,role:role||'core',coreRefs:core.length}
 }))
 const emptied=groups.filter(group=>!group.topicRefs.length)
 if(emptied.length)throw outlineRejection('Removing duplicate mapped refs would leave a chapter with no teaching responsibility.',emptied.map(group=>({kind:'duplicate-only-chapter',guideId:group.guideId,chapterId:group.id,detail:`Chapter “${group.id}” contains only refs already owned by an earlier chapter. Remove the empty duplicate chapter or give it a distinct mapped responsibility.`})),422)
 ...
 if(!policy)return {...resolved,deduplicatedRefs,guides:result.guides.map(({id,title})=>({id,title})),topics:resolved.topics.map((topic,index)=>({...topic,guideId:groups[index].guideId}))}
 assertExclusionsNotTaught(excluded,owners)
 return {...resolved,deduplicatedRefs,gaps:[...resolved.gaps,...scopeExclusionGaps(excluded)],excluded,guides:result.guides.map(({id,title})=>({id,title})),
