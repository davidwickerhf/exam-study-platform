...
export async function extractOffice(bytes, extension) {
  ...
  try {
    ...
    const { stdout } = await execFileAsync('python3', [new URL('../scripts/extract-course-text.py', import.meta.url).pathname, file, 'presentation.pptx'], { maxBuffer: 64 * 1024 * 1024, timeout: 90000 })
    const result = JSON.parse(stdout)
    if (result.status === 'failed') throw new Error(result.error || 'Presentation extraction failed; original preserved.')
    const pages = result.pages || []
    return { text: result.text || '', pages, outline: pages.map((page) => page.text.split('\n').find(Boolean)).filter(Boolean).slice(0, 300) }
  } finally {
