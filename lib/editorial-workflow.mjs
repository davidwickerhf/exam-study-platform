...
async function processGenerationJob(job, edition, generate) {
  ...
  const generated = parseGeneratedJson(await generate(generationPrompt(job.job_type, edition, topic, rows), { maxOutputTokens: job.job_type === 'study-pages' ? 12000 : 8000, stage: 'draft' }))
...
export async function queueEditorialGeneration(editionIdRaw, payload = {}) {
  ...
  const topics = await sql`SELECT * FROM editorial_topic_nodes WHERE edition_id=${edition.id} AND NOT (metadata ? 'retired') ORDER BY position`
