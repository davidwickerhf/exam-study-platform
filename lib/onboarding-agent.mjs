...
export const ONBOARDING_STEPS = Object.freeze(['programme', 'electives', 'record', 'calendar', 'timetable', 'canvas'])
...
export const ONBOARDING_TOOLS = Object.freeze([
  ...
  },
  {
    ...
    function: {
      name: 'request_upload',
      description: 'Show the student the upload control for their Academic Work overview from the student portal (My Study). It is read for its results and never stored. Use this instead of describing how to attach a file; the control appears in the conversation.',
      parameters: { type: 'object', properties: {} }
    }
  },
  {
  ...
  },
  {
    ...
    function: {
      ...
      parameters: {
        type: 'object',
        properties: {
          step: { type: 'string', enum: ['electives', 'record', 'calendar', 'timetable', 'canvas'] },
          reason: { type: 'string' }
        },
        required: ['step']
      }
...
export function onboardingSystemPrompt({ name = null, state = null } = {}) {
  return [
    ...
    '',
    'The steps, in order: the programme (required), this period\'s electives, their academic record, the academic calendar, their timetable, and Canvas.',
    ...
    '- Academic record: do not ask whether to reveal an upload control. Call request_upload as soon as this step is reached; the visible control already explains why it matters and includes a skip action.',
    ...
    state ? `Already done, and not to be asked about again: ${Object.entries(state).filter(([, value]) => value === true).map(([key]) => key).join(', ') || 'nothing yet'}. Treat each as settled fact and act on the step under discussion.` : ''
  ].filter(Boolean).join('\n')
}
