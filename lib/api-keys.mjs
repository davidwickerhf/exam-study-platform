...
export async function listApiKeys() {
  if (sql) {
    const rows = await sql`SELECT id, name, prefix, scopes, created_at, last_used_at, revoked_at FROM api_keys WHERE user_id = ${userId()} ORDER BY created_at DESC`
...
export async function createApiKey({ name, scopes } = {}) {
  ...
  const owner = userId()
  const granted = normalizeScopes(scopes, { admin: isAdminUser(owner) })
  ...
  const row = { id: randomUUID(), name: label, prefix: secret.slice(0, KEY_PREFIX.length + 6), keyHash: hashKey(secret), scopes: granted, createdAt: new Date().toISOString(), lastUsedAt: null, revokedAt: null }
  if (sql) {
    await sql`INSERT INTO api_keys (id, user_id, name, prefix, key_hash, scopes, created_at) VALUES (${row.id}, ${owner}, ${row.name}, ${row.prefix}, ${row.keyHash}, ${row.scopes}, now())`
  } else {
...
export async function authenticateApiKey(secret) {
  ...
  if (sql) {
    const rows = await sql`UPDATE api_keys SET last_used_at = now() WHERE key_hash = ${keyHash} AND revoked_at IS NULL RETURNING id, user_id, scopes`
  ...
  for (const dir of await readdir(root)) {
    const match = await withRequestContext({ userId: dir }, async () => (await localRows('api_keys')).find((row) => row.keyHash === keyHash && !row.revokedAt))
