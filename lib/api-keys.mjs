...
// Revalidate a short-lived file capability without retaining the account's key.
// Its user/key IDs come only from our server-side download ticket record.
export async function apiKeyIdentityById({ keyId, userId: owner }) {
  if (sql) {
    const [row] = await sql`SELECT id, user_id, scopes FROM api_keys WHERE id=${keyId} AND user_id=${owner} AND revoked_at IS NULL AND (expires_at IS NULL OR expires_at>now())`
    return row ? { keyId: row.id, userId: row.user_id, scopes: row.scopes } : null
  }
  const { withRequestContext } = await import('./request-context.mjs')
  const row = await withRequestContext({ userId: owner }, async () => (await localRows('api_keys')).find(row => row.id === keyId && !row.revokedAt && (!row.expiresAt || row.expiresAt > new Date().toISOString())))
  return row && (!row.userId || row.userId === owner) ? { keyId: row.id, userId: owner, scopes: row.scopes } : null
}
