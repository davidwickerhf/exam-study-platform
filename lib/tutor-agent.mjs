...
export const TUTOR_TOOLS = Object.freeze([
  ...
  },
  {
    ...
    function: {
      ...
      description: 'The student\'s private exam scenario: which sitting each course is planned for, expected grades, what-if outcomes, available sessions, registration dates, and whether progression gates remain safe. Recorded academic facts are labelled separately. Use for any question about their plan, resits, deferrals, expected grades, or how a decision affects degree progress.',
      parameters: { type: 'object', properties: {} }
    }
  },
  {
...
export function tutorSystemPrompt({ memory, briefing, planner, context, today, now = new Date() }) {
  ...
  return [
    ...
    'For course explanations, syllabus claims, exercises, or assignment requirements, call search_study_sources. For absences, attendance, group work, or assessment consequences, also call get_course_obligations and get_schedule. For resits, deferrals, expected grades, or degree-plan consequences, call get_planning_context.',
...
function sourceEvidence(chunk, index) {
  ...
  const location = [sourcePath, page ? `p. ${page}` : null].filter(Boolean).join(', ')
  return {
    id: clean(`${chunk.corpus || 'course'}:${chunk.assetId || chunk.courseId || course || 'source'}:${sourcePath}:${page || index}`, 800),
    sourceType: clean(chunk.sourceType, 60) || (chunk.corpus === 'canvas' ? 'Canvas course material' : 'Maintained course material'),
    title: sourcePath.split('/').pop() || sourcePath,
    course,
    location,
    excerpt: clean(chunk.content, 700),
    url: clean(chunk.materialUrl, 600) || null,
    status: chunk.current === false ? 'historical' : 'current',
  ...
  async search_study_sources({ query, courseCode = '', sourceType = '' }, runtime = {}) {
    ...
    const groups = await Promise.all(targets.map(async (course) => {
      const [published, canvas] = await Promise.all([
        retrieveCourseContent({ query, courseId: course.id, sourcePath: lens.courseId === course.id ? lens.sourcePath : null, limit: 8 }).catch(() => []),
        retrieveCanvasCorpus({ query, courseCode: course.code, sourceType, includeHistorical: true, limit: 8 }).catch(() => [])
      ])
      return [
    ...
    const privateSources = await searchTutorAttachments({ query, courseCode: wanted, attachmentIds: lens.attachmentIds, limit: 10 }).catch(() => [])
    ...
    const chunks = [...groups.flat(), ...privateChunks].sort((left, right) => Number(right.score || 0) - Number(left.score || 0)).slice(0, 12)
    return {
...
export async function runTutorTurn(conversation, { message, context = {} }) {
  ...
  const [memory, briefing, academicState] = await Promise.all([
    readTutorMemory(),
  ...
  const uniqueEvidence = [...new Map(evidence.filter((item) => item?.id).map((item) => [item.id, item])).values()].slice(0, 30)
