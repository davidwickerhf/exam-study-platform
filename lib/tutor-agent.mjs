  ...
  async search_study_sources({ query, courseCode = '', sourceType = '' }, runtime = {}) {
    ...
    const groups = await Promise.all(targets.map(async (course) => {
      const [published, canvas] = await Promise.all([
        ...
        retrieveCanvasCorpus({ query, courseCode: course.code, sourceType, includeHistorical: !staffLookup && !['syllabus', 'requirements'].includes(sourceType), limit: perCorpusLimit }).catch(() => [])
      ])
      return [
