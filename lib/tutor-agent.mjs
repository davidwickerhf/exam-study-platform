...
export function tutorSystemPrompt({ memory, briefing, planner, context, today, now = new Date() }) {
  ...
  return [
    ...
    'For course explanations, syllabus claims, exercises, or assignment requirements, call search_study_sources. For a teaching-staff question, call get_course_staff exactly once and distinguish official coordinator from current teaching roles. For absences, attendance, group work, or assessment consequences, also call get_course_obligations and get_schedule. For resits, deferrals, expected grades, or degree-plan consequences, call get_planning_context.',
