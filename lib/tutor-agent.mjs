...
export async function runTutorTurn(conversation, { message, context = {}, sourceOptions = {}, signal = AbortSignal.timeout(180_000), onProgress = () => {} }) {
  ...
  const evidence = [...conversationEvidence(pastConversations), ...(study ? [{ id: `study:${study.versionId}:${study.revisionId}:${study.chapterId}`, sourceType:'personal-study-chapter', title: study.chapterName, course: study.course.courseCode, location:'Saved personal study revision', excerpt: study.lesson[0]?.text || '', url: study.url }] : [])]
