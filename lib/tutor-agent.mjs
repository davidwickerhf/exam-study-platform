...
export function tutorSystemPrompt({ memory, briefing, context, today, now = new Date() }) {
  ...
  return [
    ...
    'For course explanations, syllabus claims, exercises, or assignment requirements, call search_study_sources. For absences, attendance, group work, or assessment consequences, also call get_course_obligations and get_schedule.',
    'Never change the student\'s data directly. Practice sets, calendar entries, and remembered plans are proposals. Explain what you recommend, call the matching propose_* tool, and tell the student it is waiting for their approval.',
...
export async function runTutorTurn(conversation, { message, context = {} }) {
  ...
  const messages = [
    { role: 'system', content: tutorSystemPrompt({ memory, briefing: compact, context: lens, today: briefing?.today || new Date().toISOString().slice(0, 10), now: new Date() }) },
