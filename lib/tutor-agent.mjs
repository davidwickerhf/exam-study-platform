...
export function tutorSystemPrompt({ memory, briefing, today, now = new Date() }) {
...
export async function runTutorTurn(conversation, { message }) {
  ...
  const messages = [
    { role: 'system', content: tutorSystemPrompt({ memory, briefing: compact, today: briefing?.today || new Date().toISOString().slice(0, 10), now: new Date() }) },
