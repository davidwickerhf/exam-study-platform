...
export const TUTOR_HANDLERS = {
  ...
  async get_course_obligations({ courseCode = '' } = {}, runtime = {}) {
    ...
    return { obligations, evidence: [...new Map(evidence.map(item => [item.id, item])).values()], note: obligations.some((item) => item.status !== 'not-found') ? null : 'No verified course obligations were found in the connected sources.' }
  },
...
export async function runTutorTurn(conversation, { message, context = {}, signal = AbortSignal.timeout(180_000) }) {
  ...
  const [memory, academicState] = await abortable(() => Promise.all([
    readTutorMemory(),
    readAcademicState().catch(() => null)
  ]), signal)
  const runTool = createTutorToolRunner({ context: lens })

  const history = (conversation.messages || []).slice(-30)
  const messages = [
    { role: 'system', content: tutorSystemPrompt({ memory, briefing: null, planner: academicState?.workspace ? planningContext(academicState.workspace) : null, context: lens, today: new Date().toISOString().slice(0, 10), now: new Date() }) },
