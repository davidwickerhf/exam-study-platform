...
function sourceEvidence(chunk, index) {
  ...
  const location = [sourcePath, page ? `p. ${page}` : null, edition, policy ? clean(chunk.academicYear, 30) : null, policy ? clean(chunk.authority, 180) : null].filter(Boolean).join(', ')
  return {
    ...
    status: chunk.current === false ? 'historical' : policy && chunk.visibility === 'programme' ? 'programme-only' : editionLabels.length > 1 ? 'versioned' : 'current',
