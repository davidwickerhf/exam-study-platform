...
export const iotTeachingCases = [
  {
    ...
    transfer: ['A sensor produces a pulse from 12 to 18 ms. Samples occur at 0, 10, 20 and 30 ms. Would reducing the interval from 100 to 10 ms guarantee this pulse is seen?', 'No. The samples immediately surrounding this pulse are at 10 and 20 ms, when the pulse is absent. A shorter sampling interval reduces the size of gaps but this six-millisecond pulse still fits within one. A hardware latch or another event-capture mechanism would change the assumptions.'],
  ...
  },
  {
    ...
    transfer: ['A network task wakes while a higher-priority task still needs 12 ms of CPU time. There is no ISR or other work. Does waking the network task imply immediate execution?', 'No. Waking changes eligibility, but the higher-priority Ready task keeps the CPU until its remaining 12 ms finishes, assuming it does not block sooner. The network task can then run. The same Ready-versus-Running distinction applies even without a button or interrupt.'],
