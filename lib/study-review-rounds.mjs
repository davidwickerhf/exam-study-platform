...
export function reviewBaseline(chapter, located) {
  ...
  return {
    ...
    objectiveChecks: Object.fromEntries((review?.objectives || []).filter(row => row.adequate).map(row => [row.objectiveId, row])),
  }
}
...
// What a question's verdict rests on besides its own text: the teaching of its
// objectives, its plan entries, the follow-ups it links to and, for a
// remediation question, the diagnosed mistakes that link to it. A correction
// that changes any of these can invalidate an otherwise unchanged question.
export function questionDependencyKeys(chapter, q) {
  if (!q) return []
  const objectives = new Set(q.objectiveIds || [])
  return [
    ...chapter.sections.filter(section => section.objectiveIds?.some(id => objectives.has(id))).map(section => `section:${section.id}`),
    ...[...objectives].map(id => `objective:${id}`),
    ...(q.misconceptions || []).map(m => `question:${m.followUpKey}`),
    ...(q.practiceStage === 'remediation' ? chapter.questions.filter(other => other.key !== q.key && other.misconceptions?.some(m => m.followUpKey === q.key)).map(other => `question:${other.key}`) : [])
  ]
}
...
// (recorded as carried:'accepted-unchanged') only when everything that item
// depends on is unchanged too; a question whose objective teaching, plan
// entry, follow-up target or incoming diagnostic link changed keeps its error.
// A transfer or follow-up check on an accepted question whose own
// dependencies are unchanged keeps the baseline's passing row. Objective- and
// chapter-level findings, and anything on changed content, are left exactly
// as reviewed.
export function applyReviewFocus(chapter, review) {
  ...
  const dependenciesUnchanged = key => !key.startsWith('question:') || questionDependencyKeys(chapter, chapter.questions.find(q => `question:${q.key}` === key)).every(dependency => !changed.has(dependency))
  const issues = review.issues.map(issue => {
    const key = itemKeyForTopic(chapter, issue.topicId)
    return key && accepted.has(key) && issue.severity === 'error' && dependenciesUnchanged(key) ? {...issue, severity: 'warning', carried: 'accepted-unchanged'} : issue
  })
  ...
  const keep = (rows, saved, dependencies) => {
    ...
    const present=new Set(kept.map(row=>row.questionKey))
    for(const [key,row] of Object.entries(saved || {})){
      const q=question(key)
      if(q && !present.has(key) && accepted.has(`question:${key}`) && dependencies(q).every(dependency=>!changed.has(dependency)))kept.push({...row})
    }
    return kept
  }
