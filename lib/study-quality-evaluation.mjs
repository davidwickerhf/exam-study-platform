...
export async function createQualityEvaluation(input, { platform = {}, sourceOptions = {} } = {}) {
  ...
  const row = { id: `sqe-${randomUUID()}`, revision: randomUUID(), scenario, course, snapshot, topic, billing,
    contractVersion: 5, status: 'pending', stage: 0, createdAt: new Date().toISOString(), checks: [], calls: [], generated: null,
...
export async function stepQualityEvaluation(id, expectedRevision, { generate, sourceOptions = {} } = {}) {
  ...
  delete next.error
  try {
    if (row.contractVersion !== 5) throw new StudyVersionError('This evaluation used an older teaching contract. Start a new evaluation; its saved results are retained.', 409)
    ...
    if (step.kind === 'plan') {
      ...
      next.generated = { ...assertEvidence(parsed, row.snapshot.chunks), teachingPlan: row.teachingPlan }
      ...
      next.checks.push({ name: step.kind === 'evidence' ? 'Independent evidence review' : 'Reviewer detects wrong solution, misleading visual and outdated exam rules', passed, issues })
    } else next.checks.push(pedagogicalEvaluationCheck(step, parsed))
    next.stage++
...
export async function recheckQualityEvaluation(id, expectedRevision, options = {}) {
  ...
  const row = { ...structuredClone(original), id: `sqe-${randomUUID()}`, revision: randomUUID(),
    reusedFrom: id, status: 'pending', stage: 2, createdAt: new Date().toISOString(),
