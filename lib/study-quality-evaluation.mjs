import { acceptPedagogicalReview } from './study-pedagogical-review.mjs'
import { acceptFactualReview, nextFactualReview, factualAuditIssues } from './study-factual-review.mjs'
import { deriveObjectiveCoverage } from './study-pedagogy.mjs'
import { evaluationStep, pedagogicalEvaluationCheck } from './study-evaluation-steps.mjs'
...
import { resolveStudyBilling, estimateStudyCall, studyModelCost, StudyBudgetError } from './study-ai-budget.mjs'
...
export async function createQualityEvaluation(input, { platform = {}, sourceOptions = {} } = {}) {
  ...
  const row = { id: `sqe-${randomUUID()}`, revision: randomUUID(), scenario, course, snapshot, topic, billing,
    contractVersion: 6, status: 'pending', stage: 0, createdAt: new Date().toISOString(), checks: [], calls: [], generated: null,
  ...
  if(row.reusedFrom){delete row.generated.factualAudit;delete row.generated.evidenceReview;delete row.corrupted}
...
export async function stepQualityEvaluation(id, expectedRevision, { generate, sourceOptions = {} } = {}) {
  ...
  delete next.error
  try {
    if (row.contractVersion !== 6) throw new StudyVersionError('This evaluation used an older teaching contract. Start a new evaluation; its saved results are retained.', 409)
    const step = evaluationStep(row), {prompt} = step
    const maxOutputTokens = step.tokens
    const result = await generate(prompt, { responseSchema: step.responseSchema || studyResponseSchema(step.schema, row.snapshot.chunks.map(c => c.id)), billing: row.billing, jobKey: row.id, maxOutputTokens, ...(row.stage >= 2 ? { reasoningEffort: 'medium' } : {}),
      usageMetadata: { chapterId: row.topic.id, phase: step.kind } })
    const usage = result.usage || null, price = STUDY_MODELS[row.billing.model]
    const chargedMicros = usage && !usage.estimated ? studyModelCost(row.billing.model, usage.inputTokens, usage.outputTokens, usage) : estimateStudyCall(prompt, maxOutputTokens, row.billing.model).micros
    next.calls.push({ stage: row.stage, usage, chargedUsd: chargedMicros / 1000000, conservative: !usage || usage.estimated })
    const parsed = parseStudyJson(result.text, step.schema)
    let advance=true
    if (step.kind === 'plan') {
      next.teachingPlan = assertEvidence(parsed, row.snapshot.chunks)
      next.checks.push({ name: 'Objective/evidence plan before drafting', passed: true, issues: [] })
    } else if (step.kind === 'lesson') {
      next.generated = deriveObjectiveCoverage({ ...assertEvidence(parsed, row.snapshot.chunks), teachingPlan: row.teachingPlan })
      const issues = studyLessonQuality(next.generated, row.snapshot.chunks)
      next.checks.push({ name: 'Format, citations, objective mapping, solutions and arithmetic', passed: !issues.length, issues })
    } else if (['evidence', 'corruption'].includes(step.kind)) {
      acceptFactualReview(step.chapter,step.factual,parsed)
      if(step.kind==='evidence')next.generated=step.chapter
      else next.corrupted=step.chapter
      advance=!nextFactualReview(row.course,row.snapshot.sources,row.snapshot.chunks,step.chapter)
      if(advance){
        const issues=factualAuditIssues(step.chapter)
        const passed=step.kind==='evidence' ? !issues.some(i=>i.severity==='error') : reviewerCatchesKnownErrors(issues)
        next.checks.push({name:step.kind==='evidence'?'Independent evidence review':'Reviewer detects wrong solutions, incomplete ranges, misleading visuals and outdated exam rules',passed,issues})
      }
    } else {
      const review=acceptPedagogicalReview(step.chapter,step.pedagogical,parsed)
      if(row.stage===3)next.generated=step.chapter
      else {next.pedagogyFixtures ||= {}; next.pedagogyFixtures[row.stage]=step.chapter}
      advance=Boolean(review)
      if(review)next.checks.push(pedagogicalEvaluationCheck(step,review))
    }
    if(advance)next.stage++
    next.status = next.stage >= (row.scenario === 'reference' ? 7 : 4) ? 'complete' : 'pending'
    if (row.scenario === 'sources' && !await studySourcesStillAvailable(row.snapshot, row.course, sourceOptions)) throw new StudyVersionError('Evaluation source access was withdrawn.', 410)
  } catch (error) {
...
// Recheck the exact same generated artifact after changing a reviewer contract.
// Keep the original results and costs intact; never regenerate to chase a pass.
export async function recheckQualityEvaluation(id, expectedRevision, options = {}) {
  browserOnly()
  const original = await own(id)
  await readQualityEvaluation(id, options)
  if (original.revision !== expectedRevision || original.status === 'running' || !original.generated)
    throw new StudyVersionError('Wait for the current check to finish and reload this evaluation.', 409)
  const row = { ...structuredClone(original), id: `sqe-${randomUUID()}`, revision: randomUUID(),
    contractVersion:6, reusedFrom: id, status: 'pending', stage: 2, createdAt: new Date().toISOString(),
    checks: original.checks.slice(0, 2), calls: [], error: undefined, startedAt: undefined }
  if(row.reusedFrom){delete row.generated.factualAudit;delete row.generated.evidenceReview;delete row.corrupted}
  await compareAndSwapDocument(namespace, row.id, row, null)
  return report(row)
}
