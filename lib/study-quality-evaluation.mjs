...
import { StudyVersionError, lessonPrompt, reviewPrompt, lessonSchema, reviewSchema, parseStudyJson, assertEvidence } from './study-version-content.mjs'
...
function report(row) {
  ...
  return { ...safe, billing: { source: billing.source, model: billing.model, maxJobUsd: billing.maxJobUsd } }
}
...
export async function createQualityEvaluation(input, { platform = {}, sourceOptions = {} } = {}) {
  ...
  if (!Number.isFinite(cap) || cap < 0.05 || cap > 1) throw new StudyVersionError('Evaluation spending cap must be between $0.05 and $1.')
...
export async function stepQualityEvaluation(id, expectedRevision, { generate, sourceOptions = {} } = {}) {
  ...
  try {
    ...
    const result = await generate(prompt, { billing: row.billing, jobKey: row.id, maxOutputTokens,
