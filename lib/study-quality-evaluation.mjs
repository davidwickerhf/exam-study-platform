...
export async function stepQualityEvaluation(id, expectedRevision, { generate, sourceOptions = {} } = {}) {
  ...
  try {
    ...
    const result = await generate(prompt, { responseSchema: studyResponseSchema(step.schema, row.snapshot.chunks.map(c => c.id)), billing: row.billing, jobKey: row.id, maxOutputTokens, ...(row.stage >= 2 ? { reasoningEffort: 'medium' } : {}),
