...
export async function stepQualityEvaluation(id, expectedRevision, { generate, sourceOptions = {} } = {}) {
  ...
  try {
    ...
    const result = await generate(prompt, { responseSchema: studyResponseSchema(row.stage === 0 ? lessonSchema : reviewSchema, row.snapshot.chunks.map(c => c.id)), billing: row.billing, jobKey: row.id, maxOutputTokens,
