...
export async function listStudySources(
  ...
  const sources = materials
    ...
    .map((m) => ({
      ...
      announcement:m.sourceType==='announcements',
      url: m.url,
...
export async function readStudySourceSnapshot(course, keys, options = {}) {
  const sourceLimit=options.courseBundle===true?500:100,characterLimit=options.courseBundle===true?5000000:600000
  if (
    !Array.isArray(keys) ||
    !keys.length ||
    keys.length > sourceLimit ||
    new Set(keys).size !== keys.length
  )
    throw new StudyVersionError(`Select between 1 and ${sourceLimit} distinct sources.`)
  ...
  for (const source of selected) {
    ...
    if (!extracted.length) {
      if (options.retainTextlessOriginals === true && source.kind !== 'notes' && source.sha256) {
        sources.push({ ...source, textlessOriginal: true })
        continue
      }
      excluded.push({
        title: source.title,
    ...
    if (total > characterLimit)
      throw new StudyVersionError(
        `This selection exceeds ${characterLimit.toLocaleString('en-US')} characters. Select a smaller scope; no source was truncated.`
      )
    sources.push(source)
    chunks.push(...extracted)
  }
  if (!chunks.length && !(options.retainTextlessOriginals === true && sources.length))
    throw new StudyVersionError(
...
export async function studySourcesStillAvailable(snapshot, course, options) {
  ...
  // retain exact evidence privately; publishing rechecks matching digests. A
  // Canvas rename or move may change its source key without changing its bytes.
  return snapshot.sources.every((source) =>
    studySourceStillAvailable(source, available),
  )
}

export function studySourceStillAvailable(source, available) {
  return available.some(
    (current) =>
      current.key === source.key ||
      (source.sha256 && current.sha256 === source.sha256),
  )
}
