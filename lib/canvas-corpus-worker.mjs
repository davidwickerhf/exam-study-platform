import { recordCanvasSyncEvent } from './canvas-sync-log.mjs'
...
async function indexAsset({ editionId, assetId, sourcePath, extraction, assertActive, job }) {
  ...
  for (let offset = 0; offset < records.length; offset += 64) {
    await recordCanvasSyncEvent(job, { stage: 'indexing', message: 'Preparing search index batch.', item: sourcePath, completed: offset, total: records.length })
    const batch = records.slice(offset, offset + 64)
    let vectors = batch.map(() => null)
    if (embedding.configured) {
      try { vectors = await embedTexts(batch.map((record) => record.content)) } catch { await recordCanvasSyncEvent(job, { stage: 'indexing', level: 'warning', message: 'Semantic indexing unavailable for this batch. Text search remains available.', item: sourcePath }) }
    }
...
async function ingestFile({ binding, accountId, file, assertActive, job }) {
  ...
  await recordCanvasSyncEvent(job, { stage: 'extraction', message: 'Reading document text.', item: file.sourcePath })
  const extraction = await extracted(bytes, file.sourcePath)
  await recordCanvasSyncEvent(job, { stage: 'extraction', level: extraction.status === 'complete' ? 'info' : 'warning', message: extraction.status === 'complete' ? 'Document text read.' : extraction.status === 'unsupported' ? 'Text extraction is not supported for this file format.' : 'No readable text extracted.', item: file.sourcePath })
  assertActive()
  ...
  await recordCanvasSyncEvent(job, { stage: 'indexing', message: 'Preparing searchable passages.', item: file.sourcePath, total: expected })
  ...
  await recordCanvasSyncEvent(job, { stage: 'indexing', message: 'Search index ready.', item: file.sourcePath, completed: chunks, total: expected })
...
async function runCatalog(job, assertActive) {
  await recordCanvasSyncEvent(job, { stage: 'discovery', message: 'Checking enrolled Canvas course editions.' })
  ...
  assertActive()
  await recordCanvasSyncEvent(job, { stage: 'discovery', message: 'Course editions discovered. Scheduling material collection.', completed: courses.length })
...
async function runCourse(job, assertActive) {
  ...
  const scanPriorities = async () => {
    await recordCanvasSyncEvent(job, { stage: 'rules', message: 'Reading syllabus and introductory material for course requirements.' })
    try {
      const priorities = await scanCanvasPriorityEvidence({ bindingId: binding.id, accountId: job.user_id, force: job.payload?.force === true || job.payload?.stage === 'priorities', assertActive, onProgress: event => recordCanvasSyncEvent(job, event) })
      await recordCanvasSyncEvent(job, { stage: 'rules', level: priorities.status === 'needs-review' ? 'warning' : 'info', message: priorities.status === 'needs-review' ? 'Course-rule extraction needs review. Material remains searchable.' : 'Course-rule scan finished.', completed: priorities.candidates })
      return { status: priorities.status, candidates: priorities.candidates, conflicts: priorities.conflicts?.length || 0 }
    } catch (error) {
      assertActive()
      await recordCanvasSyncEvent(job, { stage: 'rules', level: 'warning', message: 'Course-rule extraction failed. Retry the scan from Canvas sync.' })
  ...
  try {
    const imported = await importCanvasCourse({ courseUrl: `${job.origin}/courses/${binding.canvas_course_id}/modules`, accessToken: token, outputFolder: root, maxFileBytes: MAX_FILE_BYTES, onProgress: async event => { assertActive(); await recordCanvasSyncEvent(job, event) } })
    const files = await filesUnder(root)
    await recordCanvasSyncEvent(job, { stage: 'download', level: imported.skipped.length ? 'warning' : 'info', message: imported.skipped.length ? `Collection finished with ${imported.skipped.length} skipped resources.` : 'Collection finished.', completed: imported.downloadedFiles })
    await recordCanvasSyncEvent(job, { stage: 'extraction', message: 'Processing collected files.', completed: 0, total: files.length })
    let total = 0
    const results = []
    for (const file of files) {
      ...
      const bytes = await readFile(file.path)
      total += bytes.length
      if (total > MAX_COURSE_BYTES) { results.push({ skipped: true, path: file.sourcePath, reason: 'course byte limit' }); await recordCanvasSyncEvent(job, { stage: 'extraction', level: 'warning', message: 'File skipped: course storage limit reached.', item: file.sourcePath }); continue }
      results.push(await ingestFile({ binding, accountId: job.user_id, file, assertActive, job }))
      await recordCanvasSyncEvent(job, { stage: 'indexing', message: 'Collected files processed.', completed: results.length, total: files.length })
    }
    assertActive()
...
export async function processNextCanvasCorpusJob() {
  if (!sql || process.env.VERCEL_ENV === 'preview') return null
  ...
  if (!job) return null
  try {
    const result = await withCanvasJobLease(assertActive => job.job_type === 'catalog' ? runCatalog(job, assertActive) : runCourse(job, assertActive), {
      renew: async () => {
