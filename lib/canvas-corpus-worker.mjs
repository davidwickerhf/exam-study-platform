...
async function runCourse(job, assertActive) {
  ...
  const scanPriorities = async () => {
    try {
      const priorities = await scanCanvasPriorityEvidence({ bindingId: binding.id, accountId: job.user_id, force: job.payload?.force === true || job.payload?.stage === 'priorities', assertActive })
