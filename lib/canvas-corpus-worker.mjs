...
import { isSupportedCanvasCourse, observeCanvasCorpusCourses, retireUnsupportedCanvasCorpusCourses, selectScheduledCanvasCourses, canvasRefreshCalendar } from './course-corpus.mjs'
...
export async function runCatalog(job, assertActive) {
  ...
  const timeContext = resolveAcademicTimeContext(canvasRefreshCalendar(programme || {}))
...
export async function scheduleDueRefreshes() {
  ...
  await sql`INSERT INTO canvas_sync_jobs (id, user_id, origin, binding_id, job_type, priority, payload)
    ...
    WHERE access.sync_paused=false AND access.auto_refresh=true AND binding.last_synced_at IS NOT NULL
    ...
    ON CONFLICT DO NOTHING`
  // Permissions are the durable opt-in. Poll current-course updates every
  // 30 minutes; only the latest current-period edition gets recurring material
  // refreshes every six hours. Historical editions remain available on demand. Partial unique indexes make this safe on every
  ...
  await sql`INSERT INTO canvas_sync_jobs (id, user_id, origin, job_type, priority, payload)
    ...
    WHERE p.collection_enabled=true
      ...
      AND NOT EXISTS (SELECT 1 FROM canvas_sync_jobs recent WHERE recent.user_id=p.user_id AND recent.origin=p.origin AND recent.job_type='catalog' AND recent.created_at > now() - interval '30 minutes')
    ON CONFLICT DO NOTHING`
  await sql`INSERT INTO canvas_sync_jobs (id, user_id, origin, binding_id, job_type, priority, payload)
    ...
    JOIN canvas_course_bindings binding ON binding.id=access.binding_id
    JOIN canvas_corpus_permissions permission ON permission.user_id=access.user_id AND permission.origin=binding.origin AND permission.collection_enabled=true
    WHERE access.sync_paused=false AND access.auto_refresh=true AND NOT EXISTS (SELECT 1 FROM canvas_sync_jobs active WHERE active.user_id=access.user_id AND active.binding_id=binding.id AND active.status IN ('pending','running'))
      AND NOT EXISTS (SELECT 1 FROM canvas_sync_jobs recent WHERE recent.user_id=access.user_id AND recent.binding_id=binding.id AND recent.job_type='course' AND recent.status='completed' AND coalesce(recent.payload->>'stage','')<>'priorities' AND recent.finished_at > now() - make_interval(hours => ${CANVAS_MATERIAL_REFRESH_HOURS}))
