...
async function runCourse(job) {
  ...
  if (/^Priority scan returned no JSON object\.?$/i.test(String(job.previous_error || '')) && binding.last_synced_at) {
    return {
...
export async function processNextCanvasCorpusJob() {
  ...
  const rows = await sql`WITH next_job AS (
    ...
    UPDATE canvas_sync_jobs job SET status='running', started_at=now(), finished_at=null, error=null, attempts=job.attempts+1
