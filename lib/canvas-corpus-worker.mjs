...
import { isSupportedCanvasCourse, observeCanvasCorpusCourses, retireUnsupportedCanvasCorpusCourses, selectScheduledCanvasCourses } from './course-corpus.mjs'
...
export async function runCatalog(job, assertActive) {
  ...
  const [programme] = await sql`SELECT planning_objectives FROM academic_programmes WHERE user_id=${job.user_id} AND is_active=true LIMIT 1`
  const timeContext = resolveAcademicTimeContext(programme?.planning_objectives?.academicPeriods || [])
