...
import { isTextFile, courseFileMediaType } from './course-file-types.mjs'
...
import { decorateCanvasCourses, fetchCanvasHub } from './canvas-hub.mjs'
import { withRequestContext } from './request-context.mjs'
import { canvasRefreshWindow } from './canvas-refresh-policy.mjs'
import { CANVAS_IMPORT_LIMITS, importCanvasCourse, listCanvasCourses } from './canvas-course-import.mjs'
import { isSupportedCanvasCourse, observeCanvasCorpusCourses, retireUnsupportedCanvasCorpusCourses, selectScheduledCanvasCourses, canvasRefreshCalendar, canvasCorpusPermission } from './course-corpus.mjs'
...
import { scanCanvasPriorityEvidence, PRIORITY_EXTRACTION_VERSION } from './priority-evidence.mjs'
...
export const CANVAS_MATERIAL_REFRESH_HOURS = Math.max(1, Number(process.env.CANVAS_MATERIAL_REFRESH_HOURS || 6))
...
export function sourceTypeForPath(sourcePath) {
  ...
  if (/announcement|discussion/.test(path)) return 'announcements'
...
export async function extracted(bytes, sourcePath) {
  ...
  if (['.xlsx', '.xls', '.ipynb', '.zip'].includes(extension) || (['.csv', '.tsv'].includes(extension) && bytes.length > 1024 * 1024)) {
  ...
  if ((isTextFile(sourcePath) || ['.csv', '.tsv'].includes(extension)) && !bytes.includes(0)) {
...
async function ingestFile({ binding, accountId, file, assertActive, job }) {
  ...
  const mediaType = courseFileMediaType(file.sourcePath) || MEDIA_TYPES.get(extension) || 'application/octet-stream'
  ...
  await sql`INSERT INTO canvas_source_snapshots
    ...
    VALUES (${snapshotId}, ${binding.id}, ${stored.id}, ${contributionId}, ${accountId}, ${permission.sharing_mode}, ${file.sourcePath}, ${file.sourcePath}, ${sourceTypeForPath(file.sourcePath)}, ${hash}, now(), ${JSON.stringify({ academicYear: binding.academic_year, courseCode: binding.course_code, extension: extension.slice(1) || null, locations: file.locations || [] })}::jsonb)
    ON CONFLICT (binding_id, resource_key, sha256, contributor_user_id) DO UPDATE SET last_seen_at=now(), retired_at=null,
      sharing_mode=excluded.sharing_mode, contribution_id=excluded.contribution_id, metadata=excluded.metadata`
  ...
  const paperJob = await indexedPaperJob(accountId,binding,file.sourcePath,hash)
  if(paperJob) await sql`INSERT INTO user_documents(user_id,namespace,document_key,value,updated_at) VALUES(${accountId},${PAPER_JOBS},${paperJob.id},${JSON.stringify(paperJob)}::jsonb,now()) ON CONFLICT DO NOTHING`
...
export async function runCatalog(job, assertActive, {commit=null}={}) {
  const permission = await canvasCorpusPermission({ accountId: job.user_id, origin: job.origin })
  if (job.payload?.scheduled && (!permission.refresh?.enabled || permission.refresh?.studyStatus === 'completed')) return { disabled: true }
  ...
  const [programme] = await sql`SELECT planning_objectives,template_programme_id,template_version_id FROM academic_programmes WHERE user_id=${job.user_id} AND is_active=true LIMIT 1`
  const refreshWindow = canvasRefreshWindow(canvasRefreshCalendar(programme || {}), { hasProgramme: Boolean(programme), studyStatus: permission.refresh?.studyStatus })
  const timeContext = { refreshWindow }
  await sql`UPDATE canvas_corpus_permissions SET refresh_policy=${JSON.stringify({ ...refreshWindow, checkedAt: new Date().toISOString() })}::jsonb WHERE user_id=${job.user_id} AND origin=${job.origin}`
  const result = await observeCanvasCorpusCourses({ accountId: job.user_id, origin: job.origin, courses, automatic: job.payload?.scheduled === true, refreshPolicy: true, timeContext, force: job.payload?.force === true, syncId: job.payload?.syncId || job.id })
  const latest = selectScheduledCanvasCourses(courses, timeContext || {})
  // Live metadata is small and more time-sensitive than unchanged file bytes.
  if (latest.length && (!job.payload?.scheduled || !permission.refresh?.updatesRefreshedAt || Date.now()-new Date(permission.refresh.updatesRefreshedAt).getTime() >= permission.refresh.updatesMinutes*60_000)) {
    const updates = await withRequestContext({ userId: job.user_id }, () => fetchCanvasHub({ origin: job.origin, token, scope: 'current', courseIds: latest.map(course => String(course.id)), parts: ['announcements', 'assignments'], days: 120, force: true }))
    // Reuse the existing dated announcement originals and lexical evidence,
    // even when the model-based priority extractor is paused.
    const bindings=await sql`SELECT b.* FROM canvas_course_bindings b JOIN canvas_corpus_access a ON a.binding_id=b.id WHERE b.origin=${job.origin} AND b.canvas_course_id=ANY(${latest.map(c=>String(c.id))}::text[]) AND a.user_id=${job.user_id} AND NOT a.sync_paused`
    for(const binding of bindings) {
      const notices=updates.announcements.filter(a=>String(a.courseId)===String(binding.canvas_course_id)).map(a=>({id:a.canvasId,title:a.title,message:a.html,posted_at:a.postedAt,user_name:a.author,published:true}))
      if(notices.length) await refreshPriorityAnnouncements({binding,accountId:job.user_id,assertActive,commit,rows:notices})
    }
    assertActive()
    job.log({ stage: 'discovery', level: updates.problems.length ? 'warning' : 'info', message: `Current-course updates refreshed: ${updates.announcements.length} announcements and ${updates.assignments.length} assignments.`, completed: latest.length })
    result.updateProblems = updates.problems
    if (!updates.problems.length) await sql`UPDATE canvas_corpus_permissions SET updates_refreshed_at=now() WHERE user_id=${job.user_id} AND origin=${job.origin}`
  }
  return result
}
...
async function runCourse(job, assertActive) {
  ...
  if (job.payload?.stage === 'freshness') return runCanvasFreshnessCheck(job, binding)
  const scanPriorities = async () => {
    if(!await withRequestContext({userId:job.user_id},()=>recurringEnabled('priorities')))return {status:'paused',reason:'Disabled in recurring pipeline settings.'}
  ...
  if (job.payload?.stage === 'priorities' || /^Priority scan returned no JSON object\.?$/i.test(String(job.previous_error || ''))) {
    return {
  ...
  try {
    ...
    let total = 0
    const results = []
    const placementManifest=JSON.parse(await readFile(imported.manifestPath,'utf8'))
    for (const file of files) {
      file.locations=materialLocations(placementManifest.resources || [],file.sourcePath)
      assertActive()
      const bytes = await readFile(file.path)
      total += bytes.length
    ...
    await sql`UPDATE canvas_course_bindings SET manifest_hash=${manifestHash}, last_synced_at=now(), next_sync_at=now() + make_interval(hours => ${CANVAS_MATERIAL_REFRESH_HOURS}), updated_at=now() WHERE id=${binding.id} AND EXISTS (SELECT 1 FROM canvas_sync_jobs WHERE id=${job.id} AND lease_token=${job.lease_token} AND status='running')`
    assertActive()
    await sql`INSERT INTO user_documents(user_id,namespace,document_key,value,updated_at) VALUES(${job.user_id},'canvas-module-inventories',${binding.id},${JSON.stringify({bindingId:binding.id,course:{courseCode:binding.course_code,courseName:binding.course_name,academicYear:binding.academic_year || 'undated',period:String(binding.period||'')},modules:placementManifest.modules,sourcePaths:placementManifest.resources.map(r=>r.target || r.path),skipped:[...placementManifest.skipped,...results.filter(r=>r.skipped)],updatedAt:new Date().toISOString()})}::jsonb,now()) ON CONFLICT(user_id,namespace,document_key) DO UPDATE SET value=excluded.value,updated_at=now()`
...
export async function scheduleDueRefreshes() {
  // A preview database is a snapshot; never fan out work for copied accounts.
  if (process.env.VERCEL_ENV === 'preview') return
  // Resolve opt-out/completion even for already-dispatched notifications.
  await sql`UPDATE canvas_sync_jobs j SET status='cancelled',lease_token=null,finished_at=now(),error='Automatic refresh is paused.'
    WHERE j.status IN ('pending','running') AND j.payload->>'scheduled'='true'
      AND NOT EXISTS(SELECT 1 FROM canvas_corpus_permissions p WHERE p.user_id=j.user_id AND p.origin=j.origin
        AND p.collection_enabled=true AND p.refresh_enabled=true AND p.study_status='studying'
        AND EXISTS(SELECT 1 FROM academic_programmes ap WHERE ap.user_id=j.user_id AND ap.is_active=true))`
  ...
  // catalogue refresh or asking the student to press Refresh.
  {
    ...
    const connectionFailures = await sql`SELECT DISTINCT user_id, origin FROM canvas_sync_jobs
      WHERE status IN ('failed','pending') AND (payload->>'blockedReason'='canvas-connection' OR error='The stored Canvas connection could not be read. Reconnect Canvas to replace it.')`
    for (const failed of connectionFailures) {
      try {
        ...
        await sql`UPDATE canvas_sync_jobs SET status='pending', attempts=least(attempts, 2), priority=greatest(priority, 110),
          error=null, payload=payload - 'blockedReason' - 'queueFailures', run_after=now(), finished_at=null, lease_token=null, heartbeat_at=null
          WHERE user_id=${failed.user_id} AND origin=${failed.origin} AND status IN ('failed','pending')
            AND (payload->>'blockedReason'='canvas-connection' OR error='The stored Canvas connection could not be read. Reconnect Canvas to replace it.')`
  ...
  await sql`INSERT INTO canvas_sync_jobs (id, user_id, origin, binding_id, job_type, priority, payload)
    SELECT concat('csj-', gen_random_uuid()), access.user_id, binding.origin, binding.id, 'course', 60, '{"stage":"priorities","scheduled":true}'::jsonb
    FROM canvas_corpus_access access
    JOIN canvas_course_bindings binding ON binding.id=access.binding_id
    JOIN canvas_corpus_permissions permission ON permission.user_id=access.user_id AND permission.origin=binding.origin AND permission.collection_enabled=true AND permission.refresh_enabled=true AND permission.study_status='studying'
    ...
    WHERE access.sync_paused=false AND access.auto_refresh=true
      AND NOT EXISTS(SELECT 1 FROM user_documents policy WHERE policy.user_id=access.user_id AND policy.namespace='study-recurring-pipelines' AND policy.document_key='settings' AND policy.value->'settings'->>'priorities'='false')
      AND EXISTS(SELECT 1 FROM academic_programmes ap WHERE ap.user_id=access.user_id AND ap.is_active=true) AND EXISTS(SELECT 1 FROM canvas_source_snapshots source JOIN editorial_source_retrieval_chunks chunk ON chunk.asset_id=source.asset_id AND chunk.edition_id=binding.edition_id WHERE source.binding_id=binding.id AND source.contributor_user_id=access.user_id AND source.retired_at IS NULL)
      AND (scan.scanned_at IS NULL OR coalesce(scan.course_profile->>'priorityExtractionVersion', '') <> ${String(PRIORITY_EXTRACTION_VERSION)}
        OR (scan.status<>'needs-review' AND scan.scanned_at < now() - interval '24 hours')
        OR (scan.status='needs-review' AND scan.scanned_at < now() - interval '6 hours')
        OR (scan.conflicts @> '[{"title":"Priority scan allowance reached"}]'::jsonb AND scan.scanned_at < now() - interval '1 minute')
        OR (${chatAvailable()} AND scan.conflicts @> '[{"title":"Priority evidence needs review"}]'::jsonb))
      AND NOT EXISTS (SELECT 1 FROM canvas_sync_jobs recent WHERE recent.user_id=access.user_id AND recent.binding_id=binding.id AND recent.created_at > now() - CASE WHEN scan.conflicts @> '[{"title":"Priority scan allowance reached"}]'::jsonb THEN interval '1 minute' ELSE interval '1 hour' END AND recent.payload->>'stage'='priorities'
        AND NOT (${chatAvailable()} AND scan.conflicts @> '[{"title":"Priority evidence needs review"}]'::jsonb))
    ON CONFLICT DO NOTHING`
  // Reconcile the period at least hourly, even with a slow user frequency.
  // Discovery decides material eligibility after resolving TODAY's calendar;
  // never enqueue from stale auto_refresh flags before that reconciliation.
  await sql`INSERT INTO canvas_sync_jobs (id, user_id, origin, job_type, priority, payload)
    SELECT concat('csj-', gen_random_uuid()), p.user_id, p.origin, 'catalog', 45, '{"scheduled":true}'::jsonb
    FROM canvas_corpus_permissions p
    WHERE p.collection_enabled=true AND p.refresh_enabled=true AND p.study_status='studying'
      AND EXISTS(SELECT 1 FROM academic_programmes programme WHERE programme.user_id=p.user_id AND programme.is_active=true)
      ...
      AND NOT EXISTS (SELECT 1 FROM canvas_sync_jobs blocked WHERE blocked.user_id=p.user_id AND blocked.origin=p.origin AND blocked.status='failed' AND blocked.payload->>'blockedReason'='canvas-connection')
      AND NOT EXISTS (SELECT 1 FROM canvas_sync_jobs recent WHERE recent.user_id=p.user_id AND recent.origin=p.origin AND recent.job_type='catalog' AND recent.created_at > now() - make_interval(mins=>least(p.refresh_updates_minutes,p.refresh_materials_minutes,60)))
    ON CONFLICT DO NOTHING`
...
export async function processNextCanvasCorpusJob({ signal } = {}) {
  ...
  try {
    ...
    const retry = interrupted || (!blocked && Number(job.attempts) < 3)
    await sql`UPDATE canvas_sync_jobs SET status=${retry ? 'pending' : 'failed'}, error=${String(error?.message || error).slice(0, 2000)},
      ...
      payload=payload || ${JSON.stringify(blocked ? {blockedReason:error.blockedReason || 'canvas-connection'} : {})}::jsonb,
