...
async function runCatalog(job) {
  ...
  return observeCanvasCorpusCourses({ accountId: job.user_id, origin: job.origin, courses, force: job.payload?.force === true })
}
...
async function runCourse(job) {
  ...
  try {
    ...
    const priorities = await scanCanvasPriorityEvidence({ bindingId: binding.id, accountId: job.user_id })
    return { course: imported.course, files: results.length, indexed: results.filter((result) => !result.skipped).length, skipped: results.filter((result) => result.skipped).length, manifestHash, priorities: { status: priorities.status, candidates: priorities.candidates, conflicts: priorities.conflicts?.length || 0 } }
  } finally {
