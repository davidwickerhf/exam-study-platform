...
export const studyVisualSchema = z.object({
  title: label,
  caption: description,
  basis: z.enum(['source', 'illustrative']).describe('source = a newly constructed teaching visual based on cited text, never a claim of copying an original image. illustrative = a newly constructed example with explicit invented inputs.'),
...
export function studyVisualIssues(visual) {
  ...
  if (/\bdata:[^,\s]*,|<\/?(?:script|iframe|svg|img)|https?:\/\//i.test(JSON.stringify(visual))) issues.push('Visuals must use validated data, not embedded images, links or executable markup.')
  return issues
}
