...
export const studyVisualSchema = z.object({
  ...
  diagram: z.discriminatedUnion('kind', [
    ...
    z.object({ kind: z.literal('comparison'), columns: z.array(label).min(2).max(4),
      rows: z.array(z.object({ label, cells: z.array(z.string().min(1).max(250)).min(2).max(4) })).min(2).max(8) }),
    ...
    z.object({ kind: z.literal('sets'), aLabel: label, bLabel: label,
      universe: z.array(label).min(2).max(24), a: z.array(label).max(24), b: z.array(label).max(24) })
  ])
})
