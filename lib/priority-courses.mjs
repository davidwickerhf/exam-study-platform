...
export function programmePriorityCourses(workspace, editorial = [], scans = []) {
  ...
  return (workspace?.courses || []).filter(course => course.code && !course.hiddenFromStats).map(course => {
    ...
    const publishedYear = yearKey(published?.editorialEdition?.academicYear || published?.courseProfile?.academicYear)
    const currentPublished = publishedYear === year && year ? published?.courseProfile : null
    return { id: published?.id || course.code, code: course.code, name: course.name,
      archived: published?.archived || false,
      ruleAcademicYear: year,
      courseProfile: currentPublished?.assessment?.status === 'confirmed' ? currentPublished : scan?.courseProfile || currentPublished || null,
