...
const sourceKinds = new Set(['syllabus', 'requirements', 'slides', 'assessments', 'activities', 'readings', 'materials'])
...
export async function retrieveCanvasCorpus({ query, courseCode = '', canonicalCourseId = '', academicYear = '', sourceType = '', includeHistorical = true, limit = 8 } = {}) {
  ...
  const base = await sql`SELECT c.id, c.edition_id, b.canonical_course_id, b.course_code, b.course_name, b.academic_year, b.period,
  ...
  if (embedding.configured) {
    try {
      ...
      if (vector) {
        ...
        const semantic = await sql`SELECT c.id, c.edition_id, b.canonical_course_id, b.course_code, b.course_name, b.academic_year, b.period,
            s.source_path, s.resource_type, s.canvas_updated_at, s.last_seen_at, s.retired_at, c.page_number, c.content,
