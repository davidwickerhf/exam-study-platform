...
export const LOCAL_GUIDE_HANDOFF = Object.freeze({
  ...
  concurrency: 'next may return several outstanding requests in `requests` (source mapping only; every other step issues exactly one). Each carries its own requestId, evidence, prompt, schema and output budget and may be computed and submitted in any order. `request` is always requests[0], so a client that reads only `request` remains correct and simply works through them one at a time. Submissions serialize on the server; a busy result means a sibling holds the lease, so retry the identical payload rather than fetching a new request. Concurrency changes latency only: cost is per token and is unchanged. On a provider rate limit, retry only that request after its indicated delay; never escalate model, effort or output caps.',
