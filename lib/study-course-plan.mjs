...
import { digest, mapPrompt, mapSchema, studyResponseSchema, assertEvidence, evidenceBatches, evidencePrompt, StudyVersionError } from './study-version-content.mjs'
...
// Mapping batches are independent slices of the same evidence, so several may
// be mapped at once. This bounds latency only: each call keeps its own budget
// reservation and settlement, and per-token cost is unchanged by the pool size.
export function studyMappingConcurrency(env=process.env) {
 const raw=env?.STUDY_MAPPING_CONCURRENCY
 if(raw===undefined || raw===null || String(raw).trim()==='')return generationLimits.mappingConcurrency
 const value=Number(String(raw).trim())
 if(!Number.isInteger(value) || value<1)throw new StudyVersionError(`STUDY_MAPPING_CONCURRENCY must be a whole number of at least 1, at most ${generationLimits.maxMappingConcurrency}.`,500)
 return Math.min(value,generationLimits.maxMappingConcurrency)
}
// Concurrent batches can finish out of order, and a batch that is still being
// split leaves a hole in front of finished ones. Accepted maps wait here, keyed
// by their content-identified batch, and join work.maps only in batch order so
// the saved run stays byte-identical to a sequential one.
export function mappingBatchHash(batch) {return digest(batch)}
export function mergeAcceptedMaps(work,batches) {
 const pending=work.mappingAccepted || {}
 let merged=0
 while(work.maps.length<batches.length){
  const entry=pending[mappingBatchHash(batches[work.maps.length])]
  if(!entry)break
  delete pending[mappingBatchHash(batches[work.maps.length])]
  work.maps.push(entry);merged++
 }
 if(Object.keys(pending).length)work.mappingAccepted=pending;else delete work.mappingAccepted
 return merged
}
// The next batches to map: the unfinished prefix, skipping any already accepted
// but still waiting for an earlier batch, bounded by the configured pool.
export function pendingMappingBatches(work,batches,limit) {
 const indexes=[]
 for(let index=work.maps.length;index<batches.length && indexes.length<limit;index++)
  if(!work.mappingAccepted?.[mappingBatchHash(batches[index])])indexes.push(index)
 return indexes
}
...
export async function registerCourseOutline(version,work) {
 ...
 work.planning={...projection,reusedSourceMaps:work.reusedSourceMaps || 0,overlappingGuides,autoPlaced:work.autoPlaced || [],
  ...(work.planningPolicy?{planningPolicy:work.planningPolicy,budget:work.planningBudget,scopeExclusions:work.scopeExclusions || [],evidenceTrims:(work.evidenceTrims || []).map(({topicId,trimmedSourceIds,trimmedCharacters,conceptsWithoutEvidence})=>({topicId,passages:trimmedSourceIds.length,trimmedCharacters,conceptsWithoutEvidence}))}:{})}
