...
export function courseEarnedCredits(course) {
  const passed = (course.attempts || []).filter(a => a.status === 'passed' || (!a.status && typeof a.grade === 'number' && a.grade >= (course.passMark ?? 5.5)))
    .sort((a, b) => String(a.examDate || a.academicYear || '').localeCompare(String(b.examDate || b.academicYear || '')))
  return passed.length ? Number(passed[0].ects ?? course.ects) || 0 : 0
}
