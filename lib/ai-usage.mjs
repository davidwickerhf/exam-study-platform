...
function buildSummary(events) {
  ...
  return {
    ...
    recent: [...events].sort((a, b) => b.createdAt.localeCompare(a.createdAt)).slice(0, 15)
  }
}

export async function getAiUsageSummary() {
  const exemptionReason=await aiQuotaExemption()
  return {...buildSummary(await readEvents()),unlimited:Boolean(exemptionReason),exemptionReason}
}
...
export async function reserveAiUsage(feature, { inputTokens, maxOutputTokens, metadata = {} } = {}) {
  ...
  if (reason && !(await aiQuotaExemption())) {
