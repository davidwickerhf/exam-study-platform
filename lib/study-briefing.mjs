...
import { homePriorities } from './workspace/home.mjs'
...
export async function studyBriefing({ days = 7, now = new Date() } = {}) {
  ...
  const scans=await canvasPriorityProfiles({accountId:currentUserId()}).catch(error=>{problems.push({source:'Course requirements',error:error.message});return []})
  const ruleCourses=programmePriorityCourses(workspace,[],scans)
  const calendar = aggregateCalendar({ ruleCourses, workspace: workspace || { courses: [] }, editorialCourses: [], institutionCalendar: institution, feeds, canvas, date: now })
  ...
  for (const assignment of canvas.assignments) {
    if (assignment.status === 'missing') {
      priorities.push({ kind: 'missing', title: assignment.title, course: assignment.courseCode, when: isoDay(assignment.dueAt), url: `/app/updates?tab=assignments&assignment=${encodeURIComponent(assignment.id)}`, why: 'Canvas has this marked missing.' })
    } else if (assignment.status === 'overdue') {
      priorities.push({ kind: 'overdue', title: assignment.title, course: assignment.courseCode, when: isoDay(assignment.dueAt), url: `/app/updates?tab=assignments&assignment=${encodeURIComponent(assignment.id)}`, why: `Due ${Math.abs(daysBetween(assignment.dueAt, today))} day(s) ago and not handed in.` })
      ...
      priorities.push({ kind: 'due', title: assignment.title, course: assignment.courseCode, when: isoDay(assignment.dueAt), url: `/app/updates?tab=assignments&assignment=${encodeURIComponent(assignment.id)}`, points: assignment.pointsPossible, why: inDays === 0 ? 'Due today.' : `Due in ${inDays} day(s).` })
    }
  }
  ...
  return {
    ...
    teaching,
    courseObligations:homePriorities({events:calendar.events,assignments:canvas.assignments,courses:ruleCourses,now:now.getTime(),limit:100}).filter(item=>item.source==='Verified course rule'),
    requirementCoverage:scans.map(scan=>({course:scan.courseCode,academicYear:scan.academicYear,status:scan.status,scannedAt:scan.scannedAt,conflicts:scan.conflicts})),
