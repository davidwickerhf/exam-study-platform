...
export async function readModelStream(response, onContent) {
  ...
  try {
    while (!finished) {
      ...
      while ((end = buffer.indexOf('\n')) >= 0) {
        ...
        if (event.error) {const failure=providerFailure(502,event);throw Object.assign(new Error(failure.message),{failure,code:failure.code})}
  ...
  } catch(error) { error.usage=usage; throw error } finally { await reader.cancel().catch(() => {}); reader.releaseLock() }
}
