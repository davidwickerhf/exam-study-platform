...
import { createQualityEvaluation, readQualityEvaluation, stepQualityEvaluation } from './study-quality-evaluation.mjs'
...
export async function studyVersionApi({
  ...
  const evaluation = pathname.match(/^\/api\/study-versions\/evaluations\/(sqe-[a-f0-9-]{36})(\/step)?$/)
  if (evaluation) {
    if (method === 'GET' && !evaluation[2]) return ok(await readQualityEvaluation(evaluation[1], { sourceOptions }))
    if (method === 'POST' && evaluation[2]) return ok(await stepQualityEvaluation(evaluation[1], body.revision, { generate: generateEvaluation, sourceOptions }))
