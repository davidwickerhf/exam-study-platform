...
export async function studyVersionApi({
  ...
  const match =
    /^\/api\/study-versions\/(sv-[a-f0-9-]+)(?:\/(refresh|stop|retry|progress|publish|submit|exams|practice|assess|practice-step|edit|improve|proposal|restore))?$/.exec(
      pathname
    )
  if (match) {
    ...
    if (method === 'GET' && action === 'practice') return ok({ records: await listStudyPractice(id, { sourceOptions }) })
