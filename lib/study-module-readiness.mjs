...
export function validateModuleReadiness(result, snapshot, course = null) {
  ...
  if(result.ready && course) {
    ...
    if(!result.scope?.length || result.scope.some(t=>!t.sourceIds.some(id=>snapshot.chunks.some(c=>c.id===id && current.has(c.sourceKey))) || t.sourceIds.some(id=>!snapshot.chunks.some(c=>c.id===id)))) throw new Error('Current-edition evidence must establish every topic in the module scope.')
  }
