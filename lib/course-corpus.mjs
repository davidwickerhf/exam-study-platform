...
export async function observeCanvasCorpusCourses({ accountId, origin, courses = [], explicit = false, force = false, syncId = '' } = {}) {
  ...
  const selected = explicit
    ...
    : selectCanvasCorpusCourses(courses)
  ...
  for (const course of selected) {
    ...
    const rows = await sql`INSERT INTO canvas_sync_jobs (id, user_id, origin, binding_id, job_type, priority, payload)
      SELECT ${`csj-${randomUUID()}`}, ${accountId}, ${origin}, ${bindingId}, 'course', ${explicit ? 120 : course.current ? 80 : course.upcoming ? 70 : 20}, ${JSON.stringify({ canvasCourseId: String(course.id), explicit, force, syncId: syncId || null })}::jsonb
      WHERE NOT EXISTS (
...
export async function canvasCorpusStatus({ accountId } = {}) {
  ...
  const courses = await sql`SELECT b.id, b.origin, b.canvas_course_id, b.edition_id, b.canonical_course_id, b.course_code, b.course_name, b.academic_year, b.period, b.last_synced_at, b.next_sync_at,
  ...
  const jobs = await sql`SELECT j.id, j.origin, j.binding_id, j.job_type, j.status, j.attempts, j.payload, j.result, j.error, j.run_after, j.created_at, j.started_at, j.finished_at,
  ...
  const latestJobs = await sql`SELECT DISTINCT ON (j.binding_id) j.id, j.origin, j.binding_id, j.job_type, j.status, j.attempts, j.payload, j.result, j.error, j.created_at, j.started_at, j.finished_at,
      b.course_code, b.course_name, b.academic_year
