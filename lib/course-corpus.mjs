...
export async function observeCanvasCorpusCourses({ accountId, origin, courses = [], explicit = false, force = false, syncId = '', automatic = false, timeContext = null, refreshPolicy = false } = {}) {
  ...
  for (const course of selected) {
    ...
    const [scraped] = automatic ? await sql`SELECT id FROM canvas_sync_jobs WHERE user_id=${accountId} AND binding_id=${bindingId}
      AND status='completed' AND job_type='course' AND coalesce(payload->>'stage','') NOT IN ('priorities','freshness') LIMIT 1` : []
    const metadataOnly = automatic && Boolean(scraped)
    const rows = await sql`INSERT INTO canvas_sync_jobs (id, user_id, origin, binding_id, job_type, priority, payload)
      ...
      WHERE EXISTS (SELECT 1 FROM canvas_corpus_access a WHERE a.user_id=${accountId} AND a.binding_id=${bindingId} AND a.sync_paused=false) AND NOT EXISTS (
        ...
        AND ((recent.status='completed' AND recent.finished_at>now()-make_interval(mins=>${metadataOnly ? permission.refresh?.updatesMinutes || 30 : permission.refresh?.materialsMinutes || 360}))
