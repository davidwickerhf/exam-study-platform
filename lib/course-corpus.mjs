import { CANVAS_REFRESH_DEFAULTS, validateCanvasRefreshSettings } from './canvas-refresh-policy.mjs'
import { createHash, randomUUID } from 'node:crypto'
import { sql } from './db.mjs'
import { loadEditorialProgrammeCatalogue, findEditorialProgramme } from './editorial-programmes.mjs'
...
export function periodFromCanvasCourse(course = {}) {
  ...
  const coded = source.match(/20\d{2}\s*[-/]\s*20\d{2}\s*[-–—]\s*(100|200|300|400|500|600)\b/)
  if (coded) return ({ 100: '1', 200: '2', 300: '3', 400: '4', 500: '5', 600: '6' })[coded[1]]
...
export function canvasRefreshCalendar(programme = {}) {
  const reference = findEditorialProgramme(programme.template_programme_id, programme.template_version_id)
  return [...(reference?.programme?.calendar || []), ...(programme.planning_objectives?.academicPeriods || [])]
}

export function selectScheduledCanvasCourses(courses = [], { academicYear = '', periodNumber = null, now = new Date(), refreshWindow = null } = {}) {
  if (refreshWindow?.mode === 'paused') return []
  ...
  for (const course of courses) {
    ...
    const years = refreshWindow?.years || [year]
    const selectedPeriod = refreshWindow ? refreshWindow.periodNumber : periodNumber
    const breakWatch = refreshWindow && ['break','calendar-unavailable'].includes(refreshWindow.mode)
    const starts = course.term?.startAt || course.startAt
    const beginsSoon = course.upcoming && starts && (new Date(starts)-now)/86400000 <= (breakWatch ? 60 : 14)
    const retained = breakWatch && course.concluded && !['deleted','unpublished'].includes(course.workflowState)
    if (!code || (!course.current && !beginsSoon && !retained) || (edition && !years.includes(edition)) || (selectedPeriod && period && Number(period) !== Number(selectedPeriod))) continue
...
export async function canvasCorpusPermission({ accountId, origin } = {}) {
  if (!sql) return { collectionEnabled: false, sharingMode: 'private', mode: 'local', refresh: CANVAS_REFRESH_DEFAULTS }
  const [row] = await sql`SELECT collection_enabled, sharing_mode, consent_version, consented_at, revoked_at, updated_at, refresh_enabled, refresh_updates_minutes, refresh_materials_minutes, study_status, refresh_policy, updates_refreshed_at
  ...
  return row ? {
    ...
    refresh: { enabled: row.refresh_enabled, updatesMinutes: row.refresh_updates_minutes, materialsMinutes: row.refresh_materials_minutes, studyStatus: row.study_status, policy: row.refresh_policy, updatesRefreshedAt: row.updates_refreshed_at },
    sharingMode: row.sharing_mode,
    consentVersion: row.consent_version,
    consentedAt: row.consented_at,
    revokedAt: row.revoked_at,
    updatedAt: row.updated_at
  } : { collectionEnabled: false, sharingMode: 'private', consentVersion: 'v1', consentedAt: null, revokedAt: null, refresh: CANVAS_REFRESH_DEFAULTS }
}

export async function setCanvasRefreshSettings({ accountId, origin, settings } = {}) {
  const refresh = validateCanvasRefreshSettings(settings)
  if (!sql) throw new Error('Automatic Canvas refresh requires hosted storage.')
  const host = new URL(origin).origin
  const rows = await sql`UPDATE canvas_corpus_permissions SET refresh_enabled=${refresh.enabled},
    refresh_updates_minutes=${refresh.updatesMinutes}, refresh_materials_minutes=${refresh.materialsMinutes},
    study_status=${refresh.studyStatus}, updates_refreshed_at=null, updated_at=now()
    WHERE user_id=${accountId} AND origin=${host} RETURNING user_id`
  if (!rows.length) throw new Error('Choose material collection permission for this Canvas connection first.')
  // Cancel automatic work only, including active leases; original files and all
  // checkpoints remain intact. Manual collection stays available.
  await sql`UPDATE canvas_sync_jobs SET status='cancelled',lease_token=null,finished_at=now(),error='Automatic refresh settings changed.'
    WHERE user_id=${accountId} AND origin=${host} AND status IN ('pending','running') AND payload->>'scheduled'='true'`
  await sql`UPDATE canvas_corpus_access a SET auto_refresh=false FROM canvas_course_bindings b
    WHERE a.binding_id=b.id AND a.user_id=${accountId} AND b.origin=${host}`
  if (refresh.enabled && refresh.studyStatus === 'studying') await sql`INSERT INTO canvas_sync_jobs(id,user_id,origin,job_type,priority,payload)
    SELECT ${`csj-${randomUUID()}`},${accountId},${host},'catalog',45,'{"scheduled":true}'::jsonb
    WHERE EXISTS(SELECT 1 FROM canvas_corpus_permissions WHERE user_id=${accountId} AND origin=${host} AND collection_enabled=true)
...
export async function observeCanvasCorpusCourses({ accountId, origin, courses = [], explicit = false, force = false, syncId = '', automatic = false, timeContext = null, refreshPolicy = false } = {}) {
  ...
  if (automatic && (!permission.refresh?.enabled || permission.refresh?.studyStatus === 'completed')) return { observed: 0, queued: 0, disabled: true }
  ...
  for (const course of selected) {
    ...
    const [scraped] = automatic ? await sql`SELECT id, finished_at FROM canvas_sync_jobs WHERE user_id=${accountId} AND binding_id=${bindingId}
    ...
    const metadataOnly = automatic && Boolean(scraped) && Date.now()-new Date(scraped.finished_at).getTime() < (permission.refresh?.materialsMinutes || 1440)*60000
    const rows = await sql`INSERT INTO canvas_sync_jobs (id, user_id, origin, binding_id, job_type, priority, payload)
      SELECT ${`csj-${randomUUID()}`}, ${accountId}, ${origin}, ${bindingId}, 'course', ${explicit ? 120 : course.current ? 80 : course.upcoming ? 70 : 20}, ${JSON.stringify({ canvasCourseId: String(course.id), explicit, force, scheduled: automatic, ...(metadataOnly ? { stage: 'freshness' } : {}), syncId: syncId || null })}::jsonb
      WHERE EXISTS (SELECT 1 FROM canvas_corpus_access a WHERE a.user_id=${accountId} AND a.binding_id=${bindingId} AND a.sync_paused=false) AND NOT EXISTS (
        SELECT 1 FROM canvas_course_bindings b WHERE b.id=${bindingId} AND ${!force && !automatic && !explicit} AND b.last_synced_at IS NOT NULL AND b.next_sync_at > now()
      ) AND NOT EXISTS (
        SELECT 1 FROM canvas_sync_jobs recent WHERE ${automatic} AND recent.user_id=${accountId} AND recent.binding_id=${bindingId}
        AND recent.job_type='course' AND coalesce(recent.payload->>'stage','')<>'priorities'
        AND (${metadataOnly} OR coalesce(recent.payload->>'stage','')<>'freshness')
        AND ((recent.status='completed' AND recent.finished_at>now()-make_interval(mins=>${metadataOnly ? permission.refresh?.updatesMinutes || 30 : permission.refresh?.materialsMinutes || 1440}))
          OR (recent.status='failed' AND recent.finished_at>now()-make_interval(hours=>${Math.max(1, Number(process.env.CANVAS_CORPUS_FAILURE_COOLDOWN_HOURS || 6))})))
      ) ON CONFLICT DO NOTHING RETURNING id`
    queued += rows.length
  }
  if (refreshPolicy) {
    const ids = (!permission.refresh?.enabled || permission.refresh?.studyStatus === 'completed' ? [] : selectScheduledCanvasCourses(courses, timeContext || {})).map(course => String(course.id))
...
export async function listCanvasCorpusMaterials({ accountId, courseCode = '', academicYear = '' } = {}) {
  ...
  const rows = await sql`SELECT DISTINCT ON (s.id)
      s.id AS snapshot_id, s.asset_id, s.source_path, s.resource_type, s.sha256, s.metadata AS source_metadata,
  ...
  return rows.map((row) => ({
    ...
    locations: row.source_metadata?.locations || [],
