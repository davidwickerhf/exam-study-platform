...
export async function observeCanvasCorpusCourses({ accountId, origin, courses = [], explicit = false, force = false } = {}) {
  ...
  const selected = explicit ? courses : selectCanvasCorpusCourses(courses)
  let queued = 0
  for (const course of selected) {
    ...
    const rows = await sql`INSERT INTO canvas_sync_jobs (id, user_id, origin, binding_id, job_type, priority, payload)
      SELECT ${`csj-${randomUUID()}`}, ${accountId}, ${origin}, ${bindingId}, 'course', ${explicit ? 120 : course.current ? 80 : course.upcoming ? 70 : 20}, ${JSON.stringify({ canvasCourseId: String(course.id), explicit })}::jsonb
      WHERE NOT EXISTS (
...
export async function enqueueCanvasCourseSync({ accountId, origin, course, force = true } = {}) {
  ...
  return observeCanvasCorpusCourses({ accountId, origin: new URL(origin).origin, courses: [course], explicit: true, force })
}
...
export async function canvasCorpusStatus({ accountId } = {}) {
  ...
  const jobs = await sql`SELECT j.id, j.binding_id, j.job_type, j.status, j.attempts, j.error, j.created_at, j.started_at, j.finished_at,
    ...
    WHERE j.user_id=${accountId} ORDER BY j.created_at DESC LIMIT 100`
  return {
    mode: 'neon',
    available: true,
    ...
    jobs: jobs.map((row) => ({ id: row.id, bindingId: row.binding_id, type: row.job_type, status: row.status, attempts: row.attempts, error: row.error, courseCode: row.course_code, courseName: row.course_name, academicYear: row.academic_year, createdAt: row.created_at, startedAt: row.started_at, finishedAt: row.finished_at }))
  }
}
...
export async function listCanvasCorpusMaterials({ accountId, courseCode = '', academicYear = '' } = {}) {
  ...
  const rows = await sql`SELECT DISTINCT ON (s.id)
      ...
      b.course_code, b.course_name, b.academic_year, b.period, b.canonical_course_id
...
export async function canvasCorpusAsset({ accountId, assetId } = {}) {
  ...
  const [row] = await sql`SELECT a.id, a.filename, a.media_type, a.byte_size, a.sha256, a.expected_chunks, a.is_complete, a.metadata
    ...
    WHERE a.id=${assetId} AND a.is_complete=true AND EXISTS (
      ...
      WHERE s.asset_id=a.id AND (s.sharing_mode='community' OR s.contributor_user_id=${accountId})
    )`
