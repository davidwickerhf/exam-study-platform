  ...
  for (const course of selected) {
    ...
    const rows = await sql`INSERT INTO canvas_sync_jobs (id, user_id, origin, binding_id, job_type, priority, payload)
      ...
      WHERE EXISTS (SELECT 1 FROM canvas_corpus_access a WHERE a.user_id=${accountId} AND a.binding_id=${bindingId} AND a.sync_paused=false) AND NOT EXISTS (
        SELECT 1 FROM canvas_course_bindings b WHERE b.id=${bindingId} AND ${!force && !automatic} AND b.last_synced_at IS NOT NULL AND b.next_sync_at > now()
      ) AND NOT EXISTS (
        ...
        AND ((recent.status='completed' AND recent.finished_at>now()-make_interval(mins=>${permission.refresh?.materialsMinutes || 360}))
