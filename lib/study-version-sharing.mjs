...
async function acceptedCommunitySource(source) {
  if (!sql || !source.snapshotId) return false
  const rows =
...
export function selectStudyPublication(revision, topicIds) {
  ...
  return {
    ...
    snapshot: {
      ...revision.snapshot,
      // The cited excerpts are copied inline here. A managed guide's reference
      // to its course run's revision is private and must not travel with them.
      ref: undefined,
      sourceIds: undefined,
