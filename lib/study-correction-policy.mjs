...
export function correctionContext(draft, chapterId) {
  const history=(draft.correctionHistory || []).filter(row=>row.chapterId===chapterId).map(({phase,findings,rejectedMerge})=>({phase,findings,...(rejectedMerge?{rejectedBecauseItBroke:rejectedMerge}:{})}))
  if(!history.length)return ''
  return `\nCORRECTION HISTORY (regression context, not additional source evidence): ${JSON.stringify(history)}\nFix the current findings while preserving earlier corrected reasoning. Earlier findings may already be resolved: inspect the latest draft before changing them. When adding missing teaching, use a worked case distinct from the independent and transfer assessments. If a teaching change exposes the answer to an existing assessment, replace that assessment with a genuinely different reasoning task. Keep source exclusions and objective coverage intact. When a finding says a mechanism or claim is not established by the supplied evidence, remove it or clearly relabel it as background outside the taught scope, and remove or replace any question that assesses it. Do not add new teaching to justify unsupported content.`
}
