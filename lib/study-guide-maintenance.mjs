...
import { refreshStudyVersion, assertAutomaticSourceScope } from './study-version-pipeline.mjs'
import { assertUnmanagedStudyVersion } from './study-version-editing.mjs'
...
export async function enrollGuideMaintenance(input,{sourceOptions={}}={}) {
  ...
  // Enroll the course run, never one of its managed guides: a guide's revisions
  // are republished by its parent, so maintenance here could never update it.
  ...
  // fresh enrollment nonce on the version, so even re-enabling the same refs
  // cannot launch work until the expected-revision mutation succeeds.
  selection.enrollmentId=randomUUID()
  ...
  await mutateStudyVersion(version.id,v=>{
    ...
    v.automation={kind:'maintenance',versionId:v.id,enrollmentId:selection.enrollmentId,settingsKey,candidate:{bindingId:refs[0].bindingId,moduleRefs:refs}}
  })
...
export async function reconcileMaintainedGuides(row,{sourceOptions={},sources,inventories}) {
  ...
  for(const entry of (row.maintainedGuides || []).filter(e=>e.enabled)) {
    ...
    try {
      ...
      const baseline={...entry,sourceKeys:[...new Set([...entry.sourceKeys,...(revision?.snapshot.sources.map(s=>s.key)||[])])]}
      const sourceKeys=maintenanceSourceSelection(baseline,row.course,sources,inventories)
      if(revision)assertAutomaticSourceScope(version,revision.snapshot,sources,inventories)
      const snapshot=await readStudySourceSnapshot(row.course,sourceKeys,{...sourceOptions,includeHistorical:true,courseBundle:version.courseBundle===true})
