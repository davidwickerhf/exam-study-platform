...
const failures = {
  evidence_check: {title:'Tutor could not verify this answer',message:'The answer did not pass its source check. Try again; your question and earlier replies are saved.',retryable:true,retryLabel:'Retry reply'},
  provider_request: {title:'Tutor could not send this request',message:'The AI service rejected the request format. The platform owner needs to check the Tutor integration. Your question is saved.',retryable:false,retryLabel:'Retry after the issue is fixed'},
  context_limit: {title:'This conversation is too long',message:'This conversation exceeds the AI service’s context limit. Start a new conversation with your question; this history remains saved.',retryable:false,retryLabel:'Start a new conversation'},
  remote_token_budget: {title:'This connection reached its AI token budget',message:'There is not enough hosted AI allowance for this request. Use direct source tools, shorten the conversation, or wait for the allowance to reset. Your saved replies remain available.',retryable:false,retryLabel:'Retry after the allowance resets'},
...
export function providerFailure(status, body) {
  ...
  if(/context_length_exceeded|prompt_too_long/.test(code)) return tutorFailure({code:'context_limit'})
  ...
  if([408,504].includes(status)) return tutorFailure({code:'timeout'})
  if([400,413,422].includes(status)) return tutorFailure({code:'provider_request'})
