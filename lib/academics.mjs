import { repairTranscriptDuplicates, courseEarnedCredits } from './academic-record-repair.mjs'
import { randomUUID } from 'node:crypto'
import { sql, userId, localRows, saveLocalRows, iso } from './db.mjs'
import { forgetRequestMemo, requestMemo } from './request-context.mjs'
import { normalizeAttendanceRecord } from './attendance.mjs'
import { normalizePersonalCalendarEvent } from './personal-calendar.mjs'

// One request asks for the academic record several times over — programme
// scoping on /api/state, /api/academics itself, and the calendar feed — and
// each read is an index query plus five child-table queries. The record cannot
// change between those calls unless this module changes it, so it is read once
// per request and forgotten the moment anything writes.
const STATE_MEMO = 'academics:state'
...
export function emptyAcademicWorkspace() {
  return {
    ...
    events: [],
    gates: [],
    planning: { objectives: {}, periodAssignments: [], academicPeriods: [], attendanceRecords: [], calendarEvents: [] },
    calendars: []
  }
}
...
function normalizeAttempt(value, index) {
  ...
  return {
    ...
    grade,
    status,
    // Course facts are snapshots, not aliases for the current catalogue.
    // They preserve where and under which identity this particular sitting
    // happened when a curriculum later moves, renames, or re-codes a course.
    courseCode: text(value?.courseCode, 40).toUpperCase(),
    courseName: text(value?.courseName, 200),
    ects: value?.ects === null || value?.ects === '' || value?.ects === undefined ? null : Math.max(0, finite(value.ects, null)),
    creditsEarned: value?.creditsEarned == null ? null : Math.max(0, finite(value.creditsEarned, null)),
    yearLevel: text(value?.yearLevel, 40),
    period: text(value?.period, 40),
    curriculumVersion: text(value?.curriculumVersion, 50)
  }
}
...
function planningFromStored(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return { objectives: {}, periodAssignments: [], academicPeriods: [], attendanceRecords: [], calendarEvents: [] }
  ...
  return { objectives: value, periodAssignments: [], academicPeriods: [], attendanceRecords: [], calendarEvents: [] }
}
...
export function normalizeAcademicWorkspace(value) {
  ...
  return {
    ...
    planning: value.planning && typeof value.planning === 'object' ? {
      ...
      attendanceRecords: (Array.isArray(value.planning.attendanceRecords) ? value.planning.attendanceRecords : []).slice(-5000).map(normalizeAttendanceRecord).filter(Boolean),
      calendarEvents: (Array.isArray(value.planning.calendarEvents) ? value.planning.calendarEvents : []).slice(-1000).map(normalizePersonalCalendarEvent).filter(Boolean)
    } : { objectives: {}, periodAssignments: [], academicPeriods: [], attendanceRecords: [], calendarEvents: [] },
...
export function academicSummary(workspace) {
  const courses = repairTranscriptDuplicates(workspace).courses.filter((course) => !course.hiddenFromStats)
  const passed = courses.filter((course) => course.attempts.some((attempt) => attempt.status === 'passed'))
  const earnedEcts = passed.reduce((sum, course) => sum + courseEarnedCredits(course), 0)
  const graded = courses.flatMap((course) => {
    ...
    return latest ? [{ grade: latest.grade, ects: latest.ects ?? course.ects }] : []
  })
  ...
  const upcoming = courses.flatMap((course) => course.attempts
    ...
    .map((attempt) => ({ courseId: course.id, code: course.code, name: course.name, ects: attempt.ects ?? course.ects, id: attempt.id, academicYear: attempt.academicYear, type: attempt.type, examDate: attempt.examDate, grade: attempt.grade, status: attempt.status })))
...
function programmeFromRows(row, courses, attempts, events, gates) {
  ...
  for (const attempt of attempts) {
    ...
    attemptsByCourse.get(attempt.course_id).push({
      id: attempt.id,
      academicYear: attempt.academic_year,
      type: attempt.type,
      examDate: dateOnly(attempt.exam_date),
      grade: attempt.grade == null ? null : Number(attempt.grade),
      status: attempt.status,
      courseCode: attempt.course_code || '',
      courseName: attempt.course_name || '',
      ects: attempt.ects == null ? null : Number(attempt.ects),
      creditsEarned: attempt.credits_earned == null ? null : Number(attempt.credits_earned),
...
async function loadProgramme(id) {
  if (sql) {
    ...
    return repairTranscriptDuplicates(programmeFromRows(row, courses, attempts, events, gates))
  }
  const found = (await localRows('academic_programmes')).find((row) => row.workspace.id === id)
  return found ? repairTranscriptDuplicates(structuredClone(found.workspace)) : null
}

// Every write drops the per-request read on both sides of the statement: once
// so nothing already queued reuses the old record, and once after it lands so
// the next read in this request sees what was just saved.
async function invalidating(write) {
  forgetRequestMemo(STATE_MEMO)
  try { return await write() }
  finally { forgetRequestMemo(STATE_MEMO) }
}

function storeProgramme(workspace, options = {}) {
  return invalidating(() => writeProgramme(workspace, options))
}

async function writeProgramme(workspace, { active } = {}) {
  if (sql) {
    const uid = userId()
    ...
    for (const [position, course] of workspace.courses.entries()) {
      ...
      for (const [index, attempt] of course.attempts.entries()) {
        await sql`INSERT INTO academic_attempts (user_id, programme_id, course_id, id, position, academic_year, type, exam_date, grade, status,
            course_code, course_name, ects, year_level, period, curriculum_version, credits_earned)
          VALUES (${uid}, ${workspace.id}, ${course.id}, ${attempt.id}, ${index}, ${attempt.academicYear}, ${attempt.type}, ${attempt.examDate}, ${attempt.grade}, ${attempt.status},
            ${attempt.courseCode}, ${attempt.courseName}, ${attempt.ects}, ${attempt.yearLevel}, ${attempt.period}, ${attempt.curriculumVersion}, ${attempt.creditsEarned})
...
function setActiveProgramme(id) {
  return invalidating(async () => {
    if (sql) {
...
function removeProgramme(id) {
  return invalidating(() => dropProgramme(id))
}

async function dropProgramme(id) {
  if (sql) {
    // These records are deliberately not FK-bound: old users may temporarily
    // own legacy/default study rows before an academic workspace is created.
    // Once a real workspace is deleted, however, its personal state goes too.
    for (const table of ['course_settings', 'item_progress', 'personal_exercises', 'flashcards', 'sr_cards', 'mistakes', 'mock_sessions', 'browser_state', 'activity_events', 'academic_snapshots']) {
      await sql.query(`DELETE FROM ${table} WHERE user_id = $1 AND programme_id = $2`, [userId(), id])
    }
    await sql`DELETE FROM user_documents WHERE user_id = ${userId()} AND document_key LIKE ${`programme:${id}:%`}`
    await sql`DELETE FROM academic_programmes WHERE user_id = ${userId()} AND id = ${id}`
    return
  }
  for (const table of ['course_settings', 'item_progress', 'personal_exercises', 'flashcards', 'sr_cards', 'mistakes', 'mock_sessions', 'browser_state', 'activity_events', 'academic_snapshots']) {
    await saveLocalRows(table, (await localRows(table)).filter((row) => row.programmeId !== id))
  }
...
async function loadAcademicState() {
...
// The workspace gate only needs evidence of an established record, not every
// attempt, calendar event and derived academic summary.
export async function academicSetupPresence() {
  if (sql) {
    const [row] = await sql`SELECT p.programme, EXISTS (
      SELECT 1 FROM academic_courses c WHERE c.user_id=p.user_id AND c.programme_id=p.id
    ) AS has_courses FROM academic_programmes p WHERE p.user_id=${userId()}
    ORDER BY (p.is_active IS TRUE) DESC, p.created_at, p.id LIMIT 1`
    return { programmeName: row?.programme || null, courseCount: row?.has_courses ? 1 : 0 }
  }
  const { workspace } = await readAcademicState()
  return { programmeName: workspace?.profile?.programme || null, courseCount: workspace?.courses?.length || 0 }
}

export async function readAcademicState() {
  // Callers edit what they are given (`workspace.calendars = …`), so each one
  // gets its own copy: the memo holds the read, never a shared mutable record.
  return structuredClone(await requestMemo(STATE_MEMO, loadAcademicState))
}
...
export function deleteAcademicData() {
  return invalidating(async () => {
    if (sql) {
