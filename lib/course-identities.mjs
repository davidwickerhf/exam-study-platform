...
export function reconcileAcademicCourseIdentities(courses = [], identity) {
  ...
  return [...grouped.values()].map(({ canonical, records }) => {
    ...
    // A cohort curriculum explains which requirement this family fulfils, but
    // a current registration is more specific about the edition the student
    // is actually sitting. A delayed/retaking student can therefore fulfil a
    // 2023-cohort requirement through a 2026 course code without the UI moving
    // that live course back to the cohort's original period.
    const active = records.find((course) => course?.status === 'upcoming' || (course?.attempts || []).some((attempt) => attempt?.status === 'upcoming' || attempt?.registered === true))
    const preferred = active
      || records.find((course) => identity.codeKey(course.code) === identity.codeKey(canonicalCode))
    ...
    return {
      ...preferred,
      ...(canonical ? {
        code: active?.code || canonical.code,
        name: active?.name || canonical.name,
        ects: active?.ects || canonical.ects,
        yearLevel: active?.yearLevel || canonical.yearLevel,
        period: active?.period || canonical.period,
        templateCourseId: canonical.id,
        programmeRequirement: canonical.requirement,
        catalogueCode: canonical.code,
        catalogueYearLevel: canonical.yearLevel,
        cataloguePeriod: canonical.period
      } : {}),
      attempts
    }
  })
