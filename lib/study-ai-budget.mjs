import { routeStudyModel } from './study-model-routing.mjs'
import { STUDY_GENERATION_LIMITS as generationLimits } from './study-generation-limits.mjs'
import { readStudyAiPreferences } from './study-ai-preferences.mjs'
...
export function studyBudgetLimits() {
  return {
    ...
    userDayUsd: setting('STUDY_USER_DAILY_USD', generationLimits.defaultJobUsd),
    userMonthUsd: setting('STUDY_USER_MONTHLY_USD', 30),
    ...
    tokensDay: setting('STUDY_TOKENS_PER_DAY', 5000000),
    personalTokensDay: setting('STUDY_PERSONAL_TOKENS_PER_DAY', 10000000),
    maxJobUsd: setting('STUDY_MAX_JOB_USD', generationLimits.defaultJobUsd)
  }
}
...
// Conservative spend accounting: do not assume cache hits, and reserve for
// cache writes and long-context pricing when supported by the model.
export function studyModelCost(model, inputTokens, outputTokens, usage = null) {
  const price=STUDY_MODELS[model]
  const long=price.longContextThreshold && inputTokens>price.longContextThreshold
  let chargedInput=inputTokens*(price.cacheWriteMultiplier || 1)
  const read=usage?.cachedInputTokens || 0, write=usage?.cacheWriteInputTokens
  if(price.cacheWriteMultiplier && Number.isSafeInteger(read) && read>=0 && Number.isSafeInteger(write) && write>=0 && read+write<=inputTokens)
    chargedInput=inputTokens-read-write+read*0.1+write*price.cacheWriteMultiplier
  return Math.ceil(chargedInput*price.input*(long?2:1)+outputTokens*price.output*(long?1.5:1))
}
export function estimateStudyCall(prompt, maxOutputTokens, model) {
  ...
  const inputTokens = Buffer.byteLength(prompt, 'utf8') + 2048,
    outputTokens = Math.min(generationLimits.chapterTokens, Math.max(1, maxOutputTokens))
  return {
    inputTokens,
    outputTokens,
    micros: studyModelCost(model, inputTokens, outputTokens)
  }
}
export function reserveStudyLedger(
  ...
  // One generation job at a time per account, as before. Within that one job a
  // caller may explicitly hold a bounded number of reservations at once, which
  // is what lets independent source-mapping batches run together. Every
  // reservation is still taken, capped and settled individually; the default of
  // one reproduces the previous single-flight behaviour exactly, and a stored
  // lease from an older deployment carries no jobKey and so still blocks.
  const held = (user.leases || (user.lease ? [user.lease] : [])).filter(lease => lease.expiresAt > now)
  const concurrency = Math.max(1, Math.min(input.concurrencyLimit || 1, generationLimits.maxMappingConcurrency))
  if (held.length >= concurrency || held.some(lease => lease.jobKey !== input.jobKey))
  ...
  if (job + amount > usd(input.maxJobUsd))
    throw new StudyBudgetError(
  ...
  user.lease = { id, jobKey: input.jobKey, expiresAt: now + generationLimits.workerLeaseMs }
  user.leases = [...held, user.lease]
...
export function settleStudyLedger(ledger, id, usage) {
  ...
  const actual = measured
    ? studyModelCost(r.model, usage.inputTokens, usage.outputTokens, usage)
    : r.micros
  ...
  user.leases = (user.leases || (user.lease ? [user.lease] : [])).filter(lease => lease.id !== id)
  if (user.lease?.id === id) user.lease = user.leases[0] || null
...
export async function resolveStudyBilling(input = {}, platform = {}) {
  input = { ...await readStudyAiPreferences(), ...Object.fromEntries(Object.entries(input).filter(([,v]) => v !== undefined)) }
  ...
  if (input.quality && !['standard', 'enhanced', 'sol', 'astra'].includes(input.quality))
    throw new StudyVersionError('Choose standard, enhanced, Sol or Astra generation.')
  if (source === 'platform' && ['enhanced', 'sol', 'astra'].includes(input.quality)) {
    if (provider !== 'openai') throw new StudyVersionError('Enhanced generation requires an OpenAI platform connection.')
    model = {enhanced:'gpt-5.4',sol:'gpt-5.6-sol',astra:'gpt-6-astra'}[input.quality]
  }
  if (
    !STUDY_MODELS[model] ||
  ...
  const maxJobUsd = Number(input.maxJobUsd ?? limits.maxJobUsd)
  if (!Number.isFinite(maxJobUsd) || maxJobUsd < 0.05 || maxJobUsd > generationLimits.maxJobUsd)
    throw new StudyVersionError(
      'Set a generation spending cap between $0.05 and $50.'
    )
  return {
    source,
    model,
...
// Guide generation has a quality-specific default; assessment and other AI
// workloads retain their own routing. Existing jobs keep their chosen model.
export async function resolveGuideBilling(input = {}, platform = {}, existing = null) {
  const modelQuality={'gpt-6-astra':'astra','gpt-5.6-sol':'sol','gpt-5.4':'enhanced'}
  const quality=input.quality ?? (existing ? modelQuality[existing.model] || 'standard' : platform.provider==='openai' ? 'astra' : 'standard')
  return resolveStudyBilling({
    ...(existing ? {billingSource:existing.source,maxJobUsd:existing.maxJobUsd} : {}),
    ...input,
    quality,
    billingSource:input.billingSource ?? input.source ?? existing?.source,
  },existing?.source==='platform' && !input.quality ? {...platform,model:existing.model} : platform)
}
...
export async function studyBudgetSummary(platform = {}) {
  ...
  return {
    ...
    exemptionReason,
    preferences: await readStudyAiPreferences(),
...
export function estimateStudyProduction(snapshot, billing) {
  ...
  const estimate = (chapters) => {
    // Twelve typical calls per chapter: planning, drafting, six question
    // solve/compare batches, three teaching batches, and pedagogical review.
    // Repeated evidence is charged even when provider caching may discount it.
    const input = chars / 3 * 12 + chapters * 24000 + batches * 1500,
      output = batches * 1800 + chapters * 32000
  ...
  return {
    ...
    model: billing.model,
    explanation: billing.unlimited ? 'Planning estimate, not a quote. General usage quotas are waived; the per-guide spending cap still applies. Unchanged chapters are reused.' :
      'Planning estimate, not a quote. Includes objective planning, teaching, independent question solving, item-by-item factual review and pedagogical review; repairs and automatic readiness checks can add cost. Exact chapter count follows source mapping. Each API call reserves a conservative maximum before it runs; generation pauses at your cap. Unchanged chapters are reused.'
  }
}
export async function runBudgetedStudyCall(
  prompt,
  options,
  { billing, jobKey, callPlatform, callPersonal, modelRouting = process.env.STUDY_MODEL_ROUTES }
) {
  if (!billing)
    throw new StudyVersionError(
  ...
  const route=routeStudyModel(billing,options,modelRouting)
  if(route){billing={...billing,model:route.model};options={...options,billing,...(route.reasoningEffort?{reasoningEffort:route.reasoningEffort}:{}),usageMetadata:{...options.usageMetadata,modelRoute:route}}}
  ...
  const estimate = estimateStudyCall(
    prompt + JSON.stringify(options.responseSchema || {}),
  ...
  const { reservation } = await mutateLedger(location, month, (ledger) =>
    reserveStudyLedger(ledger, {
      ...
      quotaExempt,
      jobKey,
      // Only a step that explicitly issues independent calls asks for room to
      // hold more than one reservation; every other step stays single-flight.
