import { effectiveStudyModelRouting, routeStudyModel } from './study-model-routing.mjs'
...
export function reserveStudyLedger(
  ...
  user.lease = { id, expiresAt: now + generationLimits.workerLeaseMs }
...
export async function resolveGuideBilling(input = {}, platform = {}, existing = null) {
  const modelQuality={'gpt-6-sol':'standard','gpt-6-astra':'astra','gpt-5.6-sol':'sol','gpt-5.4':'enhanced'}
  const quality=input.quality ?? (existing ? modelQuality[existing.model] || 'standard' : 'standard')
  const selectedPlatform=existing?.source==='platform' && !input.quality
    ? {...platform,model:existing.model}
    : quality==='standard' && platform.provider==='openai'
      ? {...platform,model:'gpt-6-sol'}
      : platform
...
export async function runBudgetedStudyCall(
  prompt,
  options,
  { billing, jobKey, callPlatform, callPersonal, modelRouting }
) {
  if (!billing)
    throw new StudyVersionError(
  ...
  const route=routeStudyModel(billing,options,effectiveStudyModelRouting(billing,modelRouting))
