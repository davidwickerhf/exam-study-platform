...
function prepareLesson(raw, topic, evidence, plan) {
  // Normalize stray cross-objective coverage links and strip any unsupported
  // evidence-id citation left in free prose before any structural or
  ...
  const parsed = stripUnsupportedEvidenceIds(normalizeObjectiveCoverageLinks(deriveObjectiveCoverage(assertEvidence(parseStudyJson(raw, teachingSchema), evidence), plan), plan), evidence)
