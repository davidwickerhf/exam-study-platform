  ...
  try {
    ...
    if (work.stage === 'readiness') {
      ...
      for (const topic of work.topics) {
        const old = previous?.chapters.find(
          (c) =>
            ...
            c.review === 'passed' && c.standard === STUDY_STANDARD
        )
        if (old) {
      ...
      if (topic) {
        ...
        work.chapters.push(
          prepareLesson(
            selectedRepair ? applyQuestionRepair(work.repair.chapter,selectedRepair,await generate(selectedRepair.prompt,{...options,responseSchema:selectedRepair.responseSchema,maxOutputTokens:selectedRepair.tokens,usageMetadata:{...options.usageMetadata,chapterId:topic.id,phase:'practice-correction'}})) : await generate(
