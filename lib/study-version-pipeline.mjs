...
function prepareLesson(raw, topic, evidence) {
  ...
  if (parsed.sections.reduce((n, s) => n + s.text.length + (s.detail?.length || 0), 0) < 1800)
    throw new StudyVersionError(
        ...
        work.chapters.push(
          prepareLesson(
            await generate(
              ...
              {
                ...options,
                responseSchema: studyResponseSchema(teachingSchema, evidence.map(c => c.id)),
      ...
      if (chapter) {
        ...
        const result = parseStudyJson(
          await generate(
            ...
            { ...options, stage: 'quality', reasoningEffort: 'medium', maxOutputTokens: 4000 }
          ),
          reviewSchema
        )
