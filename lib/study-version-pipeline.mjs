...
function scopeEvidence(snapshot) {
...
function chapterEvidence(snapshot, topic) {
  const context = new Set(scopeEvidence(snapshot).map(c=>c.id))
...
export function normalizeStudyOutline(result, snapshot, previous = []) {
  const chunks = snapshot.chunks
  assertEvidence(result, chunks)
  const context = scopeEvidence(snapshot)
...
export async function processStudyStep(
  ...
  try {
    ...
    if (work.stage === 'readiness') {
          ...
          : resolveOutlineGroups(parseStudyJson(
              await generate(
                outlinePrompt(
                  ...
                  work.maps,
                  previous?.topics || [],
                  { chapterEvidenceCharacters: CHAPTER_EVIDENCE_CHARACTERS, scopeCharacters: scopeEvidence(snapshot).reduce((n,c)=>n+c.text.length,0), evidenceSizes: snapshot.chunks.map(c=>({id:c.id,characters:c.text.length})) }
                ),
                options
              ),
              outlineSchema
      ...
      Object.assign(
        work,
        normalizeStudyOutline(result, snapshot, previous?.topics || [])
      )
