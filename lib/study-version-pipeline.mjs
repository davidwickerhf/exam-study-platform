import {courseBundleOutlineSchema,courseBundlePrompt,resolveCourseBundle,materializeCourseBundle} from './study-course-bundle.mjs'
import { courseMappingBatches, reusableCourseMap, saveCourseMap, registerCourseOutline, checkPlanningBudget } from './study-course-plan.mjs'
...
import {
  ...
  inputHash,
  matchTopicIdentity,
  mapSchema, outlineSchema, resolveOutlineGroups,
...
export async function processStudyStep(
  ...
  try {
    ...
    if (work.stage === 'readiness') {
      ...
      const result =
        version.courseBundle
          ? resolveCourseBundle(parseStudyJson(await generate(courseBundlePrompt(version.course,work.maps,previous,studyOutlineCapacity(snapshot,version.course)),{...options,responseSchema:studyResponseSchema(courseBundleOutlineSchema),usageMetadata:{...options.usageMetadata,phase:'course-outline'}}),courseBundleOutlineSchema),work.maps)
          : work.maps.length === 1
          ? work.maps[0]
          : resolveOutlineGroups(parseStudyJson(
              await generate(
                outlinePrompt(
                  version.course,
                  work.maps,
                  previous?.topics || [],
                  studyOutlineCapacity(snapshot,version.course)
                ),
                {...options,usageMetadata:{...options.usageMetadata,phase:'course-outline'}}
              ),
              outlineSchema
            ),work.maps)
      Object.assign(
      ...
      const bundleGuides=await materializeCourseBundle(version,revision)
      await commit((next) => {
        if(version.courseBundle)next.bundleGuides=bundleGuides
        if (work.edit) {
