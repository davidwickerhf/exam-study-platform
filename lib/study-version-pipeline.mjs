...
export async function refreshStudyVersion(id, input, options) {
  ...
  return mutateStudyVersion(id, async (next) => {
    ...
    next.draft = { ...newStudyDraft(snapshot, options.billing), execution: options.execution || next.draft?.execution || 'hosted', status: (options.execution || next.draft?.execution) === 'local' ? 'local-ready' : 'queued' }
