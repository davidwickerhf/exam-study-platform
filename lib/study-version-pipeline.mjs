...
const SEMANTIC_PLAN_REPLANS = 4
// Attempts are recorded before the next replan. Values 1–3 route to Mini;
// attempt 4 is the single Sol repair and its following check is arbitrated.
    ...
    if (work.stage === 'readiness') {
      ...
      if (topic) {
        ...
        if (work.structuralFill?.topicId === topic.id) {
          ...
          // still wrong, Sol gets one compact repair and arbitrates it; another
          // failure is persisted instead of buying a known-bad draft.
          const plan=work.teachingPlans[topic.id]
