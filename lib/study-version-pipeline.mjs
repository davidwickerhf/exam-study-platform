...
import { teachingPlanSchema, teachingPlanPrompt, pedagogyReviewSchema, pedagogyPrompt, pedagogyReviewIssues, pedagogicalFindings, deriveObjectiveCoverage, normalizeObjectiveCoverageLinks, objectiveCoverageIssues, OBJECTIVE_LINK_ISSUE_PATTERN } from './study-pedagogy.mjs'
...
// checkpoint.
export function splitChapterDraft(work, topic, evidence) {
  ...
  work.chapterSplits = [...(work.chapterSplits || []), {
    reason: 'provider_output_limit', originalId: topic.id, parts: parts.map(p => p.id), at: new Date().toISOString()
  }]
  return true
}
      ...
      if (topic) {
        ...
        const buildRaw = async (formatErrorNote = '') => {
          ...
          if (refreshStep) {
            ...
            try {
              raw = await generate(draftPrompt, draftOptions)
            } catch (error) {
