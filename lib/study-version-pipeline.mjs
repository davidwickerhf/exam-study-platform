  ...
  try {
    ...
    if (work.stage === 'readiness') {
      ...
      if (topic) {
        ...
        work.chapters.push(
          prepareLesson(
            selectedRepair ? applyQuestionRepair(work.repair.chapter,selectedRepair,await generate(selectedRepair.prompt+correctionContext(work,topic.id),{...options,responseSchema:selectedRepair.responseSchema,maxOutputTokens:selectedRepair.tokens,usageMetadata:{...options.usageMetadata,chapterId:topic.id,phase:selectedRepair.parts?'content-correction':selectedRepair.sectionIds?'section-correction':selectedRepair.cardIndexes?'flashcard-correction':'practice-correction',correctionAttempt:work.automaticRepairs?.[topic.id] || 0}})) : await generate(
