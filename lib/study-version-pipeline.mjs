import { questionRepairStep, applyQuestionRepair } from './study-chapter-repair.mjs'
import { nextPedagogicalReview, acceptPedagogicalReview } from './study-pedagogical-review.mjs'
import { nextFactualReview, acceptFactualReview, factualAuditIssues } from './study-factual-review.mjs'
...
function applyChapterReview(work, chapter, issues, phase = 'content') {
  ...
  work.repairPhases ||= {}
  work.repairPhases[chapter.id] ||= {}
  if (!reviewOnly && !work.repairPhases[chapter.id][phase]) {
    // Structural correction cannot spend the content-review repair. Each has
    // one bounded attempt, under the unchanged payer and spending limits.
    work.repairPhases[chapter.id][phase] = 1
    work.automaticRepairs[chapter.id] = (work.automaticRepairs[chapter.id] || 0) + 1
    work.repair = { topicId: chapter.id, phase, chapter: structuredClone(chapter) }
    work.chapters = work.chapters.filter(c => c.id !== chapter.id)
    work.error = null
  } else {
    work.status = 'failed'
    work.error = reviewOnly ? 'The saved chapter still has a flagged issue. Review the finding before requesting a correction.' : 'This chapter still needs a correction after its bounded automatic corrections. Ready chapters are preserved; review the finding before retrying.'
  }
}
  ...
  const commit = async (change) =>
    mutateStudyVersion(id, (next) => {
      ...
      if (localSubmission) {
        ...
        next.localReceipts = next.localReceipts.slice(-240)
      }
    })
  try {
    ...
    if (work.stage === 'readiness') {
      const decision = parseStudyJson(await generate(moduleReadinessPrompt(version.course, snapshot, work.moduleCandidate), {
        ...options, responseSchema: studyResponseSchema(moduleReadinessSchema), maxOutputTokens: 12000,
      ...
      if (topic) {
        ...
        if (!work.teachingPlans[topic.id]) {
          const plan = assertEvidence(parseStudyJson(await generate(
            teachingPlanPrompt(evidencePrompt(version.course, snapshot.sources, teachingEvidence(evidence)), topic),
            { ...options, responseSchema: studyResponseSchema(teachingPlanSchema, teachingEvidence(evidence).map(c => c.id)), maxOutputTokens: 12000, usageMetadata: { ...options.usageMetadata, chapterId: topic.id, phase: 'teaching-plan' } }
          ), teachingPlanSchema), teachingEvidence(evidence))
        ...
        work.chapters.push(
          prepareLesson(
            selectedRepair ? applyQuestionRepair(work.repair.chapter,selectedRepair,await generate(selectedRepair.prompt,{...options,responseSchema:selectedRepair.responseSchema,maxOutputTokens:selectedRepair.tokens,usageMetadata:{...options.usageMetadata,chapterId:topic.id,phase:selectedRepair.sectionIds?'section-correction':'practice-correction'}})) : await generate(
              lessonPrompt(version.course, snapshot.sources, evidence, topic, work.teachingPlans[topic.id]) +
      ...
      if (chapter) {
        ...
        if(invalidPracticeLinks(chapter).length) {
          const step=practiceLinkStep(chapter)
          applyPracticeLinks(chapter,step,await generate(step.prompt,{...options,responseSchema:step.responseSchema,maxOutputTokens:5000,usageMetadata:{...options.usageMetadata,chapterId:topic.id,phase:'practice-links'}}))
          ...
          if(step) {
            ...
            try {
              const raw=await generate(step.prompt+(retry?`\nYour last review response failed validation: ${retry.message}. Correct the review response, not the chapter.`:''),
                {...options,responseSchema:step.responseSchema,stage:'quality',reasoningEffort:'medium',maxOutputTokens:step.tokens,usageMetadata:{...options.usageMetadata,chapterId:topic.id,phase:'factual-'+step.kind}})
              acceptFactualReview(chapter,step,raw)
              delete chapter.factualRetry
            } catch(error) {
              if(error.status!==502 || retry)throw error
          ...
          if(!chapter.factualRetry && !nextFactualReview(version.course,snapshot.sources,evidence,chapter)) {
            ...
            if(chapter.evidenceReview.issues.some(i=>i.severity==='error'))applyChapterReview(work,chapter,chapter.evidenceReview.issues)
          }
        } else {
          const step=nextPedagogicalReview(evidencePrompt(version.course,snapshot.sources,evidence),chapter)
          const raw=await generate(step.prompt,{...options,responseSchema:step.responseSchema,stage:'quality',reasoningEffort:'medium',maxOutputTokens:step.tokens,usageMetadata:{...options.usageMetadata,chapterId:topic.id,phase:'pedagogical-review'}})
          const result=acceptPedagogicalReview(chapter,step,raw)
          if(result) {
            chapter.pedagogicalReview = result
            applyChapterReview(work, chapter, [...chapter.evidenceReview.issues, ...pedagogyReviewIssues(chapter, result)])
          }
        }
