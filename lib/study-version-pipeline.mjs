  ...
  try {
    ...
    if (work.stage === 'readiness') {
      ...
      if (topic) {
        ...
        const acceptChapter = (prepared, {base = null, findings = [], refreshBase = null} = {}) => {
        ...
        if (work.structuralFill?.topicId === topic.id) {
          ...
          const finish = () => {
            ...
            acceptChapter(state.chapter, state.base ? {base: state.base, findings: state.baseFindings || []} : {})
          }
        ...
        if (!split) {
          // A first draft that still lacks a required item goes to the
          // structural fill (drafting, no correction slot) before any review;
          // its other findings are checked when the filled chapter is accepted.
          const fillable = isFirstDraft ? contractIssues(prepared, evidence).filter(isFillableFinding) : []
          if (fillable.length) work.structuralFill = {topicId: topic.id, chapter: prepared, findings: fillable, attempts: 0}
          else acceptChapter(prepared, {refreshBase, base: work.repair?.chapter || null, findings: repairFindings || []})
        }
        }
