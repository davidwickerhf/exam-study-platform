...
import { correctionLimit, recordCorrection } from './study-correction-policy.mjs'
    ...
    if (work.stage === 'readiness') {
      ...
      if (topic) {
        ...
        if (!work.teachingPlans[topic.id]) {
          const plan = assertEvidence(parseStudyJson(await generate(
            ...
            { ...options, responseSchema: studyResponseSchema(teachingPlanSchema, teachingEvidence(evidence).map(c => c.id)), maxOutputTokens: generationLimits.planTokens, usageMetadata: { ...options.usageMetadata, chapterId: topic.id, phase: 'teaching-plan' } }
      ...
      if (chapter) {
        ...
        if(invalidPracticeLinks(chapter).length) {
          const findings=invalidPracticeLinks(chapter).map(link=>({severity:'error',itemKey:`question:${link.questionKey}`,detail:'A misconception follow-up must point to a different question testing the same objective.'}))
          if((work.automaticRepairs?.[chapter.id] || 0)>=correctionLimit(work))applyChapterReview(work,chapter,findings,'links')
          else {
          ...
          if(!chapter.factualRetry && !nextFactualReview(version.course,snapshot.sources,evidence,chapter)) {
            ...
            if(chapter.evidenceReview.issues.some(i=>i.severity==='error'))applyChapterReview(work,chapter,chapter.evidenceReview.issues,'factual')
          }
        } else {
