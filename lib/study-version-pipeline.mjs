  ...
  try {
    ...
    if (work.stage === 'readiness') {
      ...
      if (topic) {
        ...
        let raw, split = false
        if (refreshStep) {
          raw = applySourceRefresh(refreshStep,await generate(evidencePrompt(version.course,snapshot.sources,evidence)+'\n'+refreshStep.prompt,{...options,responseSchema:refreshStep.responseSchema,maxOutputTokens:generationLimits.correctionTokens,usageMetadata:{...options.usageMetadata,chapterId:topic.id,phase:'source-refresh'}}))
        } else if (selectedRepair) {
          raw = applyRepair(await generate(selectedRepair.prompt+correctionContext(work,topic.id),{...options,responseSchema:selectedRepair.responseSchema,maxOutputTokens:selectedRepair.tokens,usageMetadata:{...options.usageMetadata,chapterId:topic.id,phase:repairPhase(selectedRepair),correctionAttempt:work.automaticRepairs?.[topic.id] || 0}}))
        } else {
        ...
        if (!split) {
        work.chapters.push(prepareLesson(raw, topic, evidence, work.teachingPlans[topic.id]))
        if(refreshBase) {
