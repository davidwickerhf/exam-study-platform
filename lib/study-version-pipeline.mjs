...
import { questionCorrectionTrial, pedagogicalPrecheckEnabled } from './study-model-routing.mjs'
  ...
  try {
    ...
    if (work.stage === 'readiness') {
            ...
            if (verdict.rejected) {
              ...
              if(more){
                ...
                if(work.repair.free){
                  ...
                  if(work.pedagogicalPrechecks?.[topic.id])Object.assign(work.pedagogicalPrechecks[topic.id],{status:'complete',repair:'applied',patches:work.repair.series?.count || 1,completedAt:new Date().toISOString()})
                }
                else recordCorrectionTrial(work, topic.id, work.repair.modelTrial, 'pending-review', work.repair.modelTrial?.route === 'question-correction' ? {base:seriesBase, findings: repairFindings || [], phase: work.repair.phase} : null)
      ...
      if (chapter) {
        ...
        if(preflight.length)applyChapterReview(work,chapter,preflight.map(detail=>({severity:'error',detail})),'structure')
        ...
        } else if (pedagogicalPrecheckEnabled() && !work.pedagogicalPrechecks?.[chapter.id] && !(work.correctionHistory || []).some(row=>row.chapterId===chapter.id) && !(work.automaticRepairs?.[chapter.id])) {
          // ADDITIVE CHEAP PRE-CHECK. This is explicitly enabled by a route
          // profile and runs once per chapter, after all free checks but before
          // either independent review. It can only replace bounded existing
          // items. The complete merge is contract-checked; rejection keeps the
          // original chapter, consumes no correction, and the full reviews run
          // anyway on the next checkpoint.
          ...
          work.pedagogicalPrechecks[chapter.id]={status:rejection?'rejected':findings.some(row=>row.severity==='error')?'repairing':'complete',findings:findings.length,...(rejection?{rejection}:{}),at:new Date().toISOString()}
          work.precheckLog ||= []
