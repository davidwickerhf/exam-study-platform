  ...
  try {
    ...
    if (work.stage === 'readiness') {
      const decision = parseStudyJson(await generate(moduleReadinessPrompt(version.course, snapshot, work.moduleCandidate), {
        ...options, responseSchema: studyResponseSchema(moduleReadinessSchema), maxOutputTokens: 6000,
      ...
      if (topic) {
        ...
        if (!work.teachingPlans[topic.id]) {
          const plan = assertEvidence(parseStudyJson(await generate(
            ...
            { ...options, responseSchema: studyResponseSchema(teachingPlanSchema, teachingEvidence(evidence).map(c => c.id)), maxOutputTokens: 6000, usageMetadata: { ...options.usageMetadata, chapterId: topic.id, phase: 'teaching-plan' } }
      ...
      if (chapter) {
        ...
        if (!chapter.evidenceReview) {
          const result = parseStudyJson(await generate(
          ...
          chapter.evidenceReview = result
          if (result.issues.some(i => i.severity === 'error')) applyChapterReview(work, chapter, result.issues)
          else if (chapter.formatVersion !== 3) applyChapterReview(work, chapter, result.issues)
        } else {
          const result = parseStudyJson(await generate(
            pedagogyPrompt(evidencePrompt(version.course, snapshot.sources, evidence), chapter),
            { ...options, responseSchema: studyResponseSchema(pedagogyReviewSchema), stage: 'quality', reasoningEffort: 'medium', maxOutputTokens: 10000, usageMetadata: { ...options.usageMetadata, chapterId: topic.id, phase: 'pedagogical-review' } }
      ...
      if (
        ...
        work.chapters.some((c) => (c.review !== 'passed' || (c.formatVersion === 3 && (!c.evidenceReview || !c.pedagogicalReview || c.evidenceReview.issues.some(i => i.severity === 'error') || pedagogyReviewIssues(c, c.pedagogicalReview).some(i => i.severity === 'error')))) && !(work.edit && c.id !== work.edit.topicId && digest(c) === digest(previous?.chapters.find(old => old.id === c.id))))
      )
        throw new StudyVersionError(
      ...
      if(version.automation) {
        ...
        if(available.some(s=>(s.announcement || s.locations?.some(l=>String(l.moduleId)===version.automation.candidate.moduleId)) && s.academicYear===version.course.academicYear && !snapshot.sources.some(old=>old.key===s.key))) throw new StudyVersionError('New material or an announcement arrived during generation. Refresh with the updated sources before activating this guide.',409)
