    ...
    if (work.stage === 'readiness') {
      ...
      if (topic) {
        ...
        if (work.structuralFill?.topicId === topic.id) {
          ...
          // drafted around them. A failed proposal receives at most three
          // cheap narrowing re-plans; the next failure is persisted and stops
          // rather than buying a known-bad draft and inevitable correction.
          const plan=work.teachingPlans[topic.id]
          ...
          const arbitration=priorAttempts>=SEMANTIC_PLAN_REPLANS
