...
export async function controlStudyGeneration(id, action, billing = null, { recheck = false } = {}) {
  return mutateStudyVersion(id, (version) => {
    ...
    if (action === 'stop') {
      ...
      const recovered = Boolean(bad) && (recoverFailedChapterByLinkNormalization(draft) || recoverFailedChapterByStaleFactualJudgments(draft, version.course))
...
function prepareLesson(raw, topic, evidence, plan) {
  // Normalize stray cross-objective coverage links before any structural or
  // fingerprinted review sees this chapter, so a review/correction fingerprint
  // is always computed over the same content that was actually reviewed.
  const parsed = normalizeObjectiveCoverageLinks(deriveObjectiveCoverage(assertEvidence(parseStudyJson(raw, teachingSchema), evidence), plan), plan)
...
export async function processStudyStep(
  ...
  // or one whose only findings are now-stale answers judgments (see
  // recoverFailedChapterByStaleFactualJudgments). Recover it deterministically
  // here too, before falling into the ordinary 'chapters'/'finish' handling
  // that would otherwise just fail again with no corrective action taken.
  recoverFailedChapterByLinkNormalization(work) || recoverFailedChapterByStaleFactualJudgments(work, version.course)
