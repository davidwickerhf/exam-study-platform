...
export function normalizeStudyOutline(result, snapshot, previous = [], course) {
  ...
  for (const topic of topics) {
    ...
    expansion.push({topicId:topic.id,title:topic.title,guideId:topic.guideId||null,parts:parts.length,
      evidenceCharacters:teaching.reduce((n,c)=>n+c.text.length,0)})
  ...
  try {
    ...
    if (work.stage === 'readiness') {
      ...
      if (!result) {
        ...
        const capacity = studyOutlineCapacity(snapshot, version.course, {bundle})
        const base = bundle
        ...
        if (pending) {
          try {
            const resumed = bundle ? resolveCourseBundle(pending.proposal, work.maps) : resolveOutlineGroups(pending.proposal, work.maps)
            const resumedOutline = normalizeStudyOutline(resumed, snapshot, previous?.topics || [], version.course)
        ...
        if (!result) {
          ...
          try {
            ...
            outline = normalizeStudyOutline(result, snapshot, previous?.topics || [], version.course)
          } catch (error) {
            result = null
            outline = null
