...
import { courseMappingBatches, reusableCourseMap, saveCourseMap, registerCourseOutline, checkPlanningBudget, splitMappingBatch, studyMappingConcurrency, mappingBatchHash, mergeAcceptedMaps, pendingMappingBatches } from './study-course-plan.mjs'
...
import {
  ...
  inputHash,
  matchTopicIdentity,
  mapSchema, outlineSchema, resolveOutlineGroups, outlineCorrectionPrompt,
...
export function studyOutlineCapacity(snapshot,course) {
  const context=scopeEvidence(snapshot,course)
  return {chapterEvidenceCharacters:CHAPTER_EVIDENCE_CHARACTERS,scopeCharacters:context.reduce((n,c)=>n+c.text.length,0),scopeEvidenceIds:context.map(c=>c.id),evidenceSizes:snapshot.chunks.map(c=>({id:c.id,characters:c.text.length}))}
}
...
function rejectedOutline(work, candidate, error) {
  if (!error.outlineIssues) throw error
  ...
  const again = issued < limit
  return (work.outlineCorrection = {
    proposal: candidate, issues: error.outlineIssues, overflow: error.outlineIssueOverflow || 0,
...
export function normalizeStudyOutline(result, snapshot, previous = [], course) {
  ...
  for (const topic of topics) {
    ...
    const parts = teaching.length ? evidenceBatches(teaching,availableCharacters) : [[]]
...
// The pending local request is a bounded set: mapping may hand out several
// independent batches at once. Older drafts (and older clients) carry exactly
// one, so the single field stays authoritative when no set was recorded.
export function outstandingLocalRequests(draft) {
  if (draft?.localRequests?.length) return draft.localRequests
  return draft?.localRequest ? [draft.localRequest] : []
}
export async function processStudyStep(
  ...
  try {
    await mutateStudyVersion(id, (next) => {
      if (
        ...
        (localSubmission && (next.draft.status !== 'waiting-local' || !outstandingLocalRequests(next.draft).some(request => request.id === localSubmission.requestId && request.contractId === localSubmission.contractId))) ||
  ...
  try {
    ...
    if (work.stage === 'readiness') {
      ...
      mergeAcceptedMaps(work,batches)
      // Independent evidence slices may be mapped together. Each call reserves
      // and settles its own budget, so a wider pool changes latency, not spend.
      const concurrencyLimit = studyMappingConcurrency()
      const window = pendingMappingBatches(work,batches,concurrencyLimit)
      const mapOne = async (index) => {
        const batch = batches[index]
        const batchHash = mappingBatchHash(batch)
        const shared = await reusableCourseMap(version,work,batch)
        const cached = shared || previous?.maps?.find((m) => m.batchHash === batchHash)
        if (cached) return { index, batch, batchHash, result: cached, shared: Boolean(shared) }
        const result = assertEvidence(
          parseStudyJson(
            await generate(
              ...
              { ...options, usageMetadata:{...options.usageMetadata,phase:'source-mapping'}, responseSchema: studyResponseSchema(mapSchema, batch.map(c => c.id)),
                ...(window.length > 1 ? { concurrencyLimit: window.length } : {}) }
            ),
            mapSchema
          ),
          batch
        )
        return { index, batch, batchHash, result, shared: false }
      }
      // A partial mapping round must survive the failure that ends it: accepted
      // maps and recorded splits are committed under the held lease before the
      // remaining error propagates, so nothing already paid for is remapped.
      const keepMapping = () => commit((next) => {
        next.draft.maps = work.maps
        next.draft.reusedSourceMaps = work.reusedSourceMaps
        next.draft.mappingBatches = work.mappingBatches
        next.draft.mappingRecoveries = work.mappingRecoveries
        if (work.mappingAccepted) next.draft.mappingAccepted = work.mappingAccepted
        else delete next.draft.mappingAccepted
      })
      if (window.length) {
        const outcomes = await Promise.allSettled(window.map(mapOne))
        const outputLimits = new Map()
        let requests = [], failure = null
        for (const [position, outcome] of outcomes.entries()) {
          if (outcome.status === 'fulfilled') {
            const { batch, batchHash, result, shared } = outcome.value
            await saveCourseMap(version,work,batch,result)
            if (shared) work.reusedSourceMaps = (work.reusedSourceMaps || 0) + 1
            work.mappingAccepted = { ...work.mappingAccepted, [batchHash]: { ...result, batchHash } }
          } else if (outcome.reason?.code === 'provider_output_limit') outputLimits.set(window[position], outcome.reason)
          else if (outcome.reason?.localStudyRequest) requests.push(outcome.reason.localStudyRequest)
          else if (!failure) failure = outcome.reason
        }
        // Halve each over-long batch on its own, from the end, so splicing one
        // batch list never moves an index that has not been handled yet.
        for (const index of [...outputLimits.keys()].sort((a,b)=>b-a))
          if (!splitMappingBatch(work,index) && !failure) failure = outputLimits.get(index)
        mergeAcceptedMaps(work,batches)
        // A lost lease cannot persist anything, but it must not hide why the
        // round failed: report the provider/budget error, not the fence.
        if (failure) { await keepMapping().catch(e=>{if(e.status!==409)throw e}); throw failure }
        if (requests.length) {
          await keepMapping()
          throw Object.assign(new Error('Waiting for local generation'), { localStudyRequests: requests, localStudyRequest: requests[0] })
        }
      }
      ...
      if (!result) {
        ...
        const capacity = studyOutlineCapacity(snapshot, version.course)
        const base = bundle
          ? courseBundlePrompt(version.course, work.maps, previous, capacity)
          : outlinePrompt(version.course, work.maps, previous?.topics || [], capacity)
        const pending = work.outlineCorrection || null
        ...
        try {
          result = bundle ? resolveCourseBundle(candidate, work.maps) : resolveOutlineGroups(candidate, work.maps)
        } catch (error) {
      ...
      if (result) {
        ...
        Object.assign(
          work,
          normalizeStudyOutline(result, snapshot, previous?.topics || [], version.course)
        )
    ...
    if (execution === 'local' && error.localStudyRequest) {
      // Outstanding requests keep their original IDs across re-issues so a
      // client holding several mapping packets is never invalidated by a peer.
      const requests = error.localStudyRequests?.length ? error.localStudyRequests : [error.localStudyRequest]
      await commit(next => {
        next.draft.status = 'waiting-local'
        next.draft.lease = null
        next.draft.attempts = 0
        next.draft.localRequest = requests[0]
        // A single pending packet stays exactly as it was recorded before:
        // only a genuinely concurrent mapping round adds the set.
        if (requests.length > 1) next.draft.localRequests = requests
        else delete next.draft.localRequests
        next.draft.localContractId = requests[0].contractId
      })
