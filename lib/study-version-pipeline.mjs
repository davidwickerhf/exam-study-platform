...
function dropEvidenceIdMentionFindings(issues) {
  if (!Array.isArray(issues)) return issues
  const next = issues.filter(i => !EVIDENCE_ID_MENTION_PATTERN.test(i.detail))
  return next.length === issues.length ? issues : next
}
function clearEvidenceIdMentionCaches(chapter) {
  let next = chapter
  if (chapter.evidenceReview) {
    const issues = dropEvidenceIdMentionFindings(chapter.evidenceReview.issues)
  ...
  if (chapter.pedagogicalReview) {
    const issues = dropEvidenceIdMentionFindings(chapter.pedagogicalReview.issues)
  ...
  if (chapter.pedagogyAudit?.reviews) {
    ...
    const reviews = Object.fromEntries(Object.entries(chapter.pedagogyAudit.reviews).map(([id, review]) => {
      const issues = dropEvidenceIdMentionFindings(review.issues)
  ...
  if (chapter.factualAudit?.judgments) {
    ...
    const judgments = Object.fromEntries(Object.entries(chapter.factualAudit.judgments).map(([key, judgment]) => {
      const issues = dropEvidenceIdMentionFindings(judgment.issues)
