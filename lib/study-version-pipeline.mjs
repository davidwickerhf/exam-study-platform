...
import { renderingPreflight, repairRenderingControls, repairIncoherentProse, incoherentProseRemains, corruptProseFindingMatcher } from './study-preflight.mjs'
...
import { studyPlanPrecheckStep, semanticPlanRetryPrompt, structuralPlanRetryPrompt, STUDY_PLAN_PRECHECK_VERSION } from './study-plan-precheck.mjs'
...
const SEMANTIC_PLAN_REPLANS = 5
// Attempts are recorded before the next replan. Values 1–3 route to Mini;
// attempts 4–5 are at most two Sol repairs, each followed by arbitration.
const MINI_SEMANTIC_PLAN_REPLANS = 4
...
export async function controlStudyGeneration(id, action, billing = null, { recheck = false } = {}) {
  return mutateStudyVersion(id, (version) => {
    ...
    if (action === 'stop') {
      ...
      const recovered = Boolean(bad) && (recoverFailedChapterByLinkNormalization(draft) || recoverFailedChapterByEvidenceIdHygiene(draft) || recoverFailedChapterByRenderingHygiene(draft) || recoverFailedChapterByCorruptPlanProse(draft) || recoverFailedChapterByStaleFactualJudgments(draft, version.course) || recoverFailedChapterByStructuralFill(draft))
...
export function prepareLesson(raw, topic, evidence, plan, {mergedCoverage = false} = {}) {
  ...
  const parsed = repairIncoherentProse(repairRenderingControls(stripUnsupportedEvidenceIds(structured)))
...
const RENDER_HYGIENE_FINDING=/invisible control character|malformed LaTeX escape/i
// A render-preflight-only failure is transport hygiene, not a teaching fault.
// Repair the saved chapter and its canonical plan, clear exactly those stale
// cached findings, and re-enter review without buying another correction.
export function recoverFailedChapterByRenderingHygiene(work){
  // Search every failed chapter: an older genuine pedagogical failure must
  // not hide a later render-only failure that can be recovered for free.
  const chapter=work.chapters?.find(c=>{
    if(c.review!=='failed')return false
    const errors=(work.issues || []).filter(i=>i.topicId===c.id && i.severity==='error')
    return errors.length && errors.every(i=>RENDER_HYGIENE_FINDING.test(i.detail))
  })
  if(!chapter)return false
  const errors=(work.issues || []).filter(i=>i.topicId===chapter.id && i.severity==='error')
  const repaired=repairRenderingControls(chapter)
  if(repaired===chapter && !chapter.renderRepairs?.length)return false
  const matches=issue=>RENDER_HYGIENE_FINDING.test(String(issue?.detail || ''))
  const clean=retargetFactualFingerprint(clearMatchingReviewCaches(repaired,matches),chapter)
  if(renderingPreflight(clean).some(detail=>RENDER_HYGIENE_FINDING.test(detail)))return false
...
export async function processStudyStep(
  ...
  recoverFailedChapterByLinkNormalization(work) || recoverFailedChapterByEvidenceIdHygiene(work) || recoverFailedChapterByRenderingHygiene(work) || recoverFailedChapterByCorruptPlanProse(work) || recoverFailedChapterByStaleFactualJudgments(work, version.course) || recoverFailedChapterByStructuralFill(work)
    ...
    if (work.stage === 'readiness') {
      ...
      if (topic) {
        ...
        if (work.structuralFill?.topicId === topic.id) {
          ...
          const planPrompt=pending
            ? semantic ? semanticPlanRetryPrompt(evidence,pending.proposal,pending.issues) : structuralPlanRetryPrompt(evidence,pending.proposal,pending.issues)
          ...
          const exhaustedMiniReplans=semantic && (work.planSemanticAttempts?.[topic.id] || 0)>=MINI_SEMANTIC_PLAN_REPLANS
          const parsed = assertEvidence(parseStudyJson(await generate(
            planPrompt,
            { ...options, responseSchema: studyResponseSchema(teachingPlanResponseSchema, evidence.map(c => c.id)), maxOutputTokens: generationLimits.planTokens, usageMetadata: { ...options.usageMetadata, chapterId: topic.id, phase: exhaustedMiniReplans?'teaching-plan-repair':'teaching-plan', ...(pending ? { replan: 1 } : {}) } }
        ...
        } else if (studyPlanPrecheckEnabled(work.billing) && !(work.planSemanticChecks?.[topic.id]?.status === 'complete' && work.planSemanticChecks[topic.id].version === STUDY_PLAN_PRECHECK_VERSION)) {
          // The structural blueprint gate above proves that required rows and
          // links exist. This standard-profile semantic gate asks a cheap independent
          // reviewer whether they stay within the evidence before a chapter is
          // drafted around them. A failed proposal receives three cheap Mini
          // narrowing re-plans. If Sol confirms that the exhausted proposal is
          // still wrong, Sol gets at most two compact narrowing repairs and
          // arbitrates each; another failure is persisted instead of buying a
          // known-bad draft.
          const plan=work.teachingPlans[topic.id]
          ...
          // Mini owns the ordinary plan checks. Once all three compact
          // narrowing replans have been spent, the final verdict is arbitrated
          // by Sol instead of letting a repeated stale Mini finding block a
          // plan that no longer contains the alleged mechanism. This replaces
          // the final check; it does not add a call or weaken the gate.
          const arbitration=priorAttempts>=MINI_SEMANTIC_PLAN_REPLANS
          const findings=savedFailure || step.accept(await generate(step.prompt,{...options,responseSchema:step.responseSchema,stage:'quality',reasoningEffort:arbitration?'medium':'low',maxOutputTokens:step.tokens,usageMetadata:{...options.usageMetadata,chapterId:topic.id,phase:arbitration?'teaching-plan-arbiter':'teaching-plan-check'}}))
          ...
          if(errors.length){
            ...
            if(retries>=SEMANTIC_PLAN_REPLANS){
              ...
              const current=errors.map(row=>`${row.targetType}:${row.targetId}: ${row.detail}`)
              work.planSemanticRejectedFindings ||= {}
              const rejected=[...new Set([...(work.planSemanticRejectedFindings[topic.id] || []),...current])]
              work.planSemanticRejectedFindings[topic.id]=rejected
              work.planValidation ||= {}
              work.planValidation[topic.id]={issues:rejected,proposal:{...plan,practice}}
              delete work.teachingPlans[topic.id]
        ...
        if(work.repair?.topicId===topic.id){
          ...
          const mechanical=repairRenderingControls(repairContractMechanics(repairBase,work.teachingPlans[topic.id]))
        ...
        const produce = async note => {
          ...
          // Every merged repair already carries the chapter's current plan.
          // This matters for a patch series: after an objective patch is
          // accepted, a later section or practice patch must not replace that
          // plan with the pre-series saved copy and silently undo the edit.
          const plan = merged && work.repair?.topicId === topic.id ? raw.teachingPlan : work.teachingPlans[topic.id]
      ...
      if (chapter) {
        ...
        const repairedProse = repairIncoherentProse(repairRenderingControls(chapter))
        ...
        } else if (pedagogicalPrecheckEnabled(work.billing) && work.pedagogicalPrechecks?.[chapter.id]?.contentHash !== digest(teachingContent(chapter))) {
          // ADDITIVE CHEAP PRE-CHECK. This is enabled by the standard or an
          // explicit route profile and runs once for every new chapter content hash, after all
