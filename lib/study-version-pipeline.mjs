  ...
  try {
    ...
    if (work.stage === 'readiness') {
      ...
      if (topic) {
        ...
        if (work.structuralFill?.topicId === topic.id) {
        ...
        } else if (studyPlanPrecheckEnabled() && work.planSemanticChecks?.[topic.id]?.status !== 'complete') {
          ...
          const savedFailure=work.planSemanticChecks?.[topic.id]?.status==='failed' ? work.planSemanticChecks[topic.id].findings : null
