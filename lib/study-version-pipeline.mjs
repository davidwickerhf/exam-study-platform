...
import { questionCorrectionTrial } from './study-model-routing.mjs'
      ...
      if (topic) {
        ...
        if (!result.split) {
          if (work.repair?.topicId === topic.id) {
            ...
            const targeted = (repairFindings || []).filter(finding => baseIssues.some(issue => issue.detail === finding.detail))
            ...
            if (verdict.rejected) {
              ...
              log('accepted')
              recordCorrectionTrial(work, topic.id, work.repair.modelTrial, 'pending-review', work.repair.modelTrial?.route === 'question-correction' ? {base, findings: repairFindings || [], phase: work.repair.phase} : null)
              acceptChapter(result.prepared, {refreshBase, base, findings: repairFindings || []})
            }
          } else {
