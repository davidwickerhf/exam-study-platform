...
import { studyPlanPrecheckStep } from './study-plan-precheck.mjs'
  ...
  try {
    ...
    if (work.stage === 'readiness') {
      ...
      if (topic) {
        ...
        if (work.structuralFill?.topicId === topic.id) {
        ...
        } else if (studyPlanPrecheckEnabled() && !work.planSemanticChecks?.[topic.id]) {
          ...
          const findings=step.accept(await generate(step.prompt,{...options,responseSchema:step.responseSchema,stage:'quality',reasoningEffort:'low',maxOutputTokens:step.tokens,usageMetadata:{...options.usageMetadata,chapterId:topic.id,phase:'teaching-plan-check'}}))
          const errors=findings.filter(row=>row.severity==='error')
          work.planSemanticLog ||= []
          work.planSemanticLog.push({chapterId:topic.id,attempt:(work.planSemanticAttempts?.[topic.id] || 0)+1,findings:findings.length,errors:errors.length,at:new Date().toISOString()})
          if(work.planSemanticLog.length>200)work.planSemanticLog=work.planSemanticLog.slice(-200)
          if(errors.length){
            const retries=work.planSemanticAttempts?.[topic.id] || 0
            if(retries>=1){
              work.planSemanticChecks ||= {}
              work.planSemanticChecks[topic.id]={status:'failed',findings,at:new Date().toISOString()}
              work.status='failed'
            ...
            work.planSemanticChecks ||= {}
            work.planSemanticChecks[topic.id]={status:'complete',findings,at:new Date().toISOString()}
          }
        } else {
