import { dependencyHash, questionDependencies, boundedReviewItems } from './study-review-dependencies.mjs'
...
function stateFor(chapter, context) {
  ...
  const dependencies={
    solutions:Object.fromEntries(chapter.questions.map(q=>[q.key,dependencyHash({base, ...questionDependencies(chapter,q), question: Object.fromEntries(Object.entries(q).filter(([k])=>['key','question','type','options','sourceIds'].includes(k)))})])),
    judgments:Object.fromEntries([...chapter.questions.map(q=>[`question:${q.key}`,dependencyHash({base,...questionDependencies(chapter,q)})]),...factualReviewItems(chapter).map(i=>[i.key,dependencyHash({base,item:i})])])
  }
  ...
  const legacy=saved?.version===FACTUAL_REVIEW_VERSION && !saved.dependencies && saved.fingerprint===fingerprint
  for(const kind of ['solutions','judgments'])for(const [key,hash] of Object.entries(dependencies[kind]))
    if(saved?.[kind]?.[key] && (legacy || saved.dependencies?.[kind]?.[key]===hash)) state[kind][key]=structuredClone(saved[kind][key])
  return state
}
...
export function nextFactualReview(course,sources,evidence,chapter) {
  ...
  for(const group of [boundedReviewItems(chapter.questions.filter(q=>!state.solutions[q.key]),state.batchSize || 24), boundedReviewItems(chapter.questions.filter(q=>!state.judgments[`question:${q.key}`]),state.batchSize || 24)]) {
    ...
    if(missing.length) {
      ...
      return {kind:'solve',keys:payload.map(q=>q.key),state,schema,responseSchema:studyResponseSchema(schema),tokens:generationLimits.reviewTokens,
    ...
    if(unchecked.length) {
      ...
      return {kind:'answers',keys:payload.map(q=>q.key),state,schema,responseSchema:studyResponseSchema(schema),tokens:generationLimits.reviewTokens,
