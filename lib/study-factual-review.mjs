...
export function nextFactualReview(course,sources,evidence,chapter) {
  ...
  for(const group of batch(chapter.questions,state.batchSize || 4)) {
  ...
  const unchecked=factualReviewItems(chapter).filter(item=>!state.judgments[item.key]).slice(0,state.batchSize || 4)
  if(unchecked.length) {
