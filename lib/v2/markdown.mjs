 ...
 *
 * Lifted verbatim from public/app.js so the migrated tutor renders answers
 * exactly as the vanilla one does, and so test/tutor-markdown.test.mjs — which
 * pins three wrong-but-plausible renderings that shipped before it existed —
 * covers the code both halves run.
 *
