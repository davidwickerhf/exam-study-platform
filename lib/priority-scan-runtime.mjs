  ...
  return {
    ...
    save: (key,result) => own(async () => {
      for (let attempt=0;attempt<3;attempt++) {
        ...
        const previous=Object.entries(held?.batches || {}).filter(([id])=>id!==key)
        // Keep every successful batch in the current corpus, plus a bounded
        // history. Evicting active batches makes larger courses restart forever.
        const entries=[...previous.filter(([id])=>retained.has(id)),...previous.filter(([id])=>!retained.has(id)).slice(-63)]
...
export function priorityModelCall(accountId, bindingId, evidenceHash, {settings=llmSettings,available=chatAvailable,modelCall=callModel,budgetCall=runBudgetedStudyCall} = {}) {
  return (messages, options) => withRequestContext({userId:accountId}, async () => {
    ...
    const billing=await resolveStudyBilling({billingSource:'platform',maxJobUsd:0.50},{configured:available(),provider:'openai',model:options.model || config.model})
