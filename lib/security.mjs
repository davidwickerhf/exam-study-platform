...
const CLERK_ORIGINS = 'https://*.clerk.accounts.dev https://*.clerk.com https://clerk.wicker.life https://clerk.study.wicker.life https://challenges.cloudflare.com'
...
export function contentSecurityPolicy() {
  return [
    ...
    `script-src 'self' ${CDN} ${CLERK_ORIGINS}`,
    ...
    `connect-src 'self' ${CDN} ${CLERK_ORIGINS} https://api.openai.com`,
...
export function securityHeaders({ page = false } = {}) {
  const headers = {
  ...
  if (page) headers['Content-Security-Policy'] = contentSecurityPolicy()
  return headers
}
