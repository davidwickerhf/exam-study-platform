...
export async function forkStudyVersion(id, input = {}, options = {}) {
  ...
  const fork = await createStudyVersion(version.course, version.programmeId, base.snapshot, {
  ...
  const revision = await saveStudyRevision(fork, { ...base, id: `rev-${randomUUID()}`, guides: undefined, execution,
