...
export function compareAcademicDocuments(record, transcript) {
  ...
  if (!record || !transcript) return {
    status: 'awaiting-document', recordCredits: record ? documentCredits(recordRows) : null,
    transcriptCredits: transcript ? documentCredits(transcriptRows) : null,
    recordLabel: record?.sourceLabel || null, transcriptLabel: transcript?.sourceLabel || null,
    counts: { confirmed: 0, conflict: 0, ambiguous: 0, 'record-only': 0, 'transcript-only': 0 },
    checks: [], issues: [...(record?.validation?.issues || []), ...(transcript?.validation?.issues || [])],
    message: 'Comparison becomes available after both documents have been read. You can continue setup with either document; comparison is optional.'
  }
  const checks = []
