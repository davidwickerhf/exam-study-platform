...
const resultKey = (row) => [code(row.code) || name(row.name), year(row.academicYear), row.examDate || '', row.grade ?? '', outcome(row.status), row.creditsTotal, row.creditsEarned].join('|')
...
export function documentRows(draft) {
  return (draft?.courses || []).flatMap((course) => (course.attempts || []).map((attempt) => ({
    ...
    examDate: attempt.examDate, grade: attempt.grade, status: attempt.status,
...
export function compareAcademicDocuments(record, transcript) {
  ...
  for (const row of transcriptRows.filter((item) => item.status !== 'upcoming')) {
    ...
    const status = identities.size > 1 || agreeing.length && agreeing.every((other) => matched.has(other)) ? 'ambiguous' : agreeing.length ? 'confirmed' : candidates.length ? 'conflict' : 'transcript-only'
    if (status === 'confirmed') matched.add(agreeing.find((other) => !matched.has(other)))
    checks.push({ status, course: row.code || row.name, name: row.name, academicYear: row.academicYear, transcript: row, record: candidates })
  }
  for (const row of recordRows.filter((item) => item.status !== 'upcoming' && !matched.has(item))) {
    if (!checks.some((check) => check.record.includes(row))) checks.push({ status: 'record-only', course: row.code || row.name, name: row.name, academicYear: row.academicYear, transcript: null, record: [row] })
  }
