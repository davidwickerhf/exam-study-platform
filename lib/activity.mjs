...
export async function readActivity({ since = null } = {}) {
  ...
  if (sql) {
    const rows = await sql`SELECT type, course_id, chapter_id, score, label, created_at
      FROM activity_events WHERE user_id = ${userId} AND programme_id = ${programmeId} AND created_at >= ${cutoff}::timestamptz
      ORDER BY created_at ASC`
    return rows.map(normalizeEvent)
  }
  const value = await readDocument('activity', scopedDocumentKey(programmeId, 'log'), { events: [] })
...
export async function recordActivity(type, meta = {}, { now = new Date() } = {}) {
  ...
  if (sql) {
    const userId = currentUserId()
    const programmeId = await activeProgrammeId()
    await sql`INSERT INTO activity_events (id, user_id, programme_id, type, course_id, chapter_id, score, label, created_at)
      VALUES (${randomUUID()}, ${userId}, ${programmeId}, ${event.type}, ${event.courseId}, ${event.chapterId}, ${event.score}, ${event.label}, ${event.at}::timestamptz)`
    return event
  }
  const events = await readActivity()
  events.push(event)
  const programmeId = await activeProgrammeId()
  await writeDocument('activity', scopedDocumentKey(programmeId, 'log'), { events: events.slice(-MAX_LOCAL_EVENTS) })
  return event
}
...
export async function deleteActivity() {
  ...
  const programmeId = await activeProgrammeId()
  if (sql) {
    const rows = await sql`DELETE FROM activity_events WHERE user_id = ${userId} AND programme_id = ${programmeId} RETURNING id`
    return rows.length
  }
  const events = await readActivity()
  await deleteDocument('activity', scopedDocumentKey(programmeId, 'log'))
  return events.length
}

// Account deletion is wider than the active study workspace. A student can
// keep several programmes, and privacy controls must not leave an older
// programme's activity ledger behind merely because it is not selected.
export async function deleteAllActivity() {
  ...
  const documents = await (await import('./user-store.mjs')).listDocuments('activity')
  for (const document of documents) await deleteDocument('activity', document.key)
  return documents.reduce((count, document) => count + (Array.isArray(document.value?.events) ? document.value.events.length : 0), 0)
}
