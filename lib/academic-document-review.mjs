...
export async function readDocumentReviews(ids, changes, revision) {
  ...
  for (const change of changes || []) {
    // PostgreSQL JSONB reorders object keys. Compare the reviewed values, not
    // their serialization order; altered fields and array order still matter.
    if (!reviews.some((review) => review.changes.some((held) => isDeepStrictEqual(held, change)))) throw new Error('A proposed result changed after it was checked. Read the document again.')
  }
  return reviews
}
