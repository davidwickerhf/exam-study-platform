...
import {
  deduplicatePaperJobs,
  deduplicatePaperSources,
  paperKind,
  sameCourseEdition,
} from './study-paper-identity.mjs'

export { paperKind } from './study-paper-identity.mjs'
  ...
  const sets = (await listDocuments('study-practice'))
    ...
    .filter(
      (r) =>
        ...
        r.snapshot.sources.every(
          (saved) =>
            allowed.has(saved.key) ||
            sources.some(
              (current) => saved.sha256 && current.sha256 === saved.sha256,
            ),
        ),
    )
  const sourceKeys = new Set(sets.map((s) => s.questionSourceKey))
  const papers = deduplicatePaperSources(sources, sourceKeys)
  const canonicalKey = new Map(
    papers.flatMap((paper) =>
      paper.sourceKeys.map((key) => [key, paper.key]),
    ),
  )
  const canonicalDigest = new Map(
    papers
      .filter((paper) => paper.sha256)
      .map((paper) => [paper.sha256, paper.key]),
  )
    ...
    .filter(
      (r) =>
        ...
        r.snapshot.sources.every((s) =>
          sources.some(
            (a) =>
              (a.key === s.key && a.sha256 === s.sha256) ||
              (s.sha256 && a.sha256 === s.sha256),
          ),
        ),
    )
  const jobs = deduplicatePaperJobs(
    (await listDocuments(PAPER_JOBS))
      .map((r) => r.value)
      .filter(
        (job) =>
          job.programmeId === version.programmeId &&
          sameCourseEdition(job.course, version.course) &&
          sources.some(
            (source) =>
              (job.sha256 && source.sha256 === job.sha256) ||
              (source.key === job.sourceKey && source.sha256 === job.sha256),
          ),
      ),
  )
  return {
    processing: jobs.map((job) => {
      const summary = paperJobSummary(job)
      const paper = papers.find((item) => item.sha256 === job.sha256)
      return paper ? { ...summary, sourceKey: paper.key } : summary
    }),
    ...
    sets: sets
      ...
      .map((r) => ({
        ...
        questionSourceKey: canonicalKey.get(r.questionSourceKey) ||
          canonicalDigest.get(
            r.snapshot.sources.find(
              (source) => source.key === r.questionSourceKey,
            )?.sha256,
          ) ||
          r.questionSourceKey,
        status: r.status,
