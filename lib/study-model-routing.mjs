...
const phases=new Set(['source-mapping','course-outline','teaching-plan','teaching-plan-check','teaching-plan-repair','teaching-plan-arbiter','authoring','structural-fill','pedagogical-precheck','factual-review','pedagogical-review','correction','question-correction'])
// The measured production profile. GPT-6 Sol owns the work where judgment and
// synthesis matter; Mini handles bounded extraction and verification. Question
// correction deliberately falls back to `correction`: the Mini-first pilot
// sometimes bought two extra review passes before Sol fixed the same patch.
export const STANDARD_STUDY_MODEL_ROUTES=Object.freeze({version:1,routes:Object.freeze({
 'source-mapping':Object.freeze({model:'gpt-5-mini',reasoning:'low'}),
 'course-outline':Object.freeze({model:'gpt-5-mini',reasoning:'medium'}),
 'teaching-plan':Object.freeze({model:'gpt-5-mini',reasoning:'medium'}),
 'teaching-plan-check':Object.freeze({model:'gpt-5-mini',reasoning:'low'}),
 'teaching-plan-repair':Object.freeze({model:'gpt-6-sol',reasoning:'medium'}),
 'teaching-plan-arbiter':Object.freeze({model:'gpt-6-sol',reasoning:'medium'}),
 authoring:Object.freeze({model:'gpt-6-sol',reasoning:'medium'}),
 'structural-fill':Object.freeze({model:'gpt-5-mini',reasoning:'medium'}),
 'pedagogical-precheck':Object.freeze({model:'gpt-5-mini',reasoning:'low'}),
 'factual-review':Object.freeze({model:'gpt-5-mini',reasoning:'low'}),
 'pedagogical-review':Object.freeze({model:'gpt-6-sol',reasoning:'medium'}),
 correction:Object.freeze({model:'gpt-6-sol',reasoning:'medium'})
})})
export function effectiveStudyModelRouting(billing, explicit=undefined, env=process.env) {
 if(explicit!==undefined)return explicit
 if(env.STUDY_MODEL_ROUTES)return env.STUDY_MODEL_ROUTES
 return billing?.source==='platform' && billing.provider==='openai' && billing.model==='gpt-6-sol' ? STANDARD_STUDY_MODEL_ROUTES : null
}
...
// The pre-check is additive: enabled by the measured standard profile or an
// explicit operator/pilot route. Other models and existing deployments do not
// gain another paid call merely because they have an authoring/review route.
export function pedagogicalPrecheckEnabled(billing=null,env=process.env) {
 if(effectiveStudyModelRouting(billing,undefined,env)===STANDARD_STUDY_MODEL_ROUTES)return true
 for(const raw of [env.STUDY_MODEL_ROUTES,env.STUDY_PIPELINE_MODEL_ROUTES]) {
  if(!raw)continue
  try{
...
export function studyPlanPrecheckEnabled(billing=null,env=process.env) {
 if(effectiveStudyModelRouting(billing,undefined,env)===STANDARD_STUDY_MODEL_ROUTES)return true
 for(const raw of [env.STUDY_MODEL_ROUTES,env.STUDY_PIPELINE_MODEL_ROUTES]) {
  if(!raw)continue
  try{
