...
export function routeStudyModel(billing,options,rawPolicy) {
 ...
 for(const model of Object.values(policy.routes))if(typeof model!=='string'||STUDY_MODELS[model]?.provider!=='openai')throw new StudyVersionError('A study model route has no supported OpenAI pricing.',503)
 const model=policy.routes[phase];if(!model || model===billing.model)return null
 ...
 return {model,baseModel:billing.model,phase,policyVersion:1}
}
