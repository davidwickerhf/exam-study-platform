...
export function visibleTutorConversation(conversation) {
  ...
  return {
    ...
    messages: (conversation.messages || [])
      ...
      .map(({ role, content, at, evidence, proposals, context, presentation }) => ({ role, content, at, ...(presentation ? { presentation } : {}), evidence: evidence || [], proposals: proposals || [], context: context || null }))
  }
}
...
export async function completeTutorTurn({ base, pending }, turn) {
  return saveConversation({ ...pending, messages: [...base.messages, ...turn.added], reply: { status: 'complete' } }, { expectedRevision: pending.revision })
}
