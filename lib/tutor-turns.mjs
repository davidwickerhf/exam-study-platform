...
export function visibleTutorConversation(conversation) {
  ...
  return {
    ...
    reply: conversation.reply ? {status:conversation.reply.status,startedAt:conversation.reply.startedAt,usage:conversation.reply.usage,...(conversation.reply.status==='failed' || conversation.reply.status==='stopped' ? {failure:tutorFailure(conversation.reply,conversation.reply.status==='stopped')} : {})} : null,
    messages: (conversation.messages || [])
      .filter((message) => ['user', 'assistant'].includes(message.role) && String(message.content || '').trim() && !message.tool_calls?.length)
      .map(({ id, turnId, answerRevision, role, content, at, evidence, proposals, context, presentation, usage }) => ({ id, turnId, answerRevision, role, content, at, ...(usage ? {usage} : {}), ...(presentation ? { presentation } : {}), evidence: evidence || [], proposals: proposals || [], context: context || null }))
  }
}
...
export async function completeTutorTurn({ base, pending }, turn) {
  ...
  return saveConversation({ ...pending, messages: [...base.messages, ...added], reply: { status: 'complete', usage:turn.usage || null } }, { expectedRevision: pending.revision })
}

export async function failTutorTurn({ pending }, error, stopped = false) {
  return saveConversation({ ...pending, reply: { status: stopped ? 'stopped' : 'failed', failure:tutorFailure(error,stopped) } }, { expectedRevision: pending.revision })
}
