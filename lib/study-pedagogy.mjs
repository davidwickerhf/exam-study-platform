...
export function pedagogyPrompt(context, chapter, {quoteReferences=null}={}) {
  ...
  const questionView=({answer,id,sourceIds,...question})=>question
  const artifact={
    title:chapter.title,
    sections:chapter.sections.map(({id,title,text,objectiveIds})=>({id,title,text,objectiveIds})),
...
export function objectiveCoverageIssues(chapter, plan = chapter.teachingPlan) {
  ...
  for (const q of chapter.questions) {
    if (q.practiceStage === 'remediation') {
      if (q.misconceptions?.length) issues.push(`${q.key}: remediation is a terminal follow-up; explain mistakes in its answer instead of creating further links.`)
      if (!chapter.questions.some(source => source.key !== q.key && source.misconceptions?.some(m => m.followUpKey === q.key))) issues.push(`${q.key}: remediation must be linked from a diagnosed mistake.`)
    }
    ...
    if (q.practiceStage !== 'remediation' && q.objectiveIds.some(id => objectives.get(id)?.complexity === 'difficult') && !q.misconceptions?.length) issues.push(`${q.key}: difficult-objective practice needs diagnostic mistakes and related follow-ups.`)
  }
