...
export const pedagogyReviewSchema = z.object({
  ...
  transferChecks: z.array(z.object({ questionKey:key, closestExampleSectionId:key.nullable(), changedCondition:prose, requiresNewReasoning:z.boolean(), rationale:prose })).max(24).default([]),
  followUpChecks: z.array(z.object({ questionKey:key, useful:z.boolean(), rationale:prose })).max(24).default([]),
  ...
  for(const [rows,questions,label] of [[review.transferChecks||[],transfers,'transfer'],[review.followUpChecks||[],followUps,'misconception follow-up']]) {
    ...
    for(const q of questions){
      ...
      if(label==='transfer') {
        ...
        if(!row.requiresNewReasoning)fail(`${q.key}: ${row.rationale}`)
