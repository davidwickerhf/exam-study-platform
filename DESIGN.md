...
- `/app/setup` is a focused, full-screen exception to that shell. The workspace sidebar, sticky mobile header, and bottom navigation are all absent; a small brand bar and the always-visible whole-flow “Skip for now” action are the only surrounding chrome. Four numbered phases span the page above one broad task plane and a 20rem context column, then collapse into one vertical flow on compact screens.
