...
- `/app/setup` is a focused, full-screen exception to that shell. The workspace sidebar, sticky mobile header, and bottom navigation are all absent; a small brand bar and the always-visible whole-flow “Skip for now” action are the only surrounding chrome. Four numbered phases span the page above the work area. There is no StudyMap or decorative inline SVG. At the desktop breakpoint, the Source register occupies the 20rem right column and aligns with the top of the broad active task plane; below that breakpoint, it follows the task plane in document order.
