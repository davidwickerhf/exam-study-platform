...
test('completed legacy routes translate without swallowing unfinished workflows', () => {
  ...
  assert.equal(legacyHashTarget('#/account/connections'), '/app/account?tab=connections')
  assert.equal(legacyHashTarget('#/planning/documents'), '/app/planning?tab=documents')
