...
The preview is configured for Sol/medium and renders enabled Sol/Astra choices. The production default was not switched. The implementation passes typecheck, 1,012 tests and build; these results do not establish that either modern model reliably produces accepted guides. Live evaluation should resume from the saved checkpoint when API credits are available, without resetting automatic correction counts or treating a failed review as passed.
