...
First viewport: a short course identity header, then a 180–200px course navigation column and a white main working plane. Study guides shows every matching guide, private/year context, stored read count and next unread chapter; search matches guide and chapter titles. No redundant result/reading/missing-exam metric strip. No giant card around each section. Navigation groups study work before academic administration.

Signature interaction: browse the guide library, then resume or open a chapter directly. Move to exercises without recreating question controls; switching course sections preserves unfinished answers. Materials begins with the file library; a Manage collection drawer holds sync/year collection tasks. Paper rows preserve original viewing and practice, with repeated processing failures grouped above the library.

Responsive: local navigation becomes a horizontal accessible tab list below 1024px, with keyboard orientation matching the visual layout and the active tab scrolled into view. Year control is explicit on Study guides, Materials and Results; Exercises and Mock papers name their all-years scope. Main content uses one page scroll; no inner chapter scroll. Documents open beside the list; labelled Expand and Back to split view controls change the viewing mode.
...
## September 15 study extension

Materials exposes edition choices beside the library, including All years; optional filters disclose under Filter materials. Compact phone headers and quieter helper copy leave a material row visible. Exercises opens the full question bank, grouped by original paper or generated guide/chapter, with focused answering after selection. Drafts and checked feedback survive question navigation and return to the overview. Mock-paper rows expose completed sections, question counts and source-page ranges; preparation remains in details.

Current direction and validation: `docs/design/study-guide-reader.md`. Local captures: `.impeccable/review/study-redesign/`. The fresh ship review covers the four resolved findings documented there; earlier review history above is retained and is not blanket approval of the expanded workspace.
