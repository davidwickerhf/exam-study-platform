...
- At the two-column desktop breakpoint the date/period band remains fixed while the route and status rail are independent, intrinsic-height scroll regions. At smaller breakpoints the page uses one document scroll beneath the sticky date band instead of nesting scroll containers.
- Outstanding onboarding steps produce one dismissible reminder above Priorities, sourced from `GET /api/onboarding`. It reports the missing count, links to the next actionable setup step, and keys dismissal by workspace plus the exact outstanding-step signature. Complete setup renders no reminder, and the NOW plane never duplicates it.
