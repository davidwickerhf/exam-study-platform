...
const output=process.env.STUDY_IOT_OUTPUT || '/tmp/wicker-iot-v6.json'
const report={model:'gpt-5-mini',calls:0,usd:0,checks:[],artifacts:[]}
try {
  for(const mode of ['worked','shallow','copied']) {
    const shallow=mode==='shallow'
    const f=iotPedagogyFixture({shallow})
    if(mode==='copied')for(const question of f.chapter.questions.filter(q=>q.practiceStage==='transfer')) {const guided=f.chapter.questions.find(q=>q.key===question.key.replace('-transfer','-guided'));question.question=guided.question;question.answer=guided.answer}
    const context=evidencePrompt(f.course,f.sources,f.chunks)
    for(;;){
      ...
      console.log(`${mode}: ${step.objectiveId}`)
      ...
      report.artifacts.push({mode,shallow,objectiveId:step.objectiveId,response:JSON.parse(raw)})
      const aggregate=acceptPedagogicalReview(f.chapter,step,raw)
      if(aggregate)report.checks.push(mode==='copied' ? {name:'Reviewer rejects copied transfer examples',passed:aggregate.transferChecks.length===5 && aggregate.transferChecks.every(q=>q.variation==='copied'),review:aggregate} : pedagogicalEvaluationCheck({kind:shallow?'iot-shallow':'iot-teaching',chapter:f.chapter},aggregate))
