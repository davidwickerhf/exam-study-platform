...
for(const scenario of cases){
  ...
  const result=await runToolLoop({
    ...
    tools:TUTOR_TOOLS,responseFormat:TUTOR_RESPONSE_FORMAT,reviewAnswer:grounding.review,reasoningEffort:'medium',maxRounds:6,maxOutputTokens:8192,parallelTools:true,
    ...
    runTool:async(name,args)=>{
      ...
      if(name==='get_briefing')return {priorities:[{course:code,title:assignment.title,when:assignment.dueAt,url:assignment.url}],announcements:[announcement]}
  ...
  console.log(JSON.stringify({case:scenario.name,calls,answer:raw,usage:result.usage}))
