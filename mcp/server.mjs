...
server.tool('admin_import_canvas_course', 'Download every accessible Canvas module item, file, page, assignment, discussion, quiz, and external-link reference into a structured local course folder. The CLI prompts locally only when it has an interactive terminal; MCP callers should provide courseUrl and outputFolder and name a local short-lived token via accessTokenEnv. Never pass a Canvas password or OTP here. The default only downloads locally. Optional Wicker sync is a separate rights-confirmed candidate review, never publication.', {
